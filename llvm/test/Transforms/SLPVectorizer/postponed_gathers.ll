...
; RUN: %if x86-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -slp-threshold=-10 -mtriple=x86_64-unknown-linux-gnu -S | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -slp-threshold=-10 -mtriple=aarch64-unknown-linux-gnu -S | FileCheck %s %}
