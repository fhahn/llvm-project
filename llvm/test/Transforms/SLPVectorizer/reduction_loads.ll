...
; RUN: %if x86-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=x86_64-apple-macosx10.10.0 -mattr=+sse4.2 | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=aarch64-unknown-linux-gnu -mattr=+neon | FileCheck %s %}
