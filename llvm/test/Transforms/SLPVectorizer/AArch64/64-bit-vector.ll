...
; RUN: opt -S -passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64--linux-gnu -mcpu=generic < %s | FileCheck %s
; RUN: opt -S -passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64-apple-ios -mcpu=cyclone < %s | FileCheck %s
; RUN: opt -S -passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64--linux-gnu -mcpu=kryo < %s | FileCheck --check-prefix=NO_SLP %s
; RUN: opt -S -passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64--linux-gnu -mcpu=generic -slp-min-reg-size=128 < %s | FileCheck --check-prefix=NO_SLP %s
