...
; RUN: opt -mtriple=aarch64 -passes=slp-vectorizer -slp-use-vplan-codegen -mattr=-sve -S < %s | FileCheck %s --check-prefixes=CHECK,NO-SVE
; RUN: opt -mtriple=aarch64 -passes=slp-vectorizer -slp-use-vplan-codegen -mattr=+sve -S < %s | FileCheck %s --check-prefixes=CHECK,SVE
