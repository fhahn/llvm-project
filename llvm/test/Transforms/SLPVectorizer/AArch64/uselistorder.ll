...
; RUN: opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -pass-remarks-missed=slp-vectorizer 2>&1 | FileCheck --check-prefix=VPLAN %s
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: @uselistorder_test(
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <2 x double> poison, double 0.000000e+00, i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = insertelement <2 x double> [[TMP1]], double 0.000000e+00, i32 1
; VPLAN-NEXT:    [[TMP3:%.*]] = fadd <2 x double> [[TMP2]], zeroinitializer
; VPLAN-NEXT:    [[TMP4:%.*]] = fmul <2 x double> zeroinitializer, [[TMP3]]
; VPLAN-NEXT:    [[TMP5:%.*]] = fmul <2 x double> [[TMP4]], zeroinitializer
; VPLAN-NEXT:    [[TMP6:%.*]] = select i1 false, <2 x double> zeroinitializer, <2 x double> [[TMP5]]
; VPLAN-NEXT:    [[TMP7:%.*]] = fmul <2 x double> [[TMP6]], zeroinitializer
; VPLAN-NEXT:    [[TMP8:%.*]] = fadd <2 x double> [[TMP7]], zeroinitializer
; VPLAN-NEXT:    store <2 x double> [[TMP8]], ptr null, align 8
; VPLAN-NEXT:    ret void
;
