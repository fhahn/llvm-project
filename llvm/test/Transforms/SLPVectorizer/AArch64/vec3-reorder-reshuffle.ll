...
; RUN: opt -passes=slp-vectorizer -slp-use-vplan-codegen -slp-vectorize-non-power-of-2 -mtriple=arm64-apple-ios -S %s | FileCheck --check-prefixes=VPLAN,VPLAN-NON-POW2 %s
; RUN: opt -passes=slp-vectorizer -slp-use-vplan-codegen -slp-vectorize-non-power-of-2=false -mtriple=arm64-apple-ios -S %s | FileCheck --check-prefixes=VPLAN,VPLAN-POW2-ONLY %s
...
; CHECK-NEXT:    br label [[BB8]]
;
; VPLAN-LABEL: define i1 @reorder_results(
; VPLAN-SAME: ptr [[ARG:%.*]], i1 [[ARG1:%.*]], ptr [[ARG2:%.*]], i64 [[ARG3:%.*]], ptr [[ARG4:%.*]]) {
; VPLAN-NEXT:  bb:
; VPLAN-NEXT:    [[LOAD:%.*]] = load ptr, ptr [[ARG4]], align 8
; VPLAN-NEXT:    [[LOAD4:%.*]] = load i32, ptr [[LOAD]], align 4
; VPLAN-NEXT:    [[GETELEMENTPTR:%.*]] = getelementptr i8, ptr [[LOAD]], i64 4
; VPLAN-NEXT:    [[LOAD5:%.*]] = load i32, ptr [[GETELEMENTPTR]], align 4
; VPLAN-NEXT:    [[GETELEMENTPTR6:%.*]] = getelementptr i8, ptr [[LOAD]], i64 8
; VPLAN-NEXT:    [[LOAD7:%.*]] = load i32, ptr [[GETELEMENTPTR6]], align 4
; VPLAN-NEXT:    br i1 [[ARG1]], label [[BB12:%.*]], label [[BB9:%.*]]
; VPLAN:       bb8:
; VPLAN-NEXT:    ret i1 false
; VPLAN:       bb9:
; VPLAN-NEXT:    [[FREEZE:%.*]] = freeze ptr [[ARG]]
; VPLAN-NEXT:    store i32 [[LOAD4]], ptr [[FREEZE]], align 4
; VPLAN-NEXT:    [[GETELEMENTPTR10:%.*]] = getelementptr i8, ptr [[FREEZE]], i64 4
; VPLAN-NEXT:    store i32 [[LOAD7]], ptr [[GETELEMENTPTR10]], align 4
; VPLAN-NEXT:    [[GETELEMENTPTR11:%.*]] = getelementptr i8, ptr [[FREEZE]], i64 8
; VPLAN-NEXT:    store i32 [[LOAD5]], ptr [[GETELEMENTPTR11]], align 4
; VPLAN-NEXT:    br label [[BB8:%.*]]
; VPLAN:       bb12:
; VPLAN-NEXT:    [[GETELEMENTPTR13:%.*]] = getelementptr [[STRUCT_ZOT:%.*]], ptr [[ARG2]], i64 [[ARG3]]
; VPLAN-NEXT:    store i32 [[LOAD4]], ptr [[GETELEMENTPTR13]], align 4
; VPLAN-NEXT:    [[GETELEMENTPTR14:%.*]] = getelementptr i8, ptr [[GETELEMENTPTR13]], i64 4
; VPLAN-NEXT:    store i32 [[LOAD7]], ptr [[GETELEMENTPTR14]], align 4
; VPLAN-NEXT:    [[GETELEMENTPTR15:%.*]] = getelementptr i8, ptr [[GETELEMENTPTR13]], i64 8
; VPLAN-NEXT:    store i32 [[LOAD5]], ptr [[GETELEMENTPTR15]], align 4
; VPLAN-NEXT:    br label [[BB8]]
;
bb:
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: define void @extract_mask(
; VPLAN-SAME: ptr [[OBJECT:%.*]], double [[CONV503:%.*]], double [[CONV520:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[TMP0:%.*]] = load ptr, ptr [[OBJECT]], align 8
; VPLAN-NEXT:    [[BBOX483:%.*]] = getelementptr float, ptr [[TMP0]]
; VPLAN-NEXT:    [[TMP1:%.*]] = load <2 x float>, ptr [[BBOX483]], align 8
; VPLAN-NEXT:    [[TMP2:%.*]] = fpext <2 x float> [[TMP1]] to <2 x double>
; VPLAN-NEXT:    [[TMP3:%.*]] = shufflevector <2 x double> [[TMP2]], <2 x double> poison, <2 x i32> <i32 1, i32 0>
; VPLAN-NEXT:    [[TMP4:%.*]] = insertelement <2 x double> [[TMP3]], double [[CONV503]], i32 0
; VPLAN-NEXT:    [[TMP5:%.*]] = fcmp ogt <2 x double> [[TMP4]], <double 0.000000e+00, double -2.000000e+10>
; VPLAN-NEXT:    [[TMP6:%.*]] = select <2 x i1> [[TMP5]], <2 x double> [[TMP3]], <2 x double> <double 0.000000e+00, double -2.000000e+10>
; VPLAN-NEXT:    [[TMP7:%.*]] = fsub <2 x double> zeroinitializer, [[TMP6]]
; VPLAN-NEXT:    [[TMP8:%.*]] = fptrunc <2 x double> [[TMP7]] to <2 x float>
; VPLAN-NEXT:    [[TMP9:%.*]] = extractelement <2 x float> [[TMP8]], i32 0
; VPLAN-NEXT:    [[TMP10:%.*]] = extractelement <2 x float> [[TMP8]], i32 1
; VPLAN-NEXT:    [[MUL646:%.*]] = fmul float [[TMP9]], [[TMP10]]
; VPLAN-NEXT:    [[CMP663:%.*]] = fcmp olt float [[MUL646]], 0.000000e+00
; VPLAN-NEXT:    br i1 [[CMP663]], label [[IF_THEN665:%.*]], label [[IF_END668:%.*]]
; VPLAN:       if.then665:
; VPLAN-NEXT:    [[ARRAYIDX656:%.*]] = getelementptr float, ptr [[OBJECT]], i64 10
; VPLAN-NEXT:    [[BBOX651:%.*]] = getelementptr float, ptr [[OBJECT]]
; VPLAN-NEXT:    [[CONV621:%.*]] = fptrunc double [[CONV520]] to float
; VPLAN-NEXT:    [[TMP11:%.*]] = extractelement <2 x double> [[TMP6]], i32 0
; VPLAN-NEXT:    [[TMP12:%.*]] = insertelement <2 x double> poison, double [[CONV503]], i32 0
; VPLAN-NEXT:    [[TMP13:%.*]] = insertelement <2 x double> [[TMP12]], double [[TMP11]], i32 1
; VPLAN-NEXT:    [[TMP14:%.*]] = fptrunc <2 x double> [[TMP13]] to <2 x float>
; VPLAN-NEXT:    store <2 x float> [[TMP14]], ptr [[BBOX651]], align 8
; VPLAN-NEXT:    [[BBOX_SROA_8_0_BBOX666_SROA_IDX:%.*]] = getelementptr float, ptr [[OBJECT]], i64 2
; VPLAN-NEXT:    store float [[CONV621]], ptr [[BBOX_SROA_8_0_BBOX666_SROA_IDX]], align 8
; VPLAN-NEXT:    store <2 x float> [[TMP8]], ptr [[ARRAYIDX656]], align 8
; VPLAN-NEXT:    br label [[IF_END668]]
; VPLAN:       if.end668:
; VPLAN-NEXT:    ret void
;
entry:
...
;
; VPLAN-NON-POW2-LABEL: define void @gather_2(
; VPLAN-NON-POW2-SAME: ptr [[MAT1:%.*]], float [[TMP0:%.*]], float [[TMP1:%.*]]) {
; VPLAN-NON-POW2-NEXT:  entry:
; VPLAN-NON-POW2-NEXT:    [[TMP2:%.*]] = insertelement <3 x float> poison, float [[TMP0]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP3:%.*]] = shufflevector <3 x float> [[TMP2]], <3 x float> poison, <3 x i32> zeroinitializer
; VPLAN-NON-POW2-NEXT:    [[TMP4:%.*]] = insertelement <3 x float> <float 0.000000e+00, float poison, float poison>, float [[TMP1]], i32 1
; VPLAN-NON-POW2-NEXT:    [[TMP5:%.*]] = insertelement <3 x float> [[TMP4]], float [[TMP1]], i32 2
; VPLAN-NON-POW2-NEXT:    [[TMP6:%.*]] = call <3 x float> @llvm.fmuladd.v3f32(<3 x float> [[TMP3]], <3 x float> [[TMP5]], <3 x float> zeroinitializer)
; VPLAN-NON-POW2-NEXT:    [[ARRAYIDX163:%.*]] = getelementptr [4 x [4 x float]], ptr [[MAT1]], i64 0, i64 1
; VPLAN-NON-POW2-NEXT:    [[TMP7:%.*]] = fmul <3 x float> [[TMP6]], zeroinitializer
; VPLAN-NON-POW2-NEXT:    store <3 x float> [[TMP7]], ptr [[ARRAYIDX163]], align 4
; VPLAN-NON-POW2-NEXT:    ret void
;
; VPLAN-POW2-ONLY-LABEL: define void @gather_2(
; VPLAN-POW2-ONLY-SAME: ptr [[MAT1:%.*]], float [[TMP0:%.*]], float [[TMP1:%.*]]) {
; VPLAN-POW2-ONLY-NEXT:  entry:
; VPLAN-POW2-ONLY-NEXT:    [[TMP2:%.*]] = insertelement <2 x float> poison, float [[TMP0]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP3:%.*]] = shufflevector <2 x float> [[TMP2]], <2 x float> poison, <2 x i32> zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[TMP4:%.*]] = insertelement <2 x float> <float 0.000000e+00, float poison>, float [[TMP1]], i32 1
; VPLAN-POW2-ONLY-NEXT:    [[TMP5:%.*]] = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> [[TMP3]], <2 x float> [[TMP4]], <2 x float> zeroinitializer)
; VPLAN-POW2-ONLY-NEXT:    [[TMP6:%.*]] = call float @llvm.fmuladd.f32(float [[TMP0]], float [[TMP1]], float 0.000000e+00)
; VPLAN-POW2-ONLY-NEXT:    [[TMP7:%.*]] = fmul float [[TMP6]], 0.000000e+00
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX163:%.*]] = getelementptr [4 x [4 x float]], ptr [[MAT1]], i64 0, i64 1
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX5_I_I_I280:%.*]] = getelementptr [4 x [4 x float]], ptr [[MAT1]], i64 0, i64 1, i64 2
; VPLAN-POW2-ONLY-NEXT:    [[TMP8:%.*]] = fmul <2 x float> [[TMP5]], zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP8]], ptr [[ARRAYIDX163]], align 4
; VPLAN-POW2-ONLY-NEXT:    store float [[TMP7]], ptr [[ARRAYIDX5_I_I_I280]], align 4
; VPLAN-POW2-ONLY-NEXT:    ret void
;
entry:
...
;
; VPLAN-NON-POW2-LABEL: define i32 @reorder_indices_1(
; VPLAN-NON-POW2-SAME: float [[TMP0:%.*]]) {
; VPLAN-NON-POW2-NEXT:  entry:
; VPLAN-NON-POW2-NEXT:    [[NOR1:%.*]] = alloca [0 x [3 x float]], i32 0, align 4
; VPLAN-NON-POW2-NEXT:    [[TMP1:%.*]] = load <3 x float>, ptr [[NOR1]], align 4
; VPLAN-NON-POW2-NEXT:    [[TMP2:%.*]] = shufflevector <3 x float> [[TMP1]], <3 x float> poison, <3 x i32> <i32 1, i32 2, i32 0>
; VPLAN-NON-POW2-NEXT:    [[TMP3:%.*]] = fneg <3 x float> [[TMP2]]
; VPLAN-NON-POW2-NEXT:    [[TMP4:%.*]] = insertelement <3 x float> poison, float [[TMP0]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP5:%.*]] = shufflevector <3 x float> [[TMP4]], <3 x float> poison, <3 x i32> zeroinitializer
; VPLAN-NON-POW2-NEXT:    [[TMP6:%.*]] = fmul <3 x float> [[TMP3]], [[TMP5]]
; VPLAN-NON-POW2-NEXT:    [[TMP7:%.*]] = call <3 x float> @llvm.fmuladd.v3f32(<3 x float> [[TMP1]], <3 x float> zeroinitializer, <3 x float> [[TMP6]])
; VPLAN-NON-POW2-NEXT:    [[TMP8:%.*]] = call <3 x float> @llvm.fmuladd.v3f32(<3 x float> [[TMP5]], <3 x float> [[TMP7]], <3 x float> zeroinitializer)
; VPLAN-NON-POW2-NEXT:    [[TMP9:%.*]] = fmul <3 x float> [[TMP8]], zeroinitializer
; VPLAN-NON-POW2-NEXT:    store <3 x float> [[TMP9]], ptr [[NOR1]], align 4
; VPLAN-NON-POW2-NEXT:    ret i32 0
;
; VPLAN-POW2-ONLY-LABEL: define i32 @reorder_indices_1(
; VPLAN-POW2-ONLY-SAME: float [[TMP0:%.*]]) {
; VPLAN-POW2-ONLY-NEXT:  entry:
; VPLAN-POW2-ONLY-NEXT:    [[NOR1:%.*]] = alloca [0 x [3 x float]], i32 0, align 4
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX_I:%.*]] = getelementptr float, ptr [[NOR1]], i64 1
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX2_I265:%.*]] = getelementptr float, ptr [[NOR1]], i64 2
; VPLAN-POW2-ONLY-NEXT:    [[TMP2:%.*]] = load <2 x float>, ptr [[ARRAYIDX_I]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[TMP3:%.*]] = load <2 x float>, ptr [[NOR1]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[TMP4:%.*]] = extractelement <2 x float> [[TMP3]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP5:%.*]] = fneg float [[TMP4]]
; VPLAN-POW2-ONLY-NEXT:    [[NEG11_I:%.*]] = fmul float [[TMP5]], [[TMP0]]
; VPLAN-POW2-ONLY-NEXT:    [[TMP1:%.*]] = extractelement <2 x float> [[TMP2]], i32 1
; VPLAN-POW2-ONLY-NEXT:    [[TMP6:%.*]] = call float @llvm.fmuladd.f32(float [[TMP1]], float 0.000000e+00, float [[NEG11_I]])
; VPLAN-POW2-ONLY-NEXT:    [[TMP7:%.*]] = fneg <2 x float> [[TMP2]]
; VPLAN-POW2-ONLY-NEXT:    [[TMP8:%.*]] = insertelement <2 x float> poison, float [[TMP0]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP9:%.*]] = shufflevector <2 x float> [[TMP8]], <2 x float> poison, <2 x i32> zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[TMP10:%.*]] = fmul <2 x float> [[TMP7]], [[TMP9]]
; VPLAN-POW2-ONLY-NEXT:    [[TMP11:%.*]] = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> [[TMP3]], <2 x float> zeroinitializer, <2 x float> [[TMP10]])
; VPLAN-POW2-ONLY-NEXT:    [[TMP15:%.*]] = insertelement <2 x float> poison, float [[TMP0]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP16:%.*]] = shufflevector <2 x float> [[TMP15]], <2 x float> poison, <2 x i32> zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[TMP12:%.*]] = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> [[TMP16]], <2 x float> [[TMP11]], <2 x float> zeroinitializer)
; VPLAN-POW2-ONLY-NEXT:    [[TMP13:%.*]] = call float @llvm.fmuladd.f32(float [[TMP0]], float [[TMP6]], float 0.000000e+00)
; VPLAN-POW2-ONLY-NEXT:    [[TMP14:%.*]] = fmul <2 x float> [[TMP12]], zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[MUL6_I_I_I:%.*]] = fmul float [[TMP13]], 0.000000e+00
; VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP14]], ptr [[NOR1]], align 4
; VPLAN-POW2-ONLY-NEXT:    store float [[MUL6_I_I_I]], ptr [[ARRAYIDX2_I265]], align 4
; VPLAN-POW2-ONLY-NEXT:    ret i32 0
;
entry:
...
; POW2-ONLY-NEXT:    ret void
;
; VPLAN-NON-POW2-LABEL: define void @reorder_indices_2(
; VPLAN-NON-POW2-SAME: ptr [[SPOINT:%.*]]) {
; VPLAN-NON-POW2-NEXT:  entry:
; VPLAN-NON-POW2-NEXT:    [[DSCO:%.*]] = getelementptr float, ptr [[SPOINT]], i64 0
; VPLAN-NON-POW2-NEXT:    [[TMP0:%.*]] = call <3 x float> @llvm.fmuladd.v3f32(<3 x float> zeroinitializer, <3 x float> zeroinitializer, <3 x float> zeroinitializer)
; VPLAN-NON-POW2-NEXT:    [[TMP1:%.*]] = fmul <3 x float> [[TMP0]], zeroinitializer
; VPLAN-NON-POW2-NEXT:    [[TMP2:%.*]] = shufflevector <3 x float> [[TMP1]], <3 x float> poison, <3 x i32> <i32 1, i32 2, i32 0>
; VPLAN-NON-POW2-NEXT:    store <3 x float> [[TMP2]], ptr [[DSCO]], align 4
; VPLAN-NON-POW2-NEXT:    ret void
;
; VPLAN-POW2-ONLY-LABEL: define void @reorder_indices_2(
; VPLAN-POW2-ONLY-SAME: ptr [[SPOINT:%.*]]) {
; VPLAN-POW2-ONLY-NEXT:  entry:
; VPLAN-POW2-ONLY-NEXT:    [[TMP0:%.*]] = extractelement <3 x float> zeroinitializer, i64 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP1:%.*]] = tail call float @llvm.fmuladd.f32(float [[TMP0]], float 0.000000e+00, float 0.000000e+00)
; VPLAN-POW2-ONLY-NEXT:    [[MUL4_I461:%.*]] = fmul float [[TMP1]], 0.000000e+00
; VPLAN-POW2-ONLY-NEXT:    [[DSCO:%.*]] = getelementptr float, ptr [[SPOINT]], i64 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP2:%.*]] = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> zeroinitializer, <2 x float> zeroinitializer, <2 x float> zeroinitializer)
; VPLAN-POW2-ONLY-NEXT:    [[TMP3:%.*]] = fmul <2 x float> [[TMP2]], zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP3]], ptr [[DSCO]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX5_I476:%.*]] = getelementptr float, ptr [[SPOINT]], i64 2
; VPLAN-POW2-ONLY-NEXT:    store float [[MUL4_I461]], ptr [[ARRAYIDX5_I476]], align 4
; VPLAN-POW2-ONLY-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: define void @reorder_indices_2x_load(
; VPLAN-SAME: ptr [[PNG_PTR:%.*]], ptr [[INFO_PTR:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[BIT_DEPTH:%.*]] = getelementptr i8, ptr [[INFO_PTR]], i64 0
; VPLAN-NEXT:    [[TMP0:%.*]] = load i8, ptr [[BIT_DEPTH]], align 4
; VPLAN-NEXT:    [[COLOR_TYPE:%.*]] = getelementptr i8, ptr [[INFO_PTR]], i64 1
; VPLAN-NEXT:    [[TMP1:%.*]] = load i8, ptr [[COLOR_TYPE]], align 1
; VPLAN-NEXT:    [[BIT_DEPTH37_I:%.*]] = getelementptr i8, ptr [[PNG_PTR]], i64 11
; VPLAN-NEXT:    store i8 [[TMP0]], ptr [[BIT_DEPTH37_I]], align 1
; VPLAN-NEXT:    [[COLOR_TYPE39_I:%.*]] = getelementptr i8, ptr [[PNG_PTR]], i64 10
; VPLAN-NEXT:    store i8 [[TMP1]], ptr [[COLOR_TYPE39_I]], align 2
; VPLAN-NEXT:    [[USR_BIT_DEPTH_I:%.*]] = getelementptr i8, ptr [[PNG_PTR]], i64 12
; VPLAN-NEXT:    store i8 [[TMP0]], ptr [[USR_BIT_DEPTH_I]], align 8
; VPLAN-NEXT:    ret void
;
entry:
...
; POW2-ONLY-NEXT:    ret void
;
; VPLAN-NON-POW2-LABEL: define void @reuse_shuffle_indidces_1(
; VPLAN-NON-POW2-SAME: ptr [[COL:%.*]], float [[TMP0:%.*]], float [[TMP1:%.*]]) {
; VPLAN-NON-POW2-NEXT:  entry:
; VPLAN-NON-POW2-NEXT:    [[TMP2:%.*]] = insertelement <3 x float> poison, float [[TMP1]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP3:%.*]] = insertelement <3 x float> [[TMP2]], float [[TMP0]], i32 1
; VPLAN-NON-POW2-NEXT:    [[TMP4:%.*]] = insertelement <3 x float> [[TMP3]], float [[TMP0]], i32 2
; VPLAN-NON-POW2-NEXT:    [[TMP5:%.*]] = fmul <3 x float> [[TMP4]], zeroinitializer
; VPLAN-NON-POW2-NEXT:    [[TMP6:%.*]] = fadd <3 x float> [[TMP5]], zeroinitializer
; VPLAN-NON-POW2-NEXT:    store <3 x float> [[TMP6]], ptr [[COL]], align 4
; VPLAN-NON-POW2-NEXT:    ret void
;
; VPLAN-POW2-ONLY-LABEL: define void @reuse_shuffle_indidces_1(
; VPLAN-POW2-ONLY-SAME: ptr [[COL:%.*]], float [[TMP0:%.*]], float [[TMP1:%.*]]) {
; VPLAN-POW2-ONLY-NEXT:  entry:
; VPLAN-POW2-ONLY-NEXT:    [[TMP2:%.*]] = insertelement <2 x float> poison, float [[TMP1]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP3:%.*]] = insertelement <2 x float> [[TMP2]], float [[TMP0]], i32 1
; VPLAN-POW2-ONLY-NEXT:    [[TMP4:%.*]] = fmul <2 x float> [[TMP3]], zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[TMP5:%.*]] = fadd <2 x float> [[TMP4]], zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP5]], ptr [[COL]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX33:%.*]] = getelementptr float, ptr [[COL]], i64 2
; VPLAN-POW2-ONLY-NEXT:    [[MUL38:%.*]] = fmul float [[TMP0]], 0.000000e+00
; VPLAN-POW2-ONLY-NEXT:    [[TMP6:%.*]] = fadd float [[MUL38]], 0.000000e+00
; VPLAN-POW2-ONLY-NEXT:    store float [[TMP6]], ptr [[ARRAYIDX33]], align 4
; VPLAN-POW2-ONLY-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: define void @reuse_shuffle_indices_2(
; VPLAN-SAME: ptr [[INERTIA:%.*]], double [[TMP0:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <2 x double> poison, double [[TMP0]], i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = shufflevector <2 x double> [[TMP1]], <2 x double> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP3:%.*]] = fptrunc <2 x double> [[TMP2]] to <2 x float>
; VPLAN-NEXT:    [[TMP4:%.*]] = fmul <2 x float> [[TMP3]], zeroinitializer
; VPLAN-NEXT:    [[TMP5:%.*]] = shufflevector <2 x float> [[TMP4]], <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 1, i32 poison>
; VPLAN-NEXT:    [[TMP6:%.*]] = fadd <4 x float> [[TMP5]], <float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float undef>
; VPLAN-NEXT:    [[TMP7:%.*]] = fmul <4 x float> [[TMP6]], <float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float undef>
; VPLAN-NEXT:    [[TMP8:%.*]] = fadd <4 x float> [[TMP7]], <float 0.000000e+00, float 0.000000e+00, float 0.000000e+00, float undef>
; VPLAN-NEXT:    [[TMP9:%.*]] = shufflevector <4 x float> [[TMP8]], <4 x float> poison, <3 x i32> <i32 0, i32 1, i32 2>
; VPLAN-NEXT:    store <3 x float> [[TMP9]], ptr [[INERTIA]], align 4
; VPLAN-NEXT:    ret void
;
entry:
...
; POW2-ONLY-NEXT:    ret void
;
; VPLAN-NON-POW2-LABEL: define void @reuse_shuffle_indices_cost_crash_2(
; VPLAN-NON-POW2-SAME: ptr [[BEZT:%.*]], float [[TMP0:%.*]]) {
; VPLAN-NON-POW2-NEXT:  entry:
; VPLAN-NON-POW2-NEXT:    [[FNEG:%.*]] = fmul float [[TMP0]], 0.000000e+00
; VPLAN-NON-POW2-NEXT:    [[TMP1:%.*]] = insertelement <3 x float> poison, float [[FNEG]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP2:%.*]] = shufflevector <3 x float> [[TMP1]], <3 x float> poison, <3 x i32> zeroinitializer
; VPLAN-NON-POW2-NEXT:    [[TMP3:%.*]] = insertelement <3 x float> <float poison, float poison, float 0.000000e+00>, float [[TMP0]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP6:%.*]] = insertelement <3 x float> [[TMP3]], float [[TMP0]], i32 1
; VPLAN-NON-POW2-NEXT:    [[TMP5:%.*]] = call <3 x float> @llvm.fmuladd.v3f32(<3 x float> [[TMP2]], <3 x float> [[TMP6]], <3 x float> zeroinitializer)
; VPLAN-NON-POW2-NEXT:    store <3 x float> [[TMP5]], ptr [[BEZT]], align 4
; VPLAN-NON-POW2-NEXT:    ret void
;
; VPLAN-POW2-ONLY-LABEL: define void @reuse_shuffle_indices_cost_crash_2(
; VPLAN-POW2-ONLY-SAME: ptr [[BEZT:%.*]], float [[TMP0:%.*]]) {
; VPLAN-POW2-ONLY-NEXT:  entry:
; VPLAN-POW2-ONLY-NEXT:    [[FNEG:%.*]] = fmul float [[TMP0]], 0.000000e+00
; VPLAN-POW2-ONLY-NEXT:    [[TMP1:%.*]] = insertelement <2 x float> poison, float [[TMP0]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP2:%.*]] = shufflevector <2 x float> [[TMP1]], <2 x float> poison, <2 x i32> zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[TMP3:%.*]] = insertelement <2 x float> poison, float [[FNEG]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP4:%.*]] = shufflevector <2 x float> [[TMP3]], <2 x float> poison, <2 x i32> zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[TMP5:%.*]] = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> [[TMP2]], <2 x float> [[TMP4]], <2 x float> zeroinitializer)
; VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP5]], ptr [[BEZT]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[TMP6:%.*]] = tail call float @llvm.fmuladd.f32(float [[FNEG]], float 0.000000e+00, float 0.000000e+00)
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX8_I831:%.*]] = getelementptr float, ptr [[BEZT]], i64 2
; VPLAN-POW2-ONLY-NEXT:    store float [[TMP6]], ptr [[ARRAYIDX8_I831]], align 4
; VPLAN-POW2-ONLY-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: define void @reuse_shuffle_indices_cost_crash_3(
; VPLAN-SAME: ptr [[M:%.*]], double [[CONV:%.*]], double [[CONV2:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[SUB19:%.*]] = fsub double 0.000000e+00, [[CONV2]]
; VPLAN-NEXT:    [[CONV20:%.*]] = fptrunc double [[SUB19]] to float
; VPLAN-NEXT:    store float [[CONV20]], ptr [[M]], align 4
; VPLAN-NEXT:    [[ADD:%.*]] = fadd double [[CONV]], 0.000000e+00
; VPLAN-NEXT:    [[CONV239:%.*]] = fptrunc double [[ADD]] to float
; VPLAN-NEXT:    [[ARRAYIDX25:%.*]] = getelementptr [4 x float], ptr [[M]], i64 0, i64 1
; VPLAN-NEXT:    store float [[CONV239]], ptr [[ARRAYIDX25]], align 4
; VPLAN-NEXT:    [[ADD26:%.*]] = fsub double [[CONV]], [[CONV]]
; VPLAN-NEXT:    [[CONV27:%.*]] = fptrunc double [[ADD26]] to float
; VPLAN-NEXT:    [[ARRAYIDX29:%.*]] = getelementptr [4 x float], ptr [[M]], i64 0, i64 2
; VPLAN-NEXT:    store float [[CONV27]], ptr [[ARRAYIDX29]], align 4
; VPLAN-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: define void @reuse_shuffle_indices_cost_crash_4(
; VPLAN-SAME: double [[CONV7_I:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[DATA_I111:%.*]] = alloca [0 x [0 x [0 x [3 x float]]]], i32 0, align 4
; VPLAN-NEXT:    [[ARRAYIDX_2_I:%.*]] = getelementptr [3 x float], ptr [[DATA_I111]], i64 0, i64 2
; VPLAN-NEXT:    [[MUL17_I_US:%.*]] = fmul double [[CONV7_I]], 0.000000e+00
; VPLAN-NEXT:    [[MUL_2_I_I_US:%.*]] = fmul double [[MUL17_I_US]], 0.000000e+00
; VPLAN-NEXT:    [[TMP0:%.*]] = insertelement <2 x double> poison, double [[CONV7_I]], i32 0
; VPLAN-NEXT:    [[TMP1:%.*]] = shufflevector <2 x double> [[TMP0]], <2 x double> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP2:%.*]] = fadd <2 x double> [[TMP1]], zeroinitializer
; VPLAN-NEXT:    [[ADD_2_I_I_US:%.*]] = fadd double [[MUL_2_I_I_US]], 0.000000e+00
; VPLAN-NEXT:    [[TMP6:%.*]] = insertelement <2 x double> poison, double [[CONV7_I]], i32 0
; VPLAN-NEXT:    [[TMP7:%.*]] = shufflevector <2 x double> [[TMP6]], <2 x double> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP2]], [[TMP7]]
; VPLAN-NEXT:    [[TMP4:%.*]] = fadd <2 x double> [[TMP3]], zeroinitializer
; VPLAN-NEXT:    [[TMP5:%.*]] = fptrunc <2 x double> [[TMP4]] to <2 x float>
; VPLAN-NEXT:    store <2 x float> [[TMP5]], ptr [[DATA_I111]], align 4
; VPLAN-NEXT:    [[CONV_2_I46_US:%.*]] = fptrunc double [[ADD_2_I_I_US]] to float
; VPLAN-NEXT:    store float [[CONV_2_I46_US]], ptr [[ARRAYIDX_2_I]], align 4
; VPLAN-NEXT:    [[CALL2_I_US:%.*]] = load volatile ptr, ptr [[DATA_I111]], align 8
; VPLAN-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: define void @common_mask(
; VPLAN-SAME: ptr [[M:%.*]], double [[CONV:%.*]], double [[CONV2:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[SUB19:%.*]] = fsub double [[CONV]], [[CONV]]
; VPLAN-NEXT:    [[CONV20:%.*]] = fptrunc double [[SUB19]] to float
; VPLAN-NEXT:    store float [[CONV20]], ptr [[M]], align 4
; VPLAN-NEXT:    [[ADD:%.*]] = fadd double [[CONV2]], 0.000000e+00
; VPLAN-NEXT:    [[CONV239:%.*]] = fptrunc double [[ADD]] to float
; VPLAN-NEXT:    [[ARRAYIDX25:%.*]] = getelementptr [4 x float], ptr [[M]], i64 0, i64 1
; VPLAN-NEXT:    store float [[CONV239]], ptr [[ARRAYIDX25]], align 4
; VPLAN-NEXT:    [[ADD26:%.*]] = fsub double 0.000000e+00, [[CONV]]
; VPLAN-NEXT:    [[CONV27:%.*]] = fptrunc double [[ADD26]] to float
; VPLAN-NEXT:    [[ARRAYIDX29:%.*]] = getelementptr [4 x float], ptr [[M]], i64 0, i64 2
; VPLAN-NEXT:    store float [[CONV27]], ptr [[ARRAYIDX29]], align 4
; VPLAN-NEXT:    ret void
...
; CHECK-NEXT:    ret void
;
; VPLAN-LABEL: define void @vec3_extract(
; VPLAN-SAME: <3 x i16> [[PIXEL_SROA_0_4_VEC_INSERT606:%.*]], ptr [[CALL3_I536:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[PIXEL_SROA_0_4_VEC_EXTRACT:%.*]] = extractelement <3 x i16> [[PIXEL_SROA_0_4_VEC_INSERT606]], i64 2
; VPLAN-NEXT:    [[RED668:%.*]] = getelementptr i16, ptr [[CALL3_I536]], i64 2
; VPLAN-NEXT:    store i16 [[PIXEL_SROA_0_4_VEC_EXTRACT]], ptr [[RED668]], align 2
; VPLAN-NEXT:    [[PIXEL_SROA_0_2_VEC_EXTRACT:%.*]] = extractelement <3 x i16> [[PIXEL_SROA_0_4_VEC_INSERT606]], i64 1
; VPLAN-NEXT:    [[GREEN670:%.*]] = getelementptr i16, ptr [[CALL3_I536]], i64 1
; VPLAN-NEXT:    store i16 [[PIXEL_SROA_0_2_VEC_EXTRACT]], ptr [[GREEN670]], align 2
; VPLAN-NEXT:    [[PIXEL_SROA_0_0_VEC_EXTRACT:%.*]] = extractelement <3 x i16> [[PIXEL_SROA_0_4_VEC_INSERT606]], i64 0
; VPLAN-NEXT:    store i16 [[PIXEL_SROA_0_0_VEC_EXTRACT]], ptr [[CALL3_I536]], align 2
; VPLAN-NEXT:    ret void
;
entry:
...
; POW2-ONLY-NEXT:    ret void
;
; VPLAN-NON-POW2-LABEL: define void @can_reorder_vec3_op_with_padding(
; VPLAN-NON-POW2-SAME: ptr [[A:%.*]], <3 x float> [[IN:%.*]]) {
; VPLAN-NON-POW2-NEXT:  entry:
; VPLAN-NON-POW2-NEXT:    [[TMP0:%.*]] = fsub <3 x float> [[IN]], [[IN]]
; VPLAN-NON-POW2-NEXT:    [[TMP1:%.*]] = call <3 x float> @llvm.fmuladd.v3f32(<3 x float> [[TMP0]], <3 x float> splat (float 2.000000e+00), <3 x float> splat (float 3.000000e+00))
; VPLAN-NON-POW2-NEXT:    [[TMP2:%.*]] = fmul <3 x float> [[TMP1]], splat (float 3.000000e+00)
; VPLAN-NON-POW2-NEXT:    [[TMP3:%.*]] = shufflevector <3 x float> [[TMP2]], <3 x float> poison, <3 x i32> <i32 1, i32 2, i32 0>
; VPLAN-NON-POW2-NEXT:    store <3 x float> [[TMP3]], ptr [[A]], align 4
; VPLAN-NON-POW2-NEXT:    ret void
;
; VPLAN-POW2-ONLY-LABEL: define void @can_reorder_vec3_op_with_padding(
; VPLAN-POW2-ONLY-SAME: ptr [[A:%.*]], <3 x float> [[IN:%.*]]) {
; VPLAN-POW2-ONLY-NEXT:  entry:
; VPLAN-POW2-ONLY-NEXT:    [[ARRAYIDX42_I:%.*]] = getelementptr float, ptr [[A]], i64 2
; VPLAN-POW2-ONLY-NEXT:    [[TMP0:%.*]] = extractelement <3 x float> [[IN]], i64 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP6:%.*]] = extractelement <3 x float> [[IN]], i64 0
; VPLAN-POW2-ONLY-NEXT:    [[SUB_I362:%.*]] = fsub float [[TMP0]], [[TMP6]]
; VPLAN-POW2-ONLY-NEXT:    [[TMP1:%.*]] = call float @llvm.fmuladd.f32(float [[SUB_I362]], float 2.000000e+00, float 3.000000e+00)
; VPLAN-POW2-ONLY-NEXT:    [[MUL6_I_I_I_I:%.*]] = fmul float [[TMP1]], 3.000000e+00
; VPLAN-POW2-ONLY-NEXT:    [[TMP8:%.*]] = shufflevector <3 x float> [[IN]], <3 x float> poison, <2 x i32> <i32 1, i32 2>
; VPLAN-POW2-ONLY-NEXT:    [[TMP10:%.*]] = shufflevector <3 x float> [[IN]], <3 x float> poison, <2 x i32> <i32 1, i32 2>
; VPLAN-POW2-ONLY-NEXT:    [[TMP3:%.*]] = fsub <2 x float> [[TMP8]], [[TMP10]]
; VPLAN-POW2-ONLY-NEXT:    [[TMP4:%.*]] = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> [[TMP3]], <2 x float> splat (float 2.000000e+00), <2 x float> splat (float 3.000000e+00))
; VPLAN-POW2-ONLY-NEXT:    [[TMP5:%.*]] = fmul <2 x float> [[TMP4]], splat (float 3.000000e+00)
; VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP5]], ptr [[A]], align 4
; VPLAN-POW2-ONLY-NEXT:    store float [[MUL6_I_I_I_I]], ptr [[ARRAYIDX42_I]], align 4
; VPLAN-POW2-ONLY-NEXT:    ret void
;
entry:
