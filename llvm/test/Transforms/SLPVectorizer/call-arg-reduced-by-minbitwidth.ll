...
; RUN: %if x86-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=x86_64-pc-windows-msvc19.34.0 < %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64-unknown-linux-gnu < %s | FileCheck %s %}
...
; CHECK:       pre:
; CHECK-NEXT:    [[TMP6:%.*]] = call <8 x i8> @llvm.umax.v8i8(<8 x i8> [[TMP5]], <8 x i8> splat (i8 1))
; CHECK-NEXT:    [[TMP9:%.*]] = add <8 x i8> [[TMP6]], splat (i8 1)
