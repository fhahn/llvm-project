...
; RUN: %if x86-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen < %s -mtriple=x86_64-unknown-linux-gnu | FileCheck %s --check-prefix=X86 %}
; RUN: %if aarch64-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen < %s -mtriple=aarch64-unknown-linux-gnu | FileCheck %s --check-prefix=AARCH64 %}
