...
; RUN: %if x86-registered-target %{ opt -mtriple=x86_64-- -passes=slp-vectorizer -slp-use-vplan-codegen -S < %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -mtriple=aarch64-- -passes=slp-vectorizer -slp-use-vplan-codegen -S < %s | FileCheck %s %}
