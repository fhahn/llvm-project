...
; RUN: %if x86-registered-target %{ opt -passes=slp-vectorizer -slp-use-vplan-codegen -S < %s -mtriple=x86_64-unknown-linux -slp-threshold=-165 | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -passes=slp-vectorizer -slp-use-vplan-codegen -S < %s -mtriple=aarch64-unknown-linux -slp-threshold=-165 | FileCheck %s %}
