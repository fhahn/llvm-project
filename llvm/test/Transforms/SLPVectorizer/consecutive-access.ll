...
; RUN: %if x86-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=x86_64-apple-macosx10.9.0 | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=aarch64-unknown-linux-gnu | FileCheck %s %}
