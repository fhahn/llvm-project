...
; RUN: %if x86-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=x86_64-unknown-linux < %s | FileCheck %s --check-prefix=X86 %}
; RUN: %if aarch64-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64-unknown-linux < %s | FileCheck %s --check-prefix=AARCH64 %}
