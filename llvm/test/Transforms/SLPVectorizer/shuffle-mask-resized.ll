...
; RUN: %if x86-registered-target %{ opt -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=x86_64-unknown-linux -slp-threshold=-10 < %s | FileCheck %s %}
; RUN: %if aaarch64-registered-target %{ opt -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=aarch64-unknown-linux -slp-threshold=-10 < %s | FileCheck %s %}
