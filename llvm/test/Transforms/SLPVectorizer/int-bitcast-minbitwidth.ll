...
; RUN: %if x86-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=x86_64-unknown-linux-gnu -slp-threshold=-9 < %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64-unknown-linux-gnu -slp-threshold=-9 < %s | FileCheck %s %}
