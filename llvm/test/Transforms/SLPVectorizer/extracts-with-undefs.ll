...
; RUN: %if x86-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=x86_64-unknown-linux-gnu | FileCheck %s --check-prefix=VPLAN-X86 %}
; RUN: %if aarch64-registered-target %{ opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=aarch64-unknown-linux-gnu | FileCheck %s --check-prefix=VPLAN-AARCH64 %}
...
;
; VPLAN-X86-LABEL: @test(
; VPLAN-X86-NEXT:  entry:
; VPLAN-X86-NEXT:    br label [[BODY:%.*]]
; VPLAN-X86:       body:
; VPLAN-X86-NEXT:    [[TMP0:%.*]] = phi <2 x double> [ zeroinitializer, [[ENTRY:%.*]] ], [ zeroinitializer, [[BODY]] ]
; VPLAN-X86-NEXT:    [[TMP1:%.*]] = shufflevector <2 x double> [[TMP0]], <2 x double> <double poison, double 0.000000e+00>, <2 x i32> <i32 1, i32 3>
; VPLAN-X86-NEXT:    [[TMP2:%.*]] = fmul fast <2 x double> <double 0.000000e+00, double undef>, [[TMP1]]
; VPLAN-X86-NEXT:    [[TMP3:%.*]] = call fast double @llvm.vector.reduce.fadd.v2f64(double 0.000000e+00, <2 x double> [[TMP2]])
; VPLAN-X86-NEXT:    [[CMP42_I:%.*]] = fcmp fast ole double [[TMP3]], 0.000000e+00
; VPLAN-X86-NEXT:    br i1 false, label [[BODY]], label [[EXIT:%.*]]
; VPLAN-X86:       exit:
; VPLAN-X86-NEXT:    br i1 false, label [[IF_THEN135_I:%.*]], label [[IF_END209_I:%.*]]
; VPLAN-X86:       if.then135.i:
; VPLAN-X86-NEXT:    [[TMP4:%.*]] = fcmp fast olt <2 x double> [[TMP0]], zeroinitializer
; VPLAN-X86-NEXT:    [[TMP5:%.*]] = extractelement <2 x i1> [[TMP4]], i32 0
; VPLAN-X86-NEXT:    [[TMP6:%.*]] = insertelement <2 x i1> <i1 poison, i1 false>, i1 [[TMP5]], i32 0
; VPLAN-X86-NEXT:    [[TMP7:%.*]] = select <2 x i1> [[TMP6]], <2 x double> zeroinitializer, <2 x double> zeroinitializer
; VPLAN-X86-NEXT:    [[TMP8:%.*]] = fmul fast <2 x double> zeroinitializer, [[TMP7]]
; VPLAN-X86-NEXT:    [[TMP9:%.*]] = fmul fast <2 x double> [[TMP8]], zeroinitializer
; VPLAN-X86-NEXT:    [[TMP10:%.*]] = fadd fast <2 x double> [[TMP9]], zeroinitializer
; VPLAN-X86-NEXT:    br label [[IF_END209_I]]
; VPLAN-X86:       if.end209.i:
; VPLAN-X86-NEXT:    [[TMP11:%.*]] = phi <2 x double> [ [[TMP10]], [[IF_THEN135_I]] ], [ zeroinitializer, [[EXIT]] ]
; VPLAN-X86-NEXT:    ret void
;
; VPLAN-AARCH64-LABEL: @test(
; VPLAN-AARCH64-NEXT:  entry:
; VPLAN-AARCH64-NEXT:    br label [[BODY:%.*]]
; VPLAN-AARCH64:       body:
; VPLAN-AARCH64-NEXT:    [[TMP0:%.*]] = phi <2 x double> [ zeroinitializer, [[ENTRY:%.*]] ], [ zeroinitializer, [[BODY]] ]
; VPLAN-AARCH64-NEXT:    [[TMP1:%.*]] = extractelement <2 x double> [[TMP0]], i32 1
; VPLAN-AARCH64-NEXT:    [[MUL_I478_I:%.*]] = fmul fast double [[TMP1]], 0.000000e+00
; VPLAN-AARCH64-NEXT:    [[MUL7_I485_I:%.*]] = fmul fast double undef, 0.000000e+00
; VPLAN-AARCH64-NEXT:    [[ADD8_I_I:%.*]] = fadd fast double [[MUL_I478_I]], [[MUL7_I485_I]]
; VPLAN-AARCH64-NEXT:    [[CMP42_I:%.*]] = fcmp fast ole double [[ADD8_I_I]], 0.000000e+00
; VPLAN-AARCH64-NEXT:    br i1 false, label [[BODY]], label [[EXIT:%.*]]
; VPLAN-AARCH64:       exit:
; VPLAN-AARCH64-NEXT:    br i1 false, label [[IF_THEN135_I:%.*]], label [[IF_END209_I:%.*]]
; VPLAN-AARCH64:       if.then135.i:
; VPLAN-AARCH64-NEXT:    [[TMP2:%.*]] = fcmp fast olt <2 x double> [[TMP0]], zeroinitializer
; VPLAN-AARCH64-NEXT:    [[TMP3:%.*]] = extractelement <2 x i1> [[TMP2]], i32 0
; VPLAN-AARCH64-NEXT:    [[TMP4:%.*]] = insertelement <2 x i1> <i1 poison, i1 false>, i1 [[TMP3]], i32 0
; VPLAN-AARCH64-NEXT:    [[TMP5:%.*]] = select <2 x i1> [[TMP4]], <2 x double> zeroinitializer, <2 x double> zeroinitializer
; VPLAN-AARCH64-NEXT:    [[TMP6:%.*]] = fmul fast <2 x double> zeroinitializer, [[TMP5]]
; VPLAN-AARCH64-NEXT:    [[TMP7:%.*]] = fmul fast <2 x double> [[TMP6]], zeroinitializer
; VPLAN-AARCH64-NEXT:    [[TMP8:%.*]] = fadd fast <2 x double> [[TMP7]], zeroinitializer
; VPLAN-AARCH64-NEXT:    br label [[IF_END209_I]]
; VPLAN-AARCH64:       if.end209.i:
; VPLAN-AARCH64-NEXT:    [[TMP9:%.*]] = phi <2 x double> [ [[TMP8]], [[IF_THEN135_I]] ], [ zeroinitializer, [[EXIT]] ]
