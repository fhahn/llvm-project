...
; RUN: %if x86-registered-target %{ opt -S -passes=slp-vectorizer -slp-use-vplan-codegen < %s -mtriple=x86_64-unknown-linux-gnu -o - | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -S -passes=slp-vectorizer -slp-use-vplan-codegen < %s -mtriple=aarch64-unknown-linux-gnu -o - | FileCheck %s %}
