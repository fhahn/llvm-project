...
; RUN: %if x86-registered-target %{ opt -S -passes=slp-vectorizer -slp-use-vplan-codegen < %s -mtriple=x86_64-unknown-linux-gnu -o - | FileCheck --check-prefix=VPLAN %s %}
; RUN: %if aarch64-registered-target %{ opt -S -passes=slp-vectorizer -slp-use-vplan-codegen < %s -mtriple=aarch64-unknown-linux-gnu -o - | FileCheck --check-prefix=VPLAN %s %}
...
; VPLAN-LABEL: @foo(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[TMP2:%.*]] = getelementptr float, ptr null, i64 22
; VPLAN-NEXT:    [[TMP3:%.*]] = load <2 x float>, ptr [[TMP2]], align 8
; VPLAN-NEXT:    [[TMP4:%.*]] = shufflevector <2 x float> [[TMP3]], <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
; VPLAN-NEXT:    [[TMP5:%.*]] = shufflevector <4 x float> zeroinitializer, <4 x float> [[TMP4]], <4 x i32> <i32 4, i32 5, i32 2, i32 3>
; VPLAN-NEXT:    [[TMP6:%.*]] = shufflevector <4 x float> [[TMP1:%.*]], <4 x float> zeroinitializer, <4 x i32> <i32 0, i32 5, i32 6, i32 poison>
; VPLAN-NEXT:    [[TMP7:%.*]] = extractelement <2 x float> [[TMP3]], i32 0
; VPLAN-NEXT:    [[TMP8:%.*]] = insertelement <4 x float> [[TMP6]], float [[TMP7]], i64 3
; VPLAN-NEXT:    [[TMP9:%.*]] = fpext <4 x float> [[TMP8]] to <4 x double>
; VPLAN-NEXT:    store <4 x double> [[TMP9]], ptr [[TMP0:%.*]], align 32
; VPLAN-NEXT:    ret void
;
entry:
