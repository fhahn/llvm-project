...
; RUN: opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=riscv64-unknown-linux-gnu -mcpu=sifive-x280 < %s | FileCheck %s
; RUN: opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=riscv64-unknown-linux-gnu -mcpu=sifive-x280 < %s -slp-threshold=-3 | FileCheck %s --check-prefix=THRESH
