...
; RUN: %if x86-registered-target %{ opt -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=x86_64 < %s | FileCheck %s --check-prefixes=CHECK,X86 %}
; RUN: %if aarch64-registered-target %{ opt -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=aarch64 < %s | FileCheck %s --check-prefixes=CHECK,AARCH64 %}
