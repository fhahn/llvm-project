...
; RUN: opt -slp-use-vplan-codegen < %s -mtriple=x86_64-unknown -passes=slp-vectorizer -S | FileCheck %s --check-prefixes=VPLAN-VPLAN,VPLAN-VPLAN-SSE2
; RUN: opt < %s -mtriple=x86_64-unknown -mcpu=corei7 -passes=slp-vectorizer -S | FileCheck %s --check-prefixes=CHECK,SSE4
; RUN: opt -slp-use-vplan-codegen < %s -mtriple=x86_64-unknown -mcpu=corei7 -passes=slp-vectorizer -S | FileCheck %s --check-prefixes=VPLAN-VPLAN,VPLAN-VPLAN-SSE4
; RUN: opt < %s -mtriple=x86_64-unknown -mcpu=corei7-avx -passes=slp-vectorizer -S | FileCheck %s --check-prefixes=CHECK,AVX
; RUN: opt -slp-use-vplan-codegen < %s -mtriple=x86_64-unknown -mcpu=corei7-avx -passes=slp-vectorizer -S | FileCheck %s --check-prefixes=VPLAN-VPLAN,VPLAN-VPLAN-AVX
; RUN: opt < %s -mtriple=x86_64-unknown -mcpu=core-avx2 -passes=slp-vectorizer -S | FileCheck %s --check-prefixes=CHECK,AVX2
; RUN: opt -slp-use-vplan-codegen < %s -mtriple=x86_64-unknown -mcpu=core-avx2 -passes=slp-vectorizer -S | FileCheck %s --check-prefixes=VPLAN-VPLAN,VPLAN-VPLAN-AVX2
...
;
; VPLAN-VPLAN-LABEL: @dot4f64(
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <4 x double>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <4 x double>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <4 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <4 x double> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <4 x double> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd double [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    [[TMP9:%.*]] = extractelement <4 x double> [[TMP3]], i32 2
; VPLAN-VPLAN-NEXT:    [[DOT012:%.*]] = fadd double [[DOT01]], [[TMP9]]
; VPLAN-VPLAN-NEXT:    [[TMP10:%.*]] = extractelement <4 x double> [[TMP3]], i32 3
; VPLAN-VPLAN-NEXT:    [[DOT0123:%.*]] = fadd double [[DOT012]], [[TMP10]]
; VPLAN-VPLAN-NEXT:    ret double [[DOT0123]]
;
...
;
; VPLAN-VPLAN-LABEL: @dot4f32(
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <4 x float>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <4 x float>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <4 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <4 x float> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <4 x float> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd float [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    [[TMP9:%.*]] = extractelement <4 x float> [[TMP3]], i32 2
; VPLAN-VPLAN-NEXT:    [[DOT012:%.*]] = fadd float [[DOT01]], [[TMP9]]
; VPLAN-VPLAN-NEXT:    [[TMP10:%.*]] = extractelement <4 x float> [[TMP3]], i32 3
; VPLAN-VPLAN-NEXT:    [[DOT0123:%.*]] = fadd float [[DOT012]], [[TMP10]]
; VPLAN-VPLAN-NEXT:    ret float [[DOT0123]]
;
...
;
; VPLAN-VPLAN-LABEL: @dot4f64_fast(
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <4 x double>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <4 x double>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <4 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = call fast double @llvm.vector.reduce.fadd.v4f64(double 0.000000e+00, <4 x double> [[TMP3]])
; VPLAN-VPLAN-NEXT:    ret double [[TMP4]]
;
  %ptrx1 = getelementptr inbounds double, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds double, ptr %ptry, i64 1
  %ptrx2 = getelementptr inbounds double, ptr %ptrx, i64 2
...
;
; VPLAN-VPLAN-LABEL: @dot4f32_fast(
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <4 x float>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <4 x float>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <4 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = call fast float @llvm.vector.reduce.fadd.v4f32(float 0.000000e+00, <4 x float> [[TMP3]])
; VPLAN-VPLAN-NEXT:    ret float [[TMP4]]
;
  %ptrx1 = getelementptr inbounds float, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds float, ptr %ptry, i64 1
  %ptrx2 = getelementptr inbounds float, ptr %ptrx, i64 2
...
;
; VPLAN-VPLAN-LABEL: @dot3f64(
; VPLAN-VPLAN-NEXT:    [[PTRX1:%.*]] = getelementptr inbounds double, ptr [[PTRX:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[PTRY1:%.*]] = getelementptr inbounds double, ptr [[PTRY:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[X0:%.*]] = load double, ptr [[PTRX]], align 4
; VPLAN-VPLAN-NEXT:    [[Y0:%.*]] = load double, ptr [[PTRY]], align 4
; VPLAN-VPLAN-NEXT:    [[MUL0:%.*]] = fmul double [[X0]], [[Y0]]
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <2 x double>, ptr [[PTRX1]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <2 x double>, ptr [[PTRY1]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd double [[MUL0]], [[TMP4]]
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <2 x double> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT012:%.*]] = fadd double [[DOT01]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    ret double [[DOT012]]
;
  %ptrx1 = getelementptr inbounds double, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds double, ptr %ptry, i64 1
  %ptrx2 = getelementptr inbounds double, ptr %ptrx, i64 2
...
;
; VPLAN-VPLAN-LABEL: @dot3f32(
; VPLAN-VPLAN-NEXT:    [[PTRX1:%.*]] = getelementptr inbounds float, ptr [[PTRX:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[PTRY1:%.*]] = getelementptr inbounds float, ptr [[PTRY:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[X0:%.*]] = load float, ptr [[PTRX]], align 4
; VPLAN-VPLAN-NEXT:    [[Y0:%.*]] = load float, ptr [[PTRY]], align 4
; VPLAN-VPLAN-NEXT:    [[MUL0:%.*]] = fmul float [[X0]], [[Y0]]
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <2 x float>, ptr [[PTRX1]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <2 x float>, ptr [[PTRY1]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x float> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd float [[MUL0]], [[TMP4]]
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <2 x float> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT012:%.*]] = fadd float [[DOT01]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    ret float [[DOT012]]
;
  %ptrx1 = getelementptr inbounds float, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds float, ptr %ptry, i64 1
  %ptrx2 = getelementptr inbounds float, ptr %ptrx, i64 2
...
; CHECK-NEXT:    ret double [[DOT012]]
;
; VPLAN-VPLAN-LABEL: @dot3f64_fast(
; VPLAN-VPLAN-NEXT:    [[PTRX1:%.*]] = getelementptr inbounds double, ptr [[PTRX:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[PTRY1:%.*]] = getelementptr inbounds double, ptr [[PTRY:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[X0:%.*]] = load double, ptr [[PTRX]], align 4
; VPLAN-VPLAN-NEXT:    [[Y0:%.*]] = load double, ptr [[PTRY]], align 4
...
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd fast double [[MUL0]], [[TMP4]]
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <2 x double> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT012:%.*]] = fadd fast double [[DOT01]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    ret double [[DOT012]]
;
  %ptrx1 = getelementptr inbounds double, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds double, ptr %ptry, i64 1
...
; CHECK-NEXT:    ret float [[DOT012]]
;
; VPLAN-VPLAN-LABEL: @dot3f32_fast(
; VPLAN-VPLAN-NEXT:    [[PTRX1:%.*]] = getelementptr inbounds float, ptr [[PTRX:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[PTRY1:%.*]] = getelementptr inbounds float, ptr [[PTRY:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[X0:%.*]] = load float, ptr [[PTRX]], align 4
; VPLAN-VPLAN-NEXT:    [[Y0:%.*]] = load float, ptr [[PTRY]], align 4
...
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x float> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd fast float [[MUL0]], [[TMP4]]
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <2 x float> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT012:%.*]] = fadd fast float [[DOT01]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    ret float [[DOT012]]
;
  %ptrx1 = getelementptr inbounds float, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds float, ptr %ptry, i64 1
...
;
; VPLAN-VPLAN-LABEL: @dot2f64(
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <2 x double>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <2 x double>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <2 x double> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd double [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    ret double [[DOT01]]
;
  %ptrx1 = getelementptr inbounds double, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds double, ptr %ptry, i64 1
  %x0 = load double, ptr %ptrx, align 4
...
;
; VPLAN-VPLAN-LABEL: @dot2f32(
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = load <2 x float>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load <2 x float>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x float> [[TMP3]], i32 0
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <2 x float> [[TMP3]], i32 1
; VPLAN-VPLAN-NEXT:    [[DOT01:%.*]] = fadd float [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-NEXT:    ret float [[DOT01]]
;
  %ptrx1 = getelementptr inbounds float, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds float, ptr %ptry, i64 1
  %x0 = load float, ptr %ptrx, align 4
...
;
; VPLAN-VPLAN-SSE2-LABEL: @dot2f64_fast(
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP1:%.*]] = load <2 x double>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP2:%.*]] = load <2 x double>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP4:%.*]] = call fast double @llvm.vector.reduce.fadd.v2f64(double 0.000000e+00, <2 x double> [[TMP3]])
; VPLAN-VPLAN-SSE2-NEXT:    ret double [[TMP4]]
;
...
;
; VPLAN-VPLAN-SSE4-LABEL: @dot2f64_fast(
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP1:%.*]] = load <2 x double>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP2:%.*]] = load <2 x double>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP3]], i32 0
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP5:%.*]] = extractelement <2 x double> [[TMP3]], i32 1
; VPLAN-VPLAN-SSE4-NEXT:    [[DOT01:%.*]] = fadd fast double [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-SSE4-NEXT:    ret double [[DOT01]]
;
...
;
; VPLAN-VPLAN-AVX-LABEL: @dot2f64_fast(
; VPLAN-VPLAN-AVX-NEXT:    [[TMP1:%.*]] = load <2 x double>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-AVX-NEXT:    [[TMP2:%.*]] = load <2 x double>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-AVX-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-AVX-NEXT:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP3]], i32 0
; VPLAN-VPLAN-AVX-NEXT:    [[TMP5:%.*]] = extractelement <2 x double> [[TMP3]], i32 1
; VPLAN-VPLAN-AVX-NEXT:    [[DOT01:%.*]] = fadd fast double [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-AVX-NEXT:    ret double [[DOT01]]
;
...
;
; VPLAN-VPLAN-AVX2-LABEL: @dot2f64_fast(
; VPLAN-VPLAN-AVX2-NEXT:    [[PTRX1:%.*]] = getelementptr inbounds double, ptr [[PTRX:%.*]], i64 1
; VPLAN-VPLAN-AVX2-NEXT:    [[PTRY1:%.*]] = getelementptr inbounds double, ptr [[PTRY:%.*]], i64 1
; VPLAN-VPLAN-AVX2-NEXT:    [[X0:%.*]] = load double, ptr [[PTRX]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[Y0:%.*]] = load double, ptr [[PTRY]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[X1:%.*]] = load double, ptr [[PTRX1]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[Y1:%.*]] = load double, ptr [[PTRY1]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[MUL0:%.*]] = fmul double [[X0]], [[Y0]]
; VPLAN-VPLAN-AVX2-NEXT:    [[MUL1:%.*]] = fmul double [[X1]], [[Y1]]
; VPLAN-VPLAN-AVX2-NEXT:    [[DOT01:%.*]] = fadd fast double [[MUL0]], [[MUL1]]
; VPLAN-VPLAN-AVX2-NEXT:    ret double [[DOT01]]
;
  %ptrx1 = getelementptr inbounds double, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds double, ptr %ptry, i64 1
  %x0 = load double, ptr %ptrx, align 4
...
;
; VPLAN-VPLAN-SSE2-LABEL: @dot2f32_fast(
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP1:%.*]] = load <2 x float>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP2:%.*]] = load <2 x float>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP3:%.*]] = fmul <2 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-SSE2-NEXT:    [[TMP4:%.*]] = call fast float @llvm.vector.reduce.fadd.v2f32(float 0.000000e+00, <2 x float> [[TMP3]])
; VPLAN-VPLAN-SSE2-NEXT:    ret float [[TMP4]]
;
...
;
; VPLAN-VPLAN-SSE4-LABEL: @dot2f32_fast(
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP1:%.*]] = load <2 x float>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP2:%.*]] = load <2 x float>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP3:%.*]] = fmul <2 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP4:%.*]] = extractelement <2 x float> [[TMP3]], i32 0
; VPLAN-VPLAN-SSE4-NEXT:    [[TMP5:%.*]] = extractelement <2 x float> [[TMP3]], i32 1
; VPLAN-VPLAN-SSE4-NEXT:    [[DOT01:%.*]] = fadd fast float [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-SSE4-NEXT:    ret float [[DOT01]]
;
...
;
; VPLAN-VPLAN-AVX-LABEL: @dot2f32_fast(
; VPLAN-VPLAN-AVX-NEXT:    [[TMP1:%.*]] = load <2 x float>, ptr [[PTRX:%.*]], align 4
; VPLAN-VPLAN-AVX-NEXT:    [[TMP2:%.*]] = load <2 x float>, ptr [[PTRY:%.*]], align 4
; VPLAN-VPLAN-AVX-NEXT:    [[TMP3:%.*]] = fmul <2 x float> [[TMP1]], [[TMP2]]
; VPLAN-VPLAN-AVX-NEXT:    [[TMP4:%.*]] = extractelement <2 x float> [[TMP3]], i32 0
; VPLAN-VPLAN-AVX-NEXT:    [[TMP5:%.*]] = extractelement <2 x float> [[TMP3]], i32 1
; VPLAN-VPLAN-AVX-NEXT:    [[DOT01:%.*]] = fadd fast float [[TMP4]], [[TMP5]]
; VPLAN-VPLAN-AVX-NEXT:    ret float [[DOT01]]
;
...
;
; VPLAN-VPLAN-AVX2-LABEL: @dot2f32_fast(
; VPLAN-VPLAN-AVX2-NEXT:    [[PTRX1:%.*]] = getelementptr inbounds float, ptr [[PTRX:%.*]], i64 1
; VPLAN-VPLAN-AVX2-NEXT:    [[PTRY1:%.*]] = getelementptr inbounds float, ptr [[PTRY:%.*]], i64 1
; VPLAN-VPLAN-AVX2-NEXT:    [[X0:%.*]] = load float, ptr [[PTRX]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[Y0:%.*]] = load float, ptr [[PTRY]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[X1:%.*]] = load float, ptr [[PTRX1]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[Y1:%.*]] = load float, ptr [[PTRY1]], align 4
; VPLAN-VPLAN-AVX2-NEXT:    [[MUL0:%.*]] = fmul float [[X0]], [[Y0]]
; VPLAN-VPLAN-AVX2-NEXT:    [[MUL1:%.*]] = fmul float [[X1]], [[Y1]]
; VPLAN-VPLAN-AVX2-NEXT:    [[DOT01:%.*]] = fadd fast float [[MUL0]], [[MUL1]]
; VPLAN-VPLAN-AVX2-NEXT:    ret float [[DOT01]]
;
  %ptrx1 = getelementptr inbounds float, ptr %ptrx, i64 1
  %ptry1 = getelementptr inbounds float, ptr %ptry, i64 1
  %x0 = load float, ptr %ptrx, align 4
