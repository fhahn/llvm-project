...
; RUN: opt < %s -passes=slp-vectorizer -S -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=CHECK,LEGACY %s
; RUN: opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=CHECK,VPLAN %s

define void @tiny_tree_fully_vectorizable(ptr noalias nocapture %dst, ptr noalias nocapture readonly %src, i64 %count) #0 {
; LEGACY-LABEL: @tiny_tree_fully_vectorizable(
; LEGACY-NEXT:  entry:
; LEGACY-NEXT:    [[CMP12:%.*]] = icmp eq i64 [[COUNT:%.*]], 0
; LEGACY-NEXT:    br i1 [[CMP12]], label [[FOR_END:%.*]], label [[FOR_BODY:%.*]]
; LEGACY:       for.body:
; LEGACY-NEXT:    [[I_015:%.*]] = phi i64 [ [[INC:%.*]], [[FOR_BODY]] ], [ 0, [[ENTRY:%.*]] ]
; LEGACY-NEXT:    [[DST_ADDR_014:%.*]] = phi ptr [ [[ADD_PTR4:%.*]], [[FOR_BODY]] ], [ [[DST:%.*]], [[ENTRY]] ]
; LEGACY-NEXT:    [[SRC_ADDR_013:%.*]] = phi ptr [ [[ADD_PTR:%.*]], [[FOR_BODY]] ], [ [[SRC:%.*]], [[ENTRY]] ]
; LEGACY-NEXT:    [[TMP0:%.*]] = load <2 x double>, ptr [[SRC_ADDR_013]], align 8
; LEGACY-NEXT:    store <2 x double> [[TMP0]], ptr [[DST_ADDR_014]], align 8
; LEGACY-NEXT:    [[ADD_PTR]] = getelementptr inbounds double, ptr [[SRC_ADDR_013]], i64 [[I_015]]
; LEGACY-NEXT:    [[ADD_PTR4]] = getelementptr inbounds double, ptr [[DST_ADDR_014]], i64 [[I_015]]
; LEGACY-NEXT:    [[INC]] = add i64 [[I_015]], 1
; LEGACY-NEXT:    [[EXITCOND:%.*]] = icmp eq i64 [[INC]], [[COUNT]]
; LEGACY-NEXT:    br i1 [[EXITCOND]], label [[FOR_END]], label [[FOR_BODY]]
; LEGACY:       for.end:
; LEGACY-NEXT:    ret void
;
; VPLAN-LABEL: @tiny_tree_fully_vectorizable(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[CMP12:%.*]] = icmp eq i64 [[COUNT:%.*]], 0
; VPLAN-NEXT:    br i1 [[CMP12]], label [[FOR_END:%.*]], label [[FOR_BODY:%.*]]
; VPLAN:       for.body:
; VPLAN-NEXT:    [[I_015:%.*]] = phi i64 [ [[INC:%.*]], [[FOR_BODY]] ], [ 0, [[ENTRY:%.*]] ]
; VPLAN-NEXT:    [[DST_ADDR_014:%.*]] = phi ptr [ [[ADD_PTR4:%.*]], [[FOR_BODY]] ], [ [[DST:%.*]], [[ENTRY]] ]
; VPLAN-NEXT:    [[SRC_ADDR_013:%.*]] = phi ptr [ [[ADD_PTR:%.*]], [[FOR_BODY]] ], [ [[SRC:%.*]], [[ENTRY]] ]
; VPLAN-NEXT:    [[WIDE_LOAD:%.*]] = load <2 x double>, ptr [[SRC_ADDR_013]], align 8
; VPLAN-NEXT:    store <2 x double> [[WIDE_LOAD]], ptr [[DST_ADDR_014]], align 8
; VPLAN-NEXT:    [[ADD_PTR]] = getelementptr inbounds double, ptr [[SRC_ADDR_013]], i64 [[I_015]]
; VPLAN-NEXT:    [[ADD_PTR4]] = getelementptr inbounds double, ptr [[DST_ADDR_014]], i64 [[I_015]]
; VPLAN-NEXT:    [[INC]] = add i64 [[I_015]], 1
; VPLAN-NEXT:    [[EXITCOND:%.*]] = icmp eq i64 [[INC]], [[COUNT]]
; VPLAN-NEXT:    br i1 [[EXITCOND]], label [[FOR_END]], label [[FOR_BODY]]
; VPLAN:       for.end:
; VPLAN-NEXT:    ret void
;
entry:
...
; LEGACY-LABEL: @tiny_tree_fully_vectorizable2(
; LEGACY-NEXT:  entry:
; LEGACY-NEXT:    [[CMP20:%.*]] = icmp eq i64 [[COUNT:%.*]], 0
; LEGACY-NEXT:    br i1 [[CMP20]], label [[FOR_END:%.*]], label [[FOR_BODY:%.*]]
; LEGACY:       for.body:
; LEGACY-NEXT:    [[I_023:%.*]] = phi i64 [ [[INC:%.*]], [[FOR_BODY]] ], [ 0, [[ENTRY:%.*]] ]
; LEGACY-NEXT:    [[DST_ADDR_022:%.*]] = phi ptr [ [[ADD_PTR8:%.*]], [[FOR_BODY]] ], [ [[DST:%.*]], [[ENTRY]] ]
; LEGACY-NEXT:    [[SRC_ADDR_021:%.*]] = phi ptr [ [[ADD_PTR:%.*]], [[FOR_BODY]] ], [ [[SRC:%.*]], [[ENTRY]] ]
; LEGACY-NEXT:    [[TMP0:%.*]] = load <4 x float>, ptr [[SRC_ADDR_021]], align 4
; LEGACY-NEXT:    store <4 x float> [[TMP0]], ptr [[DST_ADDR_022]], align 4
; LEGACY-NEXT:    [[ADD_PTR]] = getelementptr inbounds float, ptr [[SRC_ADDR_021]], i64 [[I_023]]
; LEGACY-NEXT:    [[ADD_PTR8]] = getelementptr inbounds float, ptr [[DST_ADDR_022]], i64 [[I_023]]
; LEGACY-NEXT:    [[INC]] = add i64 [[I_023]], 1
; LEGACY-NEXT:    [[EXITCOND:%.*]] = icmp eq i64 [[INC]], [[COUNT]]
; LEGACY-NEXT:    br i1 [[EXITCOND]], label [[FOR_END]], label [[FOR_BODY]]
; LEGACY:       for.end:
; LEGACY-NEXT:    ret void
;
; VPLAN-LABEL: @tiny_tree_fully_vectorizable2(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[CMP20:%.*]] = icmp eq i64 [[COUNT:%.*]], 0
; VPLAN-NEXT:    br i1 [[CMP20]], label [[FOR_END:%.*]], label [[FOR_BODY:%.*]]
; VPLAN:       for.body:
; VPLAN-NEXT:    [[I_023:%.*]] = phi i64 [ [[INC:%.*]], [[FOR_BODY]] ], [ 0, [[ENTRY:%.*]] ]
; VPLAN-NEXT:    [[DST_ADDR_022:%.*]] = phi ptr [ [[ADD_PTR8:%.*]], [[FOR_BODY]] ], [ [[DST:%.*]], [[ENTRY]] ]
; VPLAN-NEXT:    [[SRC_ADDR_021:%.*]] = phi ptr [ [[ADD_PTR:%.*]], [[FOR_BODY]] ], [ [[SRC:%.*]], [[ENTRY]] ]
; VPLAN-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x float>, ptr [[SRC_ADDR_021]], align 4
; VPLAN-NEXT:    store <4 x float> [[WIDE_LOAD]], ptr [[DST_ADDR_022]], align 4
; VPLAN-NEXT:    [[ADD_PTR]] = getelementptr inbounds float, ptr [[SRC_ADDR_021]], i64 [[I_023]]
; VPLAN-NEXT:    [[ADD_PTR8]] = getelementptr inbounds float, ptr [[DST_ADDR_022]], i64 [[I_023]]
; VPLAN-NEXT:    [[INC]] = add i64 [[I_023]], 1
; VPLAN-NEXT:    [[EXITCOND:%.*]] = icmp eq i64 [[INC]], [[COUNT]]
; VPLAN-NEXT:    br i1 [[EXITCOND]], label [[FOR_END]], label [[FOR_BODY]]
; VPLAN:       for.end:
; VPLAN-NEXT:    ret void
