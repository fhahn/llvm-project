...
; RUN: opt < %s -passes=slp-vectorizer,dce -slp-use-vplan-codegen -slp-vectorize-non-power-of-2 -S -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=VPLAN-VPLAN,VPLAN-VPLAN-NON-POW2 %s
; RUN: opt < %s -passes=slp-vectorizer,dce -slp-use-vplan-codegen -slp-vectorize-non-power-of-2=false -S -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=VPLAN-VPLAN,VPLAN-VPLAN-POW2-ONLY %s
...
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = getelementptr inbounds float, ptr [[B:%.*]], i64 10
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = load float, ptr [[TMP1]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP3:%.*]] = fmul float [[TMP2]], [[T:%.*]]
; VPLAN-VPLAN-NEXT:    [[TMP4:%.*]] = fpext float [[TMP3]] to double
; VPLAN-VPLAN-NEXT:    [[TMP5:%.*]] = fadd double [[TMP4]], 4.000000e+00
; VPLAN-VPLAN-NEXT:    [[TMP6:%.*]] = fptosi double [[TMP5]] to i8
; VPLAN-VPLAN-NEXT:    store i8 [[TMP6]], ptr [[A:%.*]], align 1
; VPLAN-VPLAN-NEXT:    [[TMP7:%.*]] = getelementptr inbounds float, ptr [[B]], i64 11
; VPLAN-VPLAN-NEXT:    [[TMP8:%.*]] = load float, ptr [[TMP7]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP9:%.*]] = fmul float [[TMP8]], [[T]]
; VPLAN-VPLAN-NEXT:    [[TMP10:%.*]] = fpext float [[TMP9]] to double
; VPLAN-VPLAN-NEXT:    [[TMP11:%.*]] = fadd double [[TMP10]], 5.000000e+00
; VPLAN-VPLAN-NEXT:    [[TMP12:%.*]] = fptosi double [[TMP11]] to i8
; VPLAN-VPLAN-NEXT:    [[TMP13:%.*]] = getelementptr inbounds i8, ptr [[A]], i64 1
; VPLAN-VPLAN-NEXT:    store i8 [[TMP12]], ptr [[TMP13]], align 1
; VPLAN-VPLAN-NEXT:    [[TMP14:%.*]] = getelementptr inbounds float, ptr [[B]], i64 12
; VPLAN-VPLAN-NEXT:    [[TMP15:%.*]] = load float, ptr [[TMP14]], align 4
; VPLAN-VPLAN-NEXT:    [[TMP16:%.*]] = fmul float [[TMP15]], [[T]]
; VPLAN-VPLAN-NEXT:    [[TMP17:%.*]] = fpext float [[TMP16]] to double
; VPLAN-VPLAN-NEXT:    [[TMP18:%.*]] = fadd double [[TMP17]], 6.000000e+00
; VPLAN-VPLAN-NEXT:    [[TMP19:%.*]] = fptosi double [[TMP18]] to i8
; VPLAN-VPLAN-NEXT:    [[TMP20:%.*]] = getelementptr inbounds i8, ptr [[A]], i64 2
; VPLAN-VPLAN-NEXT:    store i8 [[TMP19]], ptr [[TMP20]], align 1
; VPLAN-VPLAN-NEXT:    ret i32 undef
;
...
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = shufflevector <4 x float> [[F:%.*]], <4 x float> poison, <2 x i32> <i32 0, i32 1>
; VPLAN-VPLAN-NEXT:    store <2 x float> [[TMP2]], ptr [[P:%.*]], align 4
...
; CHECK-NEXT:    ret void
;
; VPLAN-VPLAN-LABEL: @test_v4f32_v2f32_splat_store(
; VPLAN-VPLAN-NEXT:    [[X0:%.*]] = extractelement <4 x float> [[F:%.*]], i64 0
; VPLAN-VPLAN-NEXT:    [[P1:%.*]] = getelementptr inbounds float, ptr [[P:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    store float [[X0]], ptr [[P]], align 4
; VPLAN-VPLAN-NEXT:    store float [[X0]], ptr [[P1]], align 4
; VPLAN-VPLAN-NEXT:    ret void
;
  %x0 = extractelement <4 x float> %f, i64 0
  %p1 = getelementptr inbounds float, ptr %p, i64 1
...
;
; VPLAN-VPLAN-NON-POW2-LABEL: @test_v4f32_v3f32_store(
; VPLAN-VPLAN-NON-POW2-NEXT:    [[TMP3:%.*]] = shufflevector <4 x float> [[F:%.*]], <4 x float> poison, <3 x i32> <i32 0, i32 1, i32 2>
; VPLAN-VPLAN-NON-POW2-NEXT:    store <3 x float> [[TMP3]], ptr [[P:%.*]], align 4
; VPLAN-VPLAN-NON-POW2-NEXT:    ret void
;
; VPLAN-VPLAN-POW2-ONLY-LABEL: @test_v4f32_v3f32_store(
; VPLAN-VPLAN-POW2-ONLY-NEXT:    [[X2:%.*]] = extractelement <4 x float> [[F:%.*]], i64 2
; VPLAN-VPLAN-POW2-ONLY-NEXT:    [[P2:%.*]] = getelementptr inbounds float, ptr [[P:%.*]], i64 2
; VPLAN-VPLAN-POW2-ONLY-NEXT:    [[TMP2:%.*]] = shufflevector <4 x float> [[F]], <4 x float> poison, <2 x i32> <i32 0, i32 1>
; VPLAN-VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP2]], ptr [[P]], align 4
; VPLAN-VPLAN-POW2-ONLY-NEXT:    store float [[X2]], ptr [[P2]], align 4
...
; CHECK-NEXT:    ret void
;
; VPLAN-VPLAN-LABEL: @test_v4f32_v3f32_splat_store(
; VPLAN-VPLAN-NEXT:    [[X0:%.*]] = extractelement <4 x float> [[F:%.*]], i64 0
; VPLAN-VPLAN-NEXT:    [[P1:%.*]] = getelementptr inbounds float, ptr [[P:%.*]], i64 1
; VPLAN-VPLAN-NEXT:    [[P2:%.*]] = getelementptr inbounds float, ptr [[P]], i64 2
; VPLAN-VPLAN-NEXT:    store float [[X0]], ptr [[P]], align 4
; VPLAN-VPLAN-NEXT:    store float [[X0]], ptr [[P1]], align 4
; VPLAN-VPLAN-NEXT:    store float [[X0]], ptr [[P2]], align 4
; VPLAN-VPLAN-NEXT:    ret void
;
  %x0 = extractelement <4 x float> %f, i64 0
  %p1 = getelementptr inbounds float, ptr %p, i64 1
...
; CHECK-NEXT:    ret void
;
; VPLAN-VPLAN-LABEL: @test_v4f32_v4f32_store(
; VPLAN-VPLAN-NEXT:    store <4 x float> [[F:%.*]], ptr [[P:%.*]], align 4
; VPLAN-VPLAN-NEXT:    ret void
;
  %x0 = extractelement <4 x float> %f, i64 0
  %x1 = extractelement <4 x float> %f, i64 1
...
; CHECK-NEXT:    ret void
;
; VPLAN-VPLAN-LABEL: @test_v4f32_v4f32_splat_store(
; VPLAN-VPLAN-NEXT:    [[X0:%.*]] = extractelement <4 x float> [[F:%.*]], i64 0
; VPLAN-VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <4 x float> poison, float [[X0]], i32 0
; VPLAN-VPLAN-NEXT:    [[TMP2:%.*]] = shufflevector <4 x float> [[TMP1]], <4 x float> poison, <4 x i32> zeroinitializer
; VPLAN-VPLAN-NEXT:    store <4 x float> [[TMP2]], ptr [[P:%.*]], align 4
; VPLAN-VPLAN-NEXT:    ret void
;
  %x0 = extractelement <4 x float> %f, i64 0
  %p1 = getelementptr inbounds float, ptr %p, i64 1
