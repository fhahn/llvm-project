...
; RUN: opt -slp-use-vplan-codegen -passes=slp-vectorizer -slp-vectorize-non-power-of-2 -mtriple=x86_64-apple-macosx -S %s | FileCheck --check-prefixes=VPLAN-NON-POW2 %s
...
; RUN: opt -slp-use-vplan-codegen -passes=slp-vectorizer -slp-vectorize-non-power-of-2=false -mtriple=x86_64-apple-macosx -S %s | FileCheck --check-prefixes=VPLAN-POW2-ONLY %s
...
;
; VPLAN-NON-POW2-LABEL: define void @test_insert_loads(
; VPLAN-NON-POW2-SAME: ptr [[A:%.*]], ptr noalias [[B:%.*]], float [[TMP0:%.*]]) #[[ATTR0:[0-9]+]] {
; VPLAN-NON-POW2-NEXT:  entry:
; VPLAN-NON-POW2-NEXT:    [[A_28:%.*]] = getelementptr i8, ptr [[A]], i64 28
; VPLAN-NON-POW2-NEXT:    [[L_A_28:%.*]] = load float, ptr [[A_28]], align 4
; VPLAN-NON-POW2-NEXT:    [[A_12:%.*]] = getelementptr i8, ptr [[A]], i64 12
; VPLAN-NON-POW2-NEXT:    [[L_A_12:%.*]] = load float, ptr [[A_12]], align 4
; VPLAN-NON-POW2-NEXT:    [[GEP_4:%.*]] = getelementptr i8, ptr [[B]], i64 4
; VPLAN-NON-POW2-NEXT:    [[L_B_0:%.*]] = load float, ptr [[B]], align 4
; VPLAN-NON-POW2-NEXT:    [[TMP5:%.*]] = insertelement <7 x float> poison, float [[TMP0]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP10:%.*]] = shufflevector <7 x float> [[TMP5]], <7 x float> poison, <7 x i32> zeroinitializer
; VPLAN-NON-POW2-NEXT:    [[TMP1:%.*]] = insertelement <7 x float> <float poison, float poison, float poison, float 4.000000e+00, float 3.000000e+00, float 2.000000e+00, float 1.000000e+00>, float [[L_A_12]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP2:%.*]] = insertelement <7 x float> [[TMP1]], float [[L_A_28]], i32 1
; VPLAN-NON-POW2-NEXT:    [[TMP3:%.*]] = insertelement <7 x float> [[TMP2]], float [[L_A_28]], i32 2
; VPLAN-NON-POW2-NEXT:    [[TMP4:%.*]] = insertelement <7 x float> <float poison, float 0.000000e+00, float 0.000000e+00, float 4.000000e+00, float 3.000000e+00, float 2.000000e+00, float 1.000000e+00>, float [[L_B_0]], i32 0
; VPLAN-NON-POW2-NEXT:    [[TMP11:%.*]] = call <7 x float> @llvm.fmuladd.v7f32(<7 x float> [[TMP10]], <7 x float> [[TMP3]], <7 x float> [[TMP4]])
; VPLAN-NON-POW2-NEXT:    store <7 x float> [[TMP11]], ptr [[GEP_4]], align 4
...
;
; VPLAN-POW2-ONLY-LABEL: define void @test_insert_loads(
; VPLAN-POW2-ONLY-SAME: ptr [[A:%.*]], ptr noalias [[B:%.*]], float [[TMP0:%.*]]) #[[ATTR0:[0-9]+]] {
; VPLAN-POW2-ONLY-NEXT:  entry:
; VPLAN-POW2-ONLY-NEXT:    [[MULADD_0:%.*]] = tail call float @llvm.fmuladd.f32(float [[TMP0]], float 1.000000e+00, float 1.000000e+00)
...
; VPLAN-POW2-ONLY-NEXT:    [[TMP3:%.*]] = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> [[TMP2]], <2 x float> <float 3.000000e+00, float 2.000000e+00>, <2 x float> <float 3.000000e+00, float 2.000000e+00>)
; VPLAN-POW2-ONLY-NEXT:    [[A_28:%.*]] = getelementptr i8, ptr [[A]], i64 28
; VPLAN-POW2-ONLY-NEXT:    [[L_A_28:%.*]] = load float, ptr [[A_28]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[A_12:%.*]] = getelementptr i8, ptr [[A]], i64 12
; VPLAN-POW2-ONLY-NEXT:    [[L_A_12:%.*]] = load float, ptr [[A_12]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[GEP_4:%.*]] = getelementptr i8, ptr [[B]], i64 4
; VPLAN-POW2-ONLY-NEXT:    [[L_B_0:%.*]] = load float, ptr [[B]], align 4
; VPLAN-POW2-ONLY-NEXT:    [[GEP_28:%.*]] = getelementptr i8, ptr [[B]], i64 28
; VPLAN-POW2-ONLY-NEXT:    [[GEP_20:%.*]] = getelementptr i8, ptr [[B]], i64 20
; VPLAN-POW2-ONLY-NEXT:    [[TMP8:%.*]] = insertelement <4 x float> poison, float [[TMP0]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP13:%.*]] = shufflevector <4 x float> [[TMP8]], <4 x float> poison, <4 x i32> zeroinitializer
; VPLAN-POW2-ONLY-NEXT:    [[TMP4:%.*]] = insertelement <4 x float> <float poison, float poison, float poison, float 4.000000e+00>, float [[L_A_12]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP5:%.*]] = insertelement <4 x float> [[TMP4]], float [[L_A_28]], i32 1
; VPLAN-POW2-ONLY-NEXT:    [[TMP6:%.*]] = insertelement <4 x float> [[TMP5]], float [[L_A_28]], i32 2
; VPLAN-POW2-ONLY-NEXT:    [[TMP7:%.*]] = insertelement <4 x float> <float poison, float 0.000000e+00, float 0.000000e+00, float 4.000000e+00>, float [[L_B_0]], i32 0
; VPLAN-POW2-ONLY-NEXT:    [[TMP14:%.*]] = call <4 x float> @llvm.fmuladd.v4f32(<4 x float> [[TMP13]], <4 x float> [[TMP6]], <4 x float> [[TMP7]])
; VPLAN-POW2-ONLY-NEXT:    store <4 x float> [[TMP14]], ptr [[GEP_4]], align 4
; VPLAN-POW2-ONLY-NEXT:    store <2 x float> [[TMP3]], ptr [[GEP_20]], align 4
; VPLAN-POW2-ONLY-NEXT:    store float [[MULADD_0]], ptr [[GEP_28]], align 4
