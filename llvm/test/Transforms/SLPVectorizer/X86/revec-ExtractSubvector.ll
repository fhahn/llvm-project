...
; RUN: opt -slp-use-vplan-codegen -passes=slp-vectorizer -S -mtriple=x86_64-unknown-linux-gnu -slp-revec < %s | FileCheck --check-prefix=VPLAN %s
...
; VPLAN-NEXT:    [[RET0:%.*]] = insertvalue { <2 x float>, <2 x float> } poison, <2 x float> [[TMP3]], 0
