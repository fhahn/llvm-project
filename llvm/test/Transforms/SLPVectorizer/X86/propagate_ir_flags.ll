...
; The VPlan-based codegen path is not run here: for @select_nnan it keeps the
; nnan common to all lanes, while the existing path drops it (the bug this test
; was pre-committed for, see #207982), so the two cannot share check lines. Add
; the run line back once the existing path preserves the flags as well.
