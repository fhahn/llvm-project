...
; VPLAN-LABEL: @foo(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[TMP0:%.*]] = insertelement <4 x i32> poison, i32 [[N:%.*]], i32 0
; VPLAN-NEXT:    [[TMP1:%.*]] = shufflevector <4 x i32> [[TMP0]], <4 x i32> poison, <4 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP2:%.*]] = mul <4 x i32> [[TMP1]], <i32 5, i32 9, i32 8, i32 10>
; VPLAN-NEXT:    [[TMP3:%.*]] = add nsw <4 x i32> [[TMP2]], splat (i32 9)
; VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <4 x i32> [[TMP3]], i32 0
; VPLAN-NEXT:    store <4 x i32> [[TMP3]], ptr [[A:%.*]], align 4
; VPLAN-NEXT:    [[EXTERNALUSE1:%.*]] = add nsw i32 [[TMP4]], [[M:%.*]]
; VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <4 x i32> [[TMP3]], i32 0
; VPLAN-NEXT:    [[EXTERNALUSE2:%.*]] = mul nsw i32 [[TMP5]], [[M]]
; VPLAN-NEXT:    [[ADD10:%.*]] = add nsw i32 [[EXTERNALUSE1]], [[EXTERNALUSE2]]
; VPLAN-NEXT:    ret i32 [[ADD10]]
;
entry:
  %mul = mul nsw i32 %n, 5
