...
; RUN: opt -aa-pipeline='basic-aa,scoped-noalias-aa' -passes=slp-vectorizer -mtriple=x86_64-apple-darwin -S %s | FileCheck --check-prefixes=CHECK,LEGACY %s
; RUN: opt -aa-pipeline='basic-aa,scoped-noalias-aa' -passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=x86_64-apple-darwin -S %s | FileCheck --check-prefixes=CHECK,VPLAN %s
...
; LEGACY-LABEL: @gather_sequence_crash(
; LEGACY-NEXT:  bb:
; LEGACY-NEXT:    br i1 [[C_1:%.*]], label [[BB16:%.*]], label [[BB6:%.*]]
; LEGACY:       bb6:
; LEGACY-NEXT:    [[TMP8:%.*]] = getelementptr inbounds float, ptr [[ARG1:%.*]], i32 3
; LEGACY-NEXT:    [[TMP0:%.*]] = shufflevector <2 x float> [[ARG:%.*]], <2 x float> poison, <4 x i32> <i32 poison, i32 0, i32 1, i32 poison>
; LEGACY-NEXT:    [[TMP1:%.*]] = shufflevector <4 x float> [[TMP0]], <4 x float> <float poison, float poison, float poison, float 0.000000e+00>, <4 x i32> <i32 poison, i32 1, i32 2, i32 7>
; LEGACY-NEXT:    [[TMP2:%.*]] = insertelement <4 x float> poison, float [[ARG2:%.*]], i32 0
; LEGACY-NEXT:    [[TMP3:%.*]] = shufflevector <4 x float> [[TMP1]], <4 x float> [[TMP2]], <4 x i32> <i32 4, i32 1, i32 2, i32 3>
; LEGACY-NEXT:    [[TMP4:%.*]] = fmul <4 x float> [[TMP3]], zeroinitializer
; LEGACY-NEXT:    store <4 x float> [[TMP4]], ptr [[TMP8]], align 4
; LEGACY-NEXT:    ret void
; LEGACY:       bb16:
; LEGACY-NEXT:    br label [[BB17:%.*]]
; LEGACY:       bb17:
; LEGACY-NEXT:    br label [[BB18:%.*]]
; LEGACY:       bb18:
; LEGACY-NEXT:    br label [[BB19:%.*]]
; LEGACY:       bb19:
; LEGACY-NEXT:    br label [[BB20:%.*]]
; LEGACY:       bb20:
; LEGACY-NEXT:    br label [[BB21:%.*]]
; LEGACY:       bb21:
; LEGACY-NEXT:    br label [[BB22:%.*]]
; LEGACY:       bb22:
; LEGACY-NEXT:    [[TMP24:%.*]] = getelementptr float, ptr [[ARG4:%.*]], i64 7
; LEGACY-NEXT:    br i1 [[C_2:%.*]], label [[BB25:%.*]], label [[BB22]]
; LEGACY:       bb25:
; LEGACY-NEXT:    [[TMP26:%.*]] = getelementptr float, ptr [[ARG4]], i64 6
; LEGACY-NEXT:    store float 0.000000e+00, ptr [[TMP24]], align 4
; LEGACY-NEXT:    [[TMP27:%.*]] = load float, ptr [[ARG5:%.*]], align 4
; LEGACY-NEXT:    [[TMP29:%.*]] = fadd float 0.000000e+00, 0.000000e+00
; LEGACY-NEXT:    store float 0.000000e+00, ptr [[TMP26]], align 4
; LEGACY-NEXT:    [[TMP30:%.*]] = getelementptr float, ptr [[ARG4]], i64 4
; LEGACY-NEXT:    [[TMP31:%.*]] = fadd float 0.000000e+00, 0.000000e+00
; LEGACY-NEXT:    store <2 x float> zeroinitializer, ptr [[TMP30]], align 4
; LEGACY-NEXT:    br label [[BB33:%.*]]
; LEGACY:       bb33:
; LEGACY-NEXT:    br label [[BB34:%.*]]
; LEGACY:       bb34:
; LEGACY-NEXT:    [[TMP35:%.*]] = getelementptr float, ptr [[ARG4]], i64 3
; LEGACY-NEXT:    [[TMP37:%.*]] = load float, ptr [[TMP35]], align 4
; LEGACY-NEXT:    [[TMP38:%.*]] = fadd float 0.000000e+00, [[TMP37]]
; LEGACY-NEXT:    store float [[TMP38]], ptr [[TMP35]], align 4
; LEGACY-NEXT:    [[TMP39:%.*]] = getelementptr float, ptr [[ARG4]], i64 1
; LEGACY-NEXT:    [[TMP5:%.*]] = load <2 x float>, ptr [[TMP39]], align 4
; LEGACY-NEXT:    [[TMP6:%.*]] = fadd <2 x float> zeroinitializer, [[TMP5]]
; LEGACY-NEXT:    store <2 x float> [[TMP6]], ptr [[TMP39]], align 4
; LEGACY-NEXT:    [[TMP44:%.*]] = load float, ptr [[ARG3:%.*]], align 4
; LEGACY-NEXT:    [[TMP45:%.*]] = load float, ptr [[ARG4]], align 4
; LEGACY-NEXT:    [[TMP46:%.*]] = fadd float 0.000000e+00, [[TMP45]]
; LEGACY-NEXT:    store float [[TMP46]], ptr [[ARG4]], align 4
; LEGACY-NEXT:    call void @quux()
; LEGACY-NEXT:    br label [[BB47:%.*]]
; LEGACY:       bb47:
; LEGACY-NEXT:    br label [[BB17]]
;
; VPLAN-LABEL: @gather_sequence_crash(
; VPLAN-NEXT:  bb:
; VPLAN-NEXT:    br i1 [[C_1:%.*]], label [[BB16:%.*]], label [[BB6:%.*]]
; VPLAN:       bb6:
; VPLAN-NEXT:    [[TMP8:%.*]] = getelementptr inbounds float, ptr [[ARG1:%.*]], i32 3
; VPLAN-NEXT:    [[TMP0:%.*]] = insertelement <4 x float> <float poison, float poison, float poison, float 0.000000e+00>, float [[ARG2:%.*]], i32 0
; VPLAN-NEXT:    [[TMP1:%.*]] = shufflevector <2 x float> [[ARG:%.*]], <2 x float> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
; VPLAN-NEXT:    [[TMP3:%.*]] = shufflevector <4 x float> [[TMP0]], <4 x float> [[TMP1]], <4 x i32> <i32 0, i32 4, i32 5, i32 3>
; VPLAN-NEXT:    [[TMP4:%.*]] = fmul <4 x float> [[TMP3]], zeroinitializer
; VPLAN-NEXT:    store <4 x float> [[TMP4]], ptr [[TMP8]], align 4
; VPLAN-NEXT:    ret void
; VPLAN:       bb16:
; VPLAN-NEXT:    br label [[BB17:%.*]]
; VPLAN:       bb17:
; VPLAN-NEXT:    br label [[BB18:%.*]]
; VPLAN:       bb18:
; VPLAN-NEXT:    br label [[BB19:%.*]]
; VPLAN:       bb19:
; VPLAN-NEXT:    br label [[BB20:%.*]]
; VPLAN:       bb20:
; VPLAN-NEXT:    br label [[BB21:%.*]]
; VPLAN:       bb21:
; VPLAN-NEXT:    br label [[BB22:%.*]]
; VPLAN:       bb22:
; VPLAN-NEXT:    [[TMP24:%.*]] = getelementptr float, ptr [[ARG4:%.*]], i64 7
; VPLAN-NEXT:    br i1 [[C_2:%.*]], label [[BB25:%.*]], label [[BB22]]
; VPLAN:       bb25:
; VPLAN-NEXT:    [[TMP26:%.*]] = getelementptr float, ptr [[ARG4]], i64 6
; VPLAN-NEXT:    store float 0.000000e+00, ptr [[TMP24]], align 4
; VPLAN-NEXT:    [[TMP27:%.*]] = load float, ptr [[ARG5:%.*]], align 4
; VPLAN-NEXT:    [[TMP29:%.*]] = fadd float 0.000000e+00, 0.000000e+00
; VPLAN-NEXT:    store float 0.000000e+00, ptr [[TMP26]], align 4
; VPLAN-NEXT:    [[TMP30:%.*]] = getelementptr float, ptr [[ARG4]], i64 4
; VPLAN-NEXT:    [[TMP31:%.*]] = fadd float 0.000000e+00, 0.000000e+00
; VPLAN-NEXT:    store <2 x float> zeroinitializer, ptr [[TMP30]], align 4
; VPLAN-NEXT:    br label [[BB33:%.*]]
; VPLAN:       bb33:
; VPLAN-NEXT:    br label [[BB34:%.*]]
; VPLAN:       bb34:
; VPLAN-NEXT:    [[TMP35:%.*]] = getelementptr float, ptr [[ARG4]], i64 3
; VPLAN-NEXT:    [[TMP37:%.*]] = load float, ptr [[TMP35]], align 4
; VPLAN-NEXT:    [[TMP38:%.*]] = fadd float 0.000000e+00, [[TMP37]]
; VPLAN-NEXT:    store float [[TMP38]], ptr [[TMP35]], align 4
; VPLAN-NEXT:    [[TMP39:%.*]] = getelementptr float, ptr [[ARG4]], i64 1
; VPLAN-NEXT:    [[WIDE_LOAD:%.*]] = load <2 x float>, ptr [[TMP39]], align 4
; VPLAN-NEXT:    [[TMP5:%.*]] = fadd <2 x float> zeroinitializer, [[WIDE_LOAD]]
; VPLAN-NEXT:    store <2 x float> [[TMP5]], ptr [[TMP39]], align 4
; VPLAN-NEXT:    [[TMP44:%.*]] = load float, ptr [[ARG3:%.*]], align 4
; VPLAN-NEXT:    [[TMP45:%.*]] = load float, ptr [[ARG4]], align 4
; VPLAN-NEXT:    [[TMP46:%.*]] = fadd float 0.000000e+00, [[TMP45]]
; VPLAN-NEXT:    store float [[TMP46]], ptr [[ARG4]], align 4
; VPLAN-NEXT:    call void @quux()
; VPLAN-NEXT:    br label [[BB47:%.*]]
; VPLAN:       bb47:
; VPLAN-NEXT:    br label [[BB17]]
;
bb:
