...
; RUN: opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefix=VPLAN %s
...
; VPLAN-LABEL: @foo(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[MUL238:%.*]] = add i32 [[M:%.*]], [[N:%.*]]
; VPLAN-NEXT:    [[TMP0:%.*]] = load <4 x i32>, ptr [[A:%.*]], align 4
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <4 x i32> poison, i32 [[MUL238]], i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = shufflevector <4 x i32> [[TMP1]], <4 x i32> poison, <4 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP3:%.*]] = mul <4 x i32> [[TMP0]], [[TMP2]]
...
;
; VPLAN-LABEL: @extr_user(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[MUL238:%.*]] = add i32 [[M:%.*]], [[N:%.*]]
; VPLAN-NEXT:    [[TMP0:%.*]] = load <4 x i32>, ptr [[A:%.*]], align 4
; VPLAN-NEXT:    [[TMP1:%.*]] = extractelement <4 x i32> [[TMP0]], i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = insertelement <4 x i32> poison, i32 [[MUL238]], i32 0
; VPLAN-NEXT:    [[TMP3:%.*]] = shufflevector <4 x i32> [[TMP2]], <4 x i32> poison, <4 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP4:%.*]] = mul <4 x i32> [[TMP0]], [[TMP3]]
; VPLAN-NEXT:    store <4 x i32> [[TMP4]], ptr [[B:%.*]], align 4
; VPLAN-NEXT:    ret i32 [[TMP1]]
;
entry:
  %0 = load i32, ptr %A, align 4
  %mul238 = add i32 %m, %n
...
; CHECK-NEXT:    ret i32 [[TMP1]]
;
; VPLAN-LABEL: @extr_user1(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[MUL238:%.*]] = add i32 [[M:%.*]], [[N:%.*]]
; VPLAN-NEXT:    [[TMP0:%.*]] = load <4 x i32>, ptr [[A:%.*]], align 4
; VPLAN-NEXT:    [[TMP1:%.*]] = extractelement <4 x i32> [[TMP0]], i32 1
; VPLAN-NEXT:    [[TMP2:%.*]] = insertelement <4 x i32> poison, i32 [[MUL238]], i32 0
; VPLAN-NEXT:    [[TMP3:%.*]] = shufflevector <4 x i32> [[TMP2]], <4 x i32> poison, <4 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP4:%.*]] = mul <4 x i32> [[TMP0]], [[TMP3]]
; VPLAN-NEXT:    store <4 x i32> [[TMP4]], ptr [[B:%.*]], align 4
