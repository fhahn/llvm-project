...
; RUN: opt < %s -aa-pipeline=basic-aa -passes=slp-vectorizer -S |FileCheck --check-prefixes=CHECK,LEGACY %s
; RUN: opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S |FileCheck --check-prefixes=CHECK,VPLAN %s
; RUN: opt < %s -aa-pipeline=basic-aa -passes=slp-vectorizer -slp-use-vplan-codegen -S |FileCheck --check-prefixes=CHECK,VPLAN %s
...
; LEGACY-LABEL: @foo1(
; LEGACY-NEXT:    [[TMP1:%.*]] = getelementptr inbounds { ptr, ptr }, ptr [[Y:%.*]], i64 0, i32 0
; LEGACY-NEXT:    [[TMP2:%.*]] = getelementptr inbounds { ptr, ptr }, ptr [[X:%.*]], i64 0, i32 0
; LEGACY-NEXT:    [[TMP3:%.*]] = load <2 x ptr>, ptr [[TMP1]], align 8
; LEGACY-NEXT:    [[TMP4:%.*]] = getelementptr i32, <2 x ptr> [[TMP3]], <2 x i64> splat (i64 16)
; LEGACY-NEXT:    store <2 x ptr> [[TMP4]], ptr [[TMP2]], align 8
; LEGACY-NEXT:    ret void
;
; VPLAN-LABEL: @foo1(
; VPLAN-NEXT:    [[TMP1:%.*]] = getelementptr inbounds { ptr, ptr }, ptr [[Y:%.*]], i64 0, i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = getelementptr inbounds { ptr, ptr }, ptr [[X:%.*]], i64 0, i32 0
; VPLAN-NEXT:    [[TMP3:%.*]] = load <2 x ptr>, ptr [[TMP1]], align 8
; VPLAN-NEXT:    [[TMP4:%.*]] = getelementptr i32, <2 x ptr> [[TMP3]], <2 x i64> splat (i64 16)
; VPLAN-NEXT:    store <2 x ptr> [[TMP4]], ptr [[TMP2]], align 8
; VPLAN-NEXT:    ret void
;
