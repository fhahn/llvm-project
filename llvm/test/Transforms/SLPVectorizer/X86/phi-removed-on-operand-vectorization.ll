...
; RUN: opt -slp-use-vplan-codegen -S --passes=slp-vectorizer -mtriple=i386-pc-windows-msvc19.34.0 -mcpu=pentium4 < %s | FileCheck %s --check-prefix=VPLAN
...
; VPLAN-SAME: double [[MUL321_I:%.*]]) #[[ATTR0:[0-9]+]] {
; VPLAN-NEXT:  [[ENTRY:.*]]:
; VPLAN-NEXT:    br label %[[DO_BODY220_I:.*]]
; VPLAN:       [[DO_BODY220_I]]:
; VPLAN-NEXT:    [[TMP0:%.*]] = phi <2 x double> [ [[TMP6:%.*]], %[[DO_BODY221_I:.*]] ], [ zeroinitializer, %[[ENTRY]] ]
; VPLAN-NEXT:    br label %[[DO_BODY221_I]]
; VPLAN:       [[DO_BODY221_I]]:
; VPLAN-NEXT:    [[TMP1:%.*]] = fadd <2 x double> [[TMP0]], zeroinitializer
; VPLAN-NEXT:    [[TMP2:%.*]] = fmul <2 x double> [[TMP1]], zeroinitializer
; VPLAN-NEXT:    [[TMP3:%.*]] = extractelement <2 x double> [[TMP2]], i32 0
; VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP2]], i32 1
; VPLAN-NEXT:    [[ADD318_I:%.*]] = fadd double [[TMP3]], [[TMP4]]
; VPLAN-NEXT:    [[TMP5:%.*]] = insertelement <2 x double> poison, double [[MUL321_I]], i32 0
; VPLAN-NEXT:    [[TMP6]] = insertelement <2 x double> [[TMP5]], double [[ADD318_I]], i32 1
; VPLAN-NEXT:    br label %[[DO_BODY220_I]]
;
entry:
