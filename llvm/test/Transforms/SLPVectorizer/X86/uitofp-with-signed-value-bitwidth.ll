...
; RUN: opt -S --passes=slp-vectorizer -mtriple=x86_64-unknown-linux-gnu < %s | FileCheck --check-prefix=CHECK %s
...
; CHECK-NEXT:    [[TMP4:%.*]] = uitofp <2 x i64> [[TMP3]] to <2 x double>
; CHECK-NEXT:    [[TMP5:%.*]] = fdiv <2 x double> [[TMP4]], zeroinitializer
; CHECK-NEXT:    [[TMP6:%.*]] = fcmp ogt <2 x double> [[TMP5]], zeroinitializer
; CHECK-NEXT:    [[TMP7:%.*]] = extractelement <2 x i1> [[TMP6]], i32 1
; CHECK-NEXT:    [[G_0_I_I:%.*]] = select i1 [[TMP7]], double 1.000000e+00, double 0.000000e+00
...
; CHECK-NEXT:    br i1 [[TMP8]], label %[[K_EXIT:.*]], label %[[IF_THEN_I:.*]]
; CHECK:       [[IF_THEN_I]]:
