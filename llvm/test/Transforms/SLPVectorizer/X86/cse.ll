...
; RUN: opt < %s -passes=slp-vectorizer -S -mtriple=i386-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=CHECK,LEGACY %s
; RUN: opt < %s -passes=slp-vectorizer -slp-use-vplan-codegen -S -mtriple=i386-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=CHECK,VPLAN %s
...
; LEGACY-LABEL: @foo(
; LEGACY-NEXT:  entry:
; LEGACY-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; LEGACY-NEXT:    [[TMP0:%.*]] = load <4 x double>, ptr [[A:%.*]], align 8
; LEGACY-NEXT:    [[TMP1:%.*]] = fmul <4 x double> [[TMP0]], <double 7.900000e+00, double 7.700000e+00, double 7.600000e+00, double 7.400000e+00>
; LEGACY-NEXT:    [[TMP2:%.*]] = insertelement <4 x double> poison, double [[CONV]], i32 0
; LEGACY-NEXT:    [[TMP3:%.*]] = shufflevector <4 x double> [[TMP2]], <4 x double> poison, <4 x i32> zeroinitializer
; LEGACY-NEXT:    [[TMP4:%.*]] = fmul <4 x double> [[TMP3]], [[TMP1]]
; LEGACY-NEXT:    [[TMP5:%.*]] = fadd <4 x double> [[TMP4]], <double 6.000000e+00, double 2.000000e+00, double 3.000000e+00, double 4.000000e+00>
; LEGACY-NEXT:    store <4 x double> [[TMP5]], ptr [[A]], align 8
; LEGACY-NEXT:    ret i32 undef
;
; VPLAN-LABEL: @foo(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; VPLAN-NEXT:    [[TMP0:%.*]] = load <4 x double>, ptr [[A:%.*]], align 8
; VPLAN-NEXT:    [[TMP1:%.*]] = fmul <4 x double> [[TMP0]], <double 7.900000e+00, double 7.700000e+00, double 7.600000e+00, double 7.400000e+00>
; VPLAN-NEXT:    [[TMP2:%.*]] = insertelement <4 x double> poison, double [[CONV]], i32 0
; VPLAN-NEXT:    [[TMP3:%.*]] = shufflevector <4 x double> [[TMP2]], <4 x double> poison, <4 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP4:%.*]] = fmul <4 x double> [[TMP3]], [[TMP1]]
; VPLAN-NEXT:    [[TMP5:%.*]] = fadd <4 x double> [[TMP4]], <double 6.000000e+00, double 2.000000e+00, double 3.000000e+00, double 4.000000e+00>
; VPLAN-NEXT:    store <4 x double> [[TMP5]], ptr [[A]], align 8
; VPLAN-NEXT:    ret i32 undef
;
entry:
...
; LEGACY-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; LEGACY-NEXT:    [[TMP0:%.*]] = load <4 x double>, ptr [[A:%.*]], align 8
; LEGACY-NEXT:    [[TMP1:%.*]] = fmul <4 x double> [[TMP0]], splat (double 7.900000e+00)
; LEGACY-NEXT:    [[TMP2:%.*]] = insertelement <4 x double> poison, double [[CONV]], i32 0
; LEGACY-NEXT:    [[TMP3:%.*]] = shufflevector <4 x double> [[TMP2]], <4 x double> poison, <4 x i32> zeroinitializer
; LEGACY-NEXT:    [[TMP4:%.*]] = fmul <4 x double> [[TMP3]], [[TMP1]]
; LEGACY-NEXT:    [[TMP5:%.*]] = fadd <4 x double> [[TMP4]], splat (double 6.000000e+00)
; LEGACY-NEXT:    store <4 x double> [[TMP5]], ptr [[A]], align 8
; LEGACY-NEXT:    ret i32 undef
;
; VPLAN-LABEL: @foo4(
...
; VPLAN-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; VPLAN-NEXT:    [[TMP0:%.*]] = load <4 x double>, ptr [[A:%.*]], align 8
; VPLAN-NEXT:    [[TMP1:%.*]] = fmul <4 x double> [[TMP0]], splat (double 7.900000e+00)
; VPLAN-NEXT:    [[TMP2:%.*]] = insertelement <4 x double> poison, double [[CONV]], i32 0
; VPLAN-NEXT:    [[TMP3:%.*]] = shufflevector <4 x double> [[TMP2]], <4 x double> poison, <4 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP4:%.*]] = fmul <4 x double> [[TMP3]], [[TMP1]]
; VPLAN-NEXT:    [[TMP5:%.*]] = fadd <4 x double> [[TMP4]], splat (double 6.000000e+00)
; VPLAN-NEXT:    store <4 x double> [[TMP5]], ptr [[A]], align 8
; VPLAN-NEXT:    ret i32 undef
;
entry:
...
; LEGACY-LABEL: @partial_mrg(
; LEGACY-NEXT:  entry:
; LEGACY-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; LEGACY-NEXT:    [[TMP0:%.*]] = load <2 x double>, ptr [[A:%.*]], align 8
; LEGACY-NEXT:    [[TMP1:%.*]] = insertelement <2 x double> poison, double [[CONV]], i32 0
; LEGACY-NEXT:    [[TMP2:%.*]] = shufflevector <2 x double> [[TMP1]], <2 x double> poison, <2 x i32> zeroinitializer
; LEGACY-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP2]], [[TMP0]]
; LEGACY-NEXT:    store <2 x double> [[TMP3]], ptr [[A]], align 8
; LEGACY-NEXT:    [[CMP:%.*]] = icmp slt i32 [[N]], 4
; LEGACY-NEXT:    br i1 [[CMP]], label [[RETURN:%.*]], label [[IF_END:%.*]]
; LEGACY:       if.end:
; LEGACY-NEXT:    [[ARRAYIDX7:%.*]] = getelementptr inbounds double, ptr [[A]], i64 2
; LEGACY-NEXT:    [[ADD:%.*]] = add nsw i32 [[N]], 4
; LEGACY-NEXT:    [[CONV12:%.*]] = sitofp i32 [[ADD]] to double
; LEGACY-NEXT:    [[TMP4:%.*]] = load <2 x double>, ptr [[ARRAYIDX7]], align 8
; LEGACY-NEXT:    [[TMP5:%.*]] = insertelement <2 x double> [[TMP1]], double [[CONV12]], i32 1
; LEGACY-NEXT:    [[TMP6:%.*]] = fmul <2 x double> [[TMP5]], [[TMP4]]
; LEGACY-NEXT:    store <2 x double> [[TMP6]], ptr [[ARRAYIDX7]], align 8
; LEGACY-NEXT:    br label [[RETURN]]
; LEGACY:       return:
; LEGACY-NEXT:    ret i32 0
;
; VPLAN-LABEL: @partial_mrg(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; VPLAN-NEXT:    [[TMP0:%.*]] = load <2 x double>, ptr [[A:%.*]], align 8
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <2 x double> poison, double [[CONV]], i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = shufflevector <2 x double> [[TMP1]], <2 x double> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP2]], [[TMP0]]
; VPLAN-NEXT:    store <2 x double> [[TMP3]], ptr [[A]], align 8
; VPLAN-NEXT:    [[CMP:%.*]] = icmp slt i32 [[N]], 4
; VPLAN-NEXT:    br i1 [[CMP]], label [[RETURN:%.*]], label [[IF_END:%.*]]
; VPLAN:       if.end:
; VPLAN-NEXT:    [[ARRAYIDX7:%.*]] = getelementptr inbounds double, ptr [[A]], i64 2
; VPLAN-NEXT:    [[ADD:%.*]] = add nsw i32 [[N]], 4
; VPLAN-NEXT:    [[CONV12:%.*]] = sitofp i32 [[ADD]] to double
; VPLAN-NEXT:    [[WIDE_LOAD:%.*]] = load <2 x double>, ptr [[ARRAYIDX7]], align 8
; VPLAN-NEXT:    [[TMP4:%.*]] = insertelement <2 x double> poison, double [[CONV]], i32 0
; VPLAN-NEXT:    [[TMP5:%.*]] = insertelement <2 x double> [[TMP4]], double [[CONV12]], i32 1
; VPLAN-NEXT:    [[TMP6:%.*]] = fmul <2 x double> [[TMP5]], [[WIDE_LOAD]]
; VPLAN-NEXT:    store <2 x double> [[TMP6]], ptr [[ARRAYIDX7]], align 8
; VPLAN-NEXT:    br label [[RETURN]]
; VPLAN:       return:
...
; LEGACY-LABEL: @cse_for_hoisted_instructions_in_preheader(
; LEGACY-NEXT:  entry:
; LEGACY-NEXT:    [[TMP0:%.*]] = insertelement <2 x i32> poison, i32 [[A:%.*]], i32 0
; LEGACY-NEXT:    [[TMP1:%.*]] = shufflevector <2 x i32> [[TMP0]], <2 x i32> poison, <2 x i32> zeroinitializer
; LEGACY-NEXT:    br label [[LOOP:%.*]]
; LEGACY:       loop:
; LEGACY-NEXT:    [[TMP2:%.*]] = or <2 x i32> splat (i32 22), [[TMP1]]
; LEGACY-NEXT:    [[TMP3:%.*]] = or <2 x i32> [[TMP2]], splat (i32 3)
; LEGACY-NEXT:    store <2 x i32> [[TMP3]], ptr [[DST:%.*]], align 4
; LEGACY-NEXT:    [[OR_2:%.*]] = or i32 [[A]], 3
; LEGACY-NEXT:    [[GEP_2:%.*]] = getelementptr inbounds i32, ptr [[DST]], i64 10
; LEGACY-NEXT:    store i32 [[OR_2]], ptr [[GEP_2]], align 4
; LEGACY-NEXT:    [[OR_3:%.*]] = or i32 [[A]], 3
; LEGACY-NEXT:    [[GEP_3:%.*]] = getelementptr inbounds i32, ptr [[DST]], i64 11
; LEGACY-NEXT:    store i32 [[OR_3]], ptr [[GEP_3]], align 4
; LEGACY-NEXT:    br i1 [[C:%.*]], label [[LOOP]], label [[EXIT:%.*]]
; LEGACY:       exit:
; LEGACY-NEXT:    ret void
;
; VPLAN-LABEL: @cse_for_hoisted_instructions_in_preheader(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    br label [[LOOP:%.*]]
; VPLAN:       loop:
; VPLAN-NEXT:    [[BROADCAST_SPLATINSERT:%.*]] = insertelement <2 x i32> poison, i32 [[A:%.*]], i32 0
; VPLAN-NEXT:    [[BROADCAST_SPLAT:%.*]] = shufflevector <2 x i32> [[BROADCAST_SPLATINSERT]], <2 x i32> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP0:%.*]] = or <2 x i32> splat (i32 22), [[BROADCAST_SPLAT]]
; VPLAN-NEXT:    [[TMP1:%.*]] = or <2 x i32> [[TMP0]], splat (i32 3)
; VPLAN-NEXT:    store <2 x i32> [[TMP1]], ptr [[DST:%.*]], align 4
; VPLAN-NEXT:    [[OR_2:%.*]] = or i32 [[A]], 3
; VPLAN-NEXT:    [[GEP_2:%.*]] = getelementptr inbounds i32, ptr [[DST]], i64 10
; VPLAN-NEXT:    store i32 [[OR_2]], ptr [[GEP_2]], align 4
; VPLAN-NEXT:    [[OR_3:%.*]] = or i32 [[A]], 3
; VPLAN-NEXT:    [[GEP_3:%.*]] = getelementptr inbounds i32, ptr [[DST]], i64 11
; VPLAN-NEXT:    store i32 [[OR_3]], ptr [[GEP_3]], align 4
; VPLAN-NEXT:    br i1 [[C:%.*]], label [[LOOP]], label [[EXIT:%.*]]
; VPLAN:       exit:
; VPLAN-NEXT:    ret void
;
entry:
