...
; RUN: opt -S -passes=slp-vectorizer -slp-use-vplan-codegen -slp-threshold=-100 -mtriple=x86_64-w64-windows-gnu < %s | FileCheck %s --check-prefix=VPLAN
...
;
; VPLAN-LABEL: @test(
; VPLAN-NEXT:  for.body92.preheader:
; VPLAN-NEXT:    br label [[FOR_BODY92:%.*]]
; VPLAN:       for.body92:
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <2 x i16> <i16 0, i16 poison>, i16 [[TMP0:%.*]], i32 1
; VPLAN-NEXT:    [[TMP2:%.*]] = sext <2 x i16> [[TMP1]] to <2 x i32>
; VPLAN-NEXT:    [[TMP6:%.*]] = zext <2 x i16> [[TMP1]] to <2 x i32>
; VPLAN-NEXT:    [[TMP7:%.*]] = shufflevector <2 x i32> [[TMP2]], <2 x i32> [[TMP6]], <2 x i32> <i32 0, i32 3>
; VPLAN-NEXT:    [[TMP3:%.*]] = shufflevector <2 x i32> [[TMP7]], <2 x i32> poison, <4 x i32> <i32 0, i32 poison, i32 1, i32 poison>
; VPLAN-NEXT:    [[TMP4:%.*]] = shufflevector <4 x i32> <i32 poison, i32 0, i32 poison, i32 0>, <4 x i32> [[TMP3]], <4 x i32> <i32 4, i32 1, i32 6, i32 3>
; VPLAN-NEXT:    [[TMP5:%.*]] = add nsw <4 x i32> zeroinitializer, [[TMP4]]
; VPLAN-NEXT:    store <4 x i32> [[TMP5]], ptr undef, align 8
; VPLAN-NEXT:    br label [[FOR_BODY92]]
;
