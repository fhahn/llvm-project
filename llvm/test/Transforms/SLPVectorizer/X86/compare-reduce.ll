...
; RUN: opt < %s -passes=slp-vectorizer,dce -S -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=CHECK,LEGACY %s
; RUN: opt -slp-use-vplan-codegen < %s -passes=slp-vectorizer,dce -S -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx | FileCheck --check-prefixes=CHECK,VPLAN %s
...
; LEGACY-LABEL: @reduce_compare(
; LEGACY-NEXT:  entry:
; LEGACY-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; LEGACY-NEXT:    [[TMP0:%.*]] = insertelement <2 x double> poison, double [[CONV]], i32 0
; LEGACY-NEXT:    [[TMP1:%.*]] = shufflevector <2 x double> [[TMP0]], <2 x double> poison, <2 x i32> zeroinitializer
; LEGACY-NEXT:    br label [[FOR_BODY:%.*]]
; LEGACY:       for.body:
; LEGACY-NEXT:    [[INDVARS_IV:%.*]] = phi i64 [ 0, [[ENTRY:%.*]] ], [ [[INDVARS_IV_NEXT:%.*]], [[FOR_INC:%.*]] ]
; LEGACY-NEXT:    [[TMP2:%.*]] = shl nsw i64 [[INDVARS_IV]], 1
; LEGACY-NEXT:    [[ARRAYIDX:%.*]] = getelementptr inbounds double, ptr [[A:%.*]], i64 [[TMP2]]
; LEGACY-NEXT:    [[TMP3:%.*]] = load <2 x double>, ptr [[ARRAYIDX]], align 8
; LEGACY-NEXT:    [[TMP4:%.*]] = fmul <2 x double> [[TMP1]], [[TMP3]]
; LEGACY-NEXT:    [[TMP5:%.*]] = fmul <2 x double> [[TMP4]], <double 7.000000e+00, double 4.000000e+00>
; LEGACY-NEXT:    [[TMP6:%.*]] = fadd <2 x double> [[TMP5]], <double 5.000000e+00, double 9.000000e+00>
; LEGACY-NEXT:    [[TMP7:%.*]] = extractelement <2 x double> [[TMP6]], i32 0
; LEGACY-NEXT:    [[TMP8:%.*]] = extractelement <2 x double> [[TMP6]], i32 1
; LEGACY-NEXT:    [[CMP11:%.*]] = fcmp ogt double [[TMP7]], [[TMP8]]
; LEGACY-NEXT:    br i1 [[CMP11]], label [[IF_THEN:%.*]], label [[FOR_INC]]
; LEGACY:       if.then:
; LEGACY-NEXT:    [[CALL:%.*]] = tail call i32 (ptr, ...) @printf(ptr @.str)
; LEGACY-NEXT:    br label [[FOR_INC]]
; LEGACY:       for.inc:
; LEGACY-NEXT:    [[INDVARS_IV_NEXT]] = add i64 [[INDVARS_IV]], 1
; LEGACY-NEXT:    [[LFTR_WIDEIV:%.*]] = trunc i64 [[INDVARS_IV_NEXT]] to i32
; LEGACY-NEXT:    [[EXITCOND:%.*]] = icmp eq i32 [[LFTR_WIDEIV]], 100
; LEGACY-NEXT:    br i1 [[EXITCOND]], label [[FOR_END:%.*]], label [[FOR_BODY]]
; LEGACY:       for.end:
; LEGACY-NEXT:    ret void
;
; VPLAN-LABEL: @reduce_compare(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[CONV:%.*]] = sitofp i32 [[N:%.*]] to double
; VPLAN-NEXT:    br label [[FOR_BODY:%.*]]
; VPLAN:       for.body:
; VPLAN-NEXT:    [[INDVARS_IV:%.*]] = phi i64 [ 0, [[ENTRY:%.*]] ], [ [[INDVARS_IV_NEXT:%.*]], [[FOR_INC:%.*]] ]
; VPLAN-NEXT:    [[TMP0:%.*]] = shl nsw i64 [[INDVARS_IV]], 1
; VPLAN-NEXT:    [[ARRAYIDX:%.*]] = getelementptr inbounds double, ptr [[A:%.*]], i64 [[TMP0]]
; VPLAN-NEXT:    [[WIDE_LOAD:%.*]] = load <2 x double>, ptr [[ARRAYIDX]], align 8
; VPLAN-NEXT:    [[BROADCAST_SPLATINSERT:%.*]] = insertelement <2 x double> poison, double [[CONV]], i32 0
...
; VPLAN-NEXT:    [[TMP2:%.*]] = fmul <2 x double> [[TMP1]], <double 7.000000e+00, double 4.000000e+00>
; VPLAN-NEXT:    [[TMP3:%.*]] = fadd <2 x double> [[TMP2]], <double 5.000000e+00, double 9.000000e+00>
; VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP3]], i32 0
; VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <2 x double> [[TMP3]], i32 1
; VPLAN-NEXT:    [[CMP11:%.*]] = fcmp ogt double [[TMP4]], [[TMP5]]
; VPLAN-NEXT:    br i1 [[CMP11]], label [[IF_THEN:%.*]], label [[FOR_INC]]
; VPLAN:       if.then:
; VPLAN-NEXT:    [[CALL:%.*]] = tail call i32 (ptr, ...) @printf(ptr @.str)
; VPLAN-NEXT:    br label [[FOR_INC]]
; VPLAN:       for.inc:
; VPLAN-NEXT:    [[INDVARS_IV_NEXT]] = add i64 [[INDVARS_IV]], 1
; VPLAN-NEXT:    [[LFTR_WIDEIV:%.*]] = trunc i64 [[INDVARS_IV_NEXT]] to i32
; VPLAN-NEXT:    [[EXITCOND:%.*]] = icmp eq i32 [[LFTR_WIDEIV]], 100
