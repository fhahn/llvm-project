...
; RUN: opt -slp-use-vplan-codegen -passes=slp-vectorizer -slp-vectorize-hor -slp-vectorize-hor-store -S < %s -mtriple=x86_64-unknown-linux-gnu | FileCheck %s --check-prefix=VPLAN
...
; VPLAN-LABEL: @test(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[CMP:%.*]] = icmp eq ptr [[PTR:%.*]], null
; VPLAN-NEXT:    br i1 [[CMP]], label [[LOOP:%.*]], label [[BAIL_OUT:%.*]]
; VPLAN:       loop:
; VPLAN-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x i32>, ptr [[PTR]], align 4
; VPLAN-NEXT:    [[TMP0:%.*]] = extractelement <4 x i32> [[WIDE_LOAD]], i32 0
; VPLAN-NEXT:    store <4 x i32> [[WIDE_LOAD]], ptr [[S:%.*]], align 4
; VPLAN-NEXT:    br label [[LOOP1:%.*]]
; VPLAN:       loop1:
; VPLAN-NEXT:    store i32 [[TMP0]], ptr [[S]], align 4
; VPLAN-NEXT:    br i1 true, label [[LOOP1]], label [[CONT:%.*]]
; VPLAN:       cont:
; VPLAN-NEXT:    [[TMP1:%.*]] = extractelement <4 x i32> [[WIDE_LOAD]], i32 0
; VPLAN-NEXT:    br i1 true, label [[LOOP]], label [[BAIL_OUT]]
; VPLAN:       bail_out:
; VPLAN-NEXT:    [[DUMMY_PHI:%.*]] = phi i32 [ 1, [[ENTRY:%.*]] ], [ [[TMP1]], [[CONT]] ]
; VPLAN-NEXT:    store i32 [[DUMMY_PHI]], ptr [[S]], align 4
; VPLAN-NEXT:    ret void
;
entry:
