...
; VPLAN-NEXT:  [[ENTRY:.*]]:
; VPLAN-NEXT:    br label %[[DO_BODY:.*]]
; VPLAN:       [[DO_BODY]]:
; VPLAN-NEXT:    [[TMP0:%.*]] = phi <12 x float> [ zeroinitializer, %[[ENTRY]] ], [ [[TMP7:%.*]], %[[DO_BODY]] ]
; VPLAN-NEXT:    [[VAL_SROA_6_1:%.*]] = select i1 true, float 0.000000e+00, float 0.000000e+00
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <2 x float> <float 0.000000e+00, float poison>, float [[VAL_SROA_6_1]], i32 1
; VPLAN-NEXT:    [[TMP2:%.*]] = fcmp uge <2 x float> [[TMP1]], zeroinitializer
; VPLAN-NEXT:    [[TMP3:%.*]] = insertelement <12 x i1> <i1 true, i1 true, i1 true, i1 true, i1 true, i1 true, i1 true, i1 true, i1 undef, i1 undef, i1 undef, i1 undef>, i1 true, i32 8
; VPLAN-NEXT:    [[TMP4:%.*]] = shufflevector <2 x i1> [[TMP2]], <2 x i1> poison, <12 x i32> <i32 0, i32 1, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison, i32 poison>
; VPLAN-NEXT:    [[TMP5:%.*]] = shufflevector <12 x i1> [[TMP3]], <12 x i1> [[TMP4]], <12 x i32> <i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7, i32 8, i32 12, i32 13, i32 11>
; VPLAN-NEXT:    [[TMP6:%.*]] = insertelement <12 x i1> [[TMP5]], i1 true, i32 11
; VPLAN-NEXT:    [[TMP7]] = select <12 x i1> [[TMP6]], <12 x float> zeroinitializer, <12 x float> zeroinitializer
; VPLAN-NEXT:    br label %[[DO_BODY]]
;
entry:
  br label %do.body
