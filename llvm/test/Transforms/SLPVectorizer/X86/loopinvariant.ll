...
; RUN: opt -slp-use-vplan-codegen < %s -passes=slp-vectorizer -S -mcpu=corei7-avx | FileCheck --check-prefix=VPLAN %s
...
; VPLAN-LABEL: @foo(
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[CMP62:%.*]] = icmp sgt i32 [[N:%.*]], 0
; VPLAN-NEXT:    br i1 [[CMP62]], label [[FOR_BODY:%.*]], label [[FOR_END:%.*]]
; VPLAN:       for.body:
; VPLAN-NEXT:    [[INDVARS_IV:%.*]] = phi i64 [ [[INDVARS_IV_NEXT:%.*]], [[FOR_BODY]] ], [ 0, [[ENTRY:%.*]] ]
; VPLAN-NEXT:    [[ARRAYIDX:%.*]] = getelementptr inbounds i32, ptr [[A:%.*]], i64 [[INDVARS_IV]]
; VPLAN-NEXT:    [[WIDE_LOAD:%.*]] = load <8 x i32>, ptr [[ARRAYIDX]], align 4
; VPLAN-NEXT:    [[BROADCAST_SPLATINSERT:%.*]] = insertelement <8 x i32> poison, i32 [[N]], i32 0
; VPLAN-NEXT:    [[BROADCAST_SPLAT:%.*]] = shufflevector <8 x i32> [[BROADCAST_SPLATINSERT]], <8 x i32> poison, <8 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP0:%.*]] = add nsw <8 x i32> [[WIDE_LOAD]], [[BROADCAST_SPLAT]]
; VPLAN-NEXT:    store <8 x i32> [[TMP0]], ptr [[ARRAYIDX]], align 4
; VPLAN-NEXT:    [[INDVARS_IV_NEXT]] = add i64 [[INDVARS_IV]], 8
; VPLAN-NEXT:    [[TMP1:%.*]] = trunc i64 [[INDVARS_IV_NEXT]] to i32
; VPLAN-NEXT:    [[CMP:%.*]] = icmp slt i32 [[TMP1]], [[N]]
; VPLAN-NEXT:    br i1 [[CMP]], label [[FOR_BODY]], label [[FOR_END]]
; VPLAN:       for.end:
