...
; RUN: opt -slp-use-vplan-codegen -S -passes=slp-vectorizer -mtriple=x86_64-apple-macosx -mcpu=haswell < %s | FileCheck --check-prefix=VPLAN %s
...
;
; VPLAN-LABEL: @test(
; VPLAN-NEXT:    [[P0:%.*]] = getelementptr inbounds i64, ptr [[P:%.*]], i64 0
; VPLAN-NEXT:    [[Q0:%.*]] = getelementptr inbounds i64, ptr [[Q:%.*]], i64 0
; VPLAN-NEXT:    [[TMP1:%.*]] = load <4 x i64>, ptr [[P0]], align 2
; VPLAN-NEXT:    [[TMP2:%.*]] = load <4 x i64>, ptr [[Q0]], align 2
; VPLAN-NEXT:    [[TMP3:%.*]] = sub nsw <4 x i64> [[TMP1]], [[TMP2]]
; VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <4 x i64> [[TMP3]], i32 0
; VPLAN-NEXT:    [[G0:%.*]] = getelementptr inbounds i32, ptr [[R:%.*]], i64 [[TMP4]]
; VPLAN-NEXT:    [[TMP5:%.*]] = extractelement <4 x i64> [[TMP3]], i32 1
; VPLAN-NEXT:    [[G1:%.*]] = getelementptr inbounds i32, ptr [[R]], i64 [[TMP5]]
; VPLAN-NEXT:    [[TMP6:%.*]] = extractelement <4 x i64> [[TMP3]], i32 2
; VPLAN-NEXT:    [[G2:%.*]] = getelementptr inbounds i32, ptr [[R]], i64 [[TMP6]]
; VPLAN-NEXT:    [[TMP7:%.*]] = extractelement <4 x i64> [[TMP3]], i32 3
; VPLAN-NEXT:    [[G3:%.*]] = getelementptr inbounds i32, ptr [[R]], i64 [[TMP7]]
; VPLAN-NEXT:    ret void
;
...
;
; VPLAN-LABEL: @test2(
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <2 x ptr> poison, ptr [[A:%.*]], i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = insertelement <2 x ptr> [[TMP1]], ptr [[B:%.*]], i32 1
; VPLAN-NEXT:    [[TMP3:%.*]] = getelementptr i64, <2 x ptr> [[TMP2]], <2 x i64> <i64 1, i64 3>
; VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x ptr> [[TMP3]], i32 0
; VPLAN-NEXT:    [[TMP5:%.*]] = ptrtoint <2 x ptr> [[TMP3]] to <2 x i64>
; VPLAN-NEXT:    [[TMP6:%.*]] = load <2 x i64>, ptr [[TMP4]], align 8
; VPLAN-NEXT:    [[TMP7:%.*]] = add <2 x i64> [[TMP5]], [[TMP6]]
; VPLAN-NEXT:    store <2 x i64> [[TMP7]], ptr [[TMP4]], align 8
; VPLAN-NEXT:    ret void
;
