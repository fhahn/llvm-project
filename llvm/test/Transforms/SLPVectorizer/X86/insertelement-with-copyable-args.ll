...
; RUN: opt -slp-use-vplan-codegen -S --passes=slp-vectorizer -mtriple=x86_64-unknown-linux-gnu -slp-threshold=-99999 < %s | FileCheck --check-prefix=VPLAN %s
...
; VPLAN-SAME: i32 [[ARG:%.*]]) {
; VPLAN-NEXT:  [[BB:.*:]]
; VPLAN-NEXT:    [[FREEZE:%.*]] = freeze i32 0
; VPLAN-NEXT:    br i1 false, label %[[BB1:.*]], label %[[BB1]]
; VPLAN:       [[BB1]]:
; VPLAN-NEXT:    [[LOAD:%.*]] = load i32, ptr addrspace(1) null, align 4
; VPLAN-NEXT:    [[TMP0:%.*]] = insertelement <4 x i32> <i32 0, i32 0, i32 0, i32 poison>, i32 [[ARG]], i32 3
; VPLAN-NEXT:    [[TMP1:%.*]] = insertelement <2 x i32> poison, i32 [[FREEZE]], i32 0
; VPLAN-NEXT:    [[TMP2:%.*]] = shufflevector <2 x i32> [[TMP1]], <2 x i32> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP3:%.*]] = add <2 x i32> [[TMP2]], zeroinitializer
; VPLAN-NEXT:    [[TMP4:%.*]] = extractelement <2 x i32> [[TMP3]], i32 0
; VPLAN-NEXT:    [[TMP15:%.*]] = insertelement <2 x i32> poison, i32 [[FREEZE]], i32 0
; VPLAN-NEXT:    [[TMP16:%.*]] = shufflevector <2 x i32> [[TMP15]], <2 x i32> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP5:%.*]] = add <2 x i32> [[TMP16]], zeroinitializer
; VPLAN-NEXT:    [[TMP6:%.*]] = insertelement <2 x i32> poison, i32 [[ARG]], i32 0
; VPLAN-NEXT:    [[TMP7:%.*]] = insertelement <2 x i32> [[TMP6]], i32 [[LOAD]], i32 1
; VPLAN-NEXT:    [[TMP8:%.*]] = icmp ult <2 x i32> [[TMP5]], [[TMP7]]
; VPLAN-NEXT:    [[TMP9:%.*]] = extractelement <2 x i1> [[TMP8]], i32 0
; VPLAN-NEXT:    [[AND:%.*]] = and i1 [[TMP9]], false
; VPLAN-NEXT:    [[TMP10:%.*]] = shufflevector <2 x i32> [[TMP3]], <2 x i32> poison, <4 x i32> <i32 0, i32 1, i32 poison, i32 poison>
; VPLAN-NEXT:    [[TMP11:%.*]] = shufflevector <4 x i32> <i32 0, i32 poison, i32 poison, i32 0>, <4 x i32> [[TMP10]], <4 x i32> <i32 0, i32 4, i32 5, i32 3>
; VPLAN-NEXT:    [[TMP12:%.*]] = icmp eq <4 x i32> [[TMP11]], [[TMP0]]
; VPLAN-NEXT:    [[TMP13:%.*]] = icmp ult <4 x i32> [[TMP11]], [[TMP0]]
; VPLAN-NEXT:    [[TMP14:%.*]] = shufflevector <4 x i1> [[TMP12]], <4 x i1> [[TMP13]], <4 x i32> <i32 0, i32 5, i32 6, i32 7>
; VPLAN-NEXT:    br i1 false, label %[[BB11:.*]], label %[[BB12:.*]]
; VPLAN:       [[BB11]]:
; VPLAN-NEXT:    [[ZEXT:%.*]] = zext i32 [[TMP4]] to i64
; VPLAN-NEXT:    ret i64 0
; VPLAN:       [[BB12]]:
; VPLAN-NEXT:    [[ZEXT13:%.*]] = zext i32 [[TMP4]] to i64
; VPLAN-NEXT:    ret i64 0
;
bb:
  %freeze = freeze i32 0
