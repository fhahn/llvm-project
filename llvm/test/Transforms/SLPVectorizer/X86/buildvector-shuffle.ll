...
; RUN: opt -S -passes=slp-vectorizer -mtriple=x86_64-unknown-linux-gnu < %s | FileCheck --check-prefixes=CHECK,LEGACY %s
; RUN: opt -slp-use-vplan-codegen -S -passes=slp-vectorizer -mtriple=x86_64-unknown-linux-gnu < %s | FileCheck --check-prefixes=CHECK,VPLAN %s
...
; LEGACY-LABEL: @test(
; LEGACY-NEXT:  entry:
; LEGACY-NEXT:    [[TMP0:%.*]] = insertelement <2 x float> poison, float [[A:%.*]], i32 0
; LEGACY-NEXT:    [[TMP1:%.*]] = shufflevector <2 x float> [[TMP0]], <2 x float> poison, <2 x i32> zeroinitializer
; LEGACY-NEXT:    br label [[LOOP:%.*]]
; LEGACY:       loop:
; LEGACY-NEXT:    [[TMP2:%.*]] = fadd <2 x float> zeroinitializer, [[TMP1]]
; LEGACY-NEXT:    [[TMP3:%.*]] = shufflevector <2 x float> [[TMP2]], <2 x float> poison, <2 x i32> zeroinitializer
; LEGACY-NEXT:    br label [[LOOP]]
;
; VPLAN-LABEL: @test(
; VPLAN-NEXT:  entry:
...
; VPLAN:       loop:
; VPLAN-NEXT:    [[TMP0:%.*]] = insertelement <2 x float> poison, float [[A:%.*]], i32 0
; VPLAN-NEXT:    [[TMP1:%.*]] = shufflevector <2 x float> [[TMP0]], <2 x float> poison, <2 x i32> zeroinitializer
; VPLAN-NEXT:    [[TMP2:%.*]] = fadd <2 x float> zeroinitializer, [[TMP1]]
; VPLAN-NEXT:    [[TMP3:%.*]] = extractelement <2 x float> [[TMP2]], i32 0
; VPLAN-NEXT:    [[AGG:%.*]] = insertelement <2 x float> [[TMP2]], float [[TMP3]], i64 1
; VPLAN-NEXT:    br label [[LOOP]]
;
entry:
  br label %loop
