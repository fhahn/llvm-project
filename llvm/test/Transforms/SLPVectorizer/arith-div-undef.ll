...
; RUN: %if x86-registered-target %{ opt < %s -mtriple=x86_64-unknown -passes=slp-vectorizer,instcombine -slp-use-vplan-codegen -S -slp-threshold=-10000 | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt < %s -mtriple=aarch64-unknown-linux-gnu -passes=slp-vectorizer,instcombine -slp-use-vplan-codegen -S -slp-threshold=-10000 | FileCheck %s %}
