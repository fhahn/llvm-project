...
; RUN: %if x86-registered-target %{ opt < %s -S -mtriple=x86_64-unknown -passes=slp-vectorizer -slp-use-vplan-codegen -slp-min-reg-size=64 -slp-threshold=-1000 | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt < %s -S -mtriple=aarch64-unknown -passes=slp-vectorizer -slp-use-vplan-codegen -slp-min-reg-size=64 -slp-threshold=-1000 | FileCheck %s %}
