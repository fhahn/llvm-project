...
; RUN: %if x86-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=x86_64-pc-linux-gnu < %s | FileCheck --check-prefix=VPLAN %s %}
; RUN: %if aarch64-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64-unknown-linux-gnu < %s | FileCheck --check-prefix=VPLAN %s %}
...
; VPLAN-SAME: double [[ADD:%.*]]) {
; VPLAN-NEXT:  entry:
; VPLAN-NEXT:    [[TMP0:%.*]] = insertelement <2 x double> <double poison, double 0.000000e+00>, double [[ADD]], i32 0
; VPLAN-NEXT:    [[TMP1:%.*]] = fmul <2 x double> [[TMP0]], zeroinitializer
; VPLAN-NEXT:    [[TMP2:%.*]] = insertelement <2 x double> <double poison, double 0.000000e+00>, double [[ADD]], i32 0
; VPLAN-NEXT:    [[TMP3:%.*]] = fmul <2 x double> [[TMP2]], zeroinitializer
; VPLAN-NEXT:    br label [[COND_TRUE45:%.*]]
; VPLAN:       cond.true45:
; VPLAN-NEXT:    [[TMP4:%.*]] = fmul <2 x double> [[TMP3]], zeroinitializer
; VPLAN-NEXT:    [[TMP5:%.*]] = fmul <2 x double> [[TMP1]], zeroinitializer
; VPLAN-NEXT:    [[TMP6:%.*]] = fadd <2 x double> zeroinitializer, [[TMP4]]
; VPLAN-NEXT:    [[TMP7:%.*]] = fsub <2 x double> [[TMP6]], zeroinitializer
; VPLAN-NEXT:    [[TMP8:%.*]] = fsub <2 x double> [[TMP7]], [[TMP5]]
; VPLAN-NEXT:    ret void
;
entry:
