...
; RUN: %if x86-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=x86_64-linux-gnu < %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -S --passes=slp-vectorizer -slp-use-vplan-codegen -mtriple=aarch64-linux-gnu < %s | FileCheck %s %}
