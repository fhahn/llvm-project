...
; RUN: %if x86-registered-target %{ opt -S -passes=slp-vectorizer -slp-use-vplan-codegen -slp-threshold=0 -mtriple=x86_64-unknown-linux-gnu < %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -S -passes=slp-vectorizer -slp-use-vplan-codegen -slp-threshold=0 -mtriple=aarch64-unknown-linux-gnu < %s | FileCheck %s %}
