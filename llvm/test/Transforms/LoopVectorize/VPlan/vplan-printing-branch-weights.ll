...
; When the original loop is profiled, the branch_weights estimated for a
; predicated block from its reciprocal execution probability are recorded on
; the VPBasicBlock (printed as !prof on the block header) already during initial
; VPlan construction, so the block's execution probability is available to the
; VPlan-based cost model. The predicate here is taken 1 in 4 iterations
; (reciprocal 4), so the recorded weights are {1, 3}.
define void @predicated_store(ptr %a, i32 %n) {
; CHECK-LABEL: VPlan for loop in 'predicated_store'
