...
; The address of the accesses below is a unit-stride consecutive address, but
; VPlan's SCEV construction cannot yet compute a SCEV for it. Hence the
; accesses are left for the legacy cost-model to widen.

; `or disjoint X, C` is equivalent to `add X, C`.
...
; A bitcast between identical types is a no-op.
define void @noop_bitcast_address(ptr noalias %A) {
; CHECK-LABEL: VPlan for loop in 'noop_bitcast_address'
; CHECK:  VPlan ' for UF>=1' {
; CHECK-NEXT:  Live-in vp<[[VP0:%[0-9]+]]> = VF
; CHECK-NEXT:  Live-in vp<[[VP1:%[0-9]+]]> = VF * UF
; CHECK-NEXT:  Live-in vp<[[VP2:%[0-9]+]]> = vector-trip-count
...
; CHECK-NEXT:      EMIT-SCALAR ir<%bc> = bitcast ir<%gep> to ptr
; CHECK-NEXT:      vp<[[VP4:%[0-9]+]]> = vector-pointer inbounds i32, ir<%bc>, ir<1>
; CHECK-NEXT:      WIDEN ir<%lv> = load vp<[[VP4]]>
; CHECK-NEXT:      EMIT ir<%add> = add ir<%lv>, ir<1>
; CHECK-NEXT:      vp<[[VP5:%[0-9]+]]> = vector-pointer inbounds i32, ir<%bc>, ir<1>
; CHECK-NEXT:      WIDEN store vp<[[VP5]]>, ir<%add>
; CHECK-NEXT:      EMIT ir<%iv.next> = add nuw nsw ir<%iv>, ir<1>
; CHECK-NEXT:      EMIT ir<%ec> = icmp eq ir<%iv>, ir<100>
; CHECK-NEXT:      EMIT vp<%index.next> = add nuw vp<[[VP3]]>, vp<[[VP1]]>
...
; An `sdiv` with non-negative operands is equivalent to an `udiv`.
...
; Extracting the result of a with-overflow intrinsic that is known not to
; overflow is equivalent to the plain binary operation.
...
; All incoming values of the blend feeding the address compute the same value.
...
; The addresses of the loads from %B below are not unit-stride consecutive
; addresses, so the loads must not be widened.
