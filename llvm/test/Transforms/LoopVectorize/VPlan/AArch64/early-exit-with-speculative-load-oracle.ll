...
; CHECK-NEXT:      EMIT vp<[[VP8:%[0-9]+]]> = any-of ir<%cmp>
...
; CHECK-NEXT:  Live-in vp<[[VP1]]> = vector-trip-count
; CHECK-EMPTY:
...
; CHECK-NEXT:    EMIT vp<%adjusted.iv> = add vp<%index>, vp<[[VP1]]>
; CHECK-NEXT:    CLONE ir<%gep.A> = getelementptr inbounds ir<%A>, vp<%adjusted.iv>
...
; CHECK-NEXT:    CLONE ir<%gep.A> = getelementptr inbounds ir<%A>, vp<%adjusted.iv>
; CHECK-NEXT:    CLONE ir<%ld.A> = load ir<%gep.A>
; CHECK-NEXT:    EMIT ir<%cmp> = icmp eq ir<%ld.A>, ir<%val>
; CHECK-NEXT:    EMIT branch-on-cond ir<%cmp> (!vplan.prof.estimated estimated {67108864, 2080374784})
; CHECK-NEXT:  Successor(s): oracle.exit, loop.latch
; CHECK-EMPTY:
; CHECK-NEXT:  loop.latch:
...
; CHECK-NEXT:      EMIT vp<[[VP10:%[0-9]+]]> = any-of ir<%cmp>
; CHECK-NEXT:      EMIT vp<%index.next> = add nuw vp<[[VP3]]>, vp<[[VP1]]>
...
; CHECK-NEXT:    EMIT-SCALAR vp<[[VP15:%[0-9]+]]> = trunc vp<[[VP14]]> to i32
; CHECK-NEXT:    EMIT vp<[[VP16:%[0-9]+]]> = add vp<[[VP3]]>, vp<[[VP15]]>
; CHECK-NEXT:  Successor(s): ir-bb<early.exit>
; CHECK-EMPTY:
; CHECK-NEXT:  ir-bb<early.exit>:
; CHECK-NEXT:    IR   %iv.lcssa = phi i32 [ %iv, %loop.header ] (extra operand: vp<[[VP16]]> from vector.early.exit)
; CHECK-NEXT:  No successors
; CHECK-EMPTY:
; CHECK-NEXT:  scalar.ph:
; CHECK-NEXT:    EMIT-SCALAR vp<%bc.resume.val> = phi [ vp<[[VP2]]>, middle.block ], [ ir<0>, ir-bb<entry> ]
...
; CHECK-NEXT:    CLONE ir<%gep.A> = getelementptr inbounds ir<%A>, vp<%adjusted.iv>
; CHECK-NEXT:    CLONE ir<%ld.A> = load ir<%gep.A>
; CHECK-NEXT:    CLONE ir<%gep.B> = getelementptr inbounds ir<%B>, vp<%adjusted.iv>
...
;
; CHECK-LABEL: VPlan for loop in 'binop_chain'
; CHECK:  VPlan 'Initial VPlan for VF={4},UF>=1' {
; CHECK-NEXT:  Live-in vp<[[VP0:%[0-9]+]]> = VF
; CHECK-NEXT:  Live-in vp<[[VP1:%[0-9]+]]> = VF * UF
; CHECK-NEXT:  Live-in vp<[[VP2:%[0-9]+]]> = vector-trip-count
...
; CHECK-NEXT:    vector.body:
; CHECK-NEXT:      vp<[[VP4:%[0-9]+]]> = SCALAR-STEPS vp<[[VP3]]>, ir<1>, vp<[[VP0]]>
; CHECK-NEXT:      SPECULATIVE-LOAD-ORACLE vp<[[VP5:%[0-9]+]]> = fn(vp<[[VP3]]>, ir<%A>, ir<%B>, ir<%C>)
; CHECK-NEXT:      CLONE ir<%gep.A> = getelementptr inbounds ir<%A>, vp<[[VP4]]>
; CHECK-NEXT:      WIDEN-INTRINSIC vp<[[VP6:%[0-9]+]]> = call llvm.speculative.load(ir<%gep.A>, ir<false>, vp<[[VP5]]>, vp<[[VP3]]>, ir<%A>, ir<%B>, ir<%C>)
; CHECK-NEXT:      EMIT vp<[[VP7:%[0-9]+]]> = freeze vp<[[VP6]]>
; CHECK-NEXT:      CLONE ir<%gep.B> = getelementptr inbounds ir<%B>, vp<[[VP4]]>
; CHECK-NEXT:      WIDEN-INTRINSIC vp<[[VP8:%[0-9]+]]> = call llvm.speculative.load(ir<%gep.B>, ir<false>, vp<[[VP5]]>, vp<[[VP3]]>, ir<%A>, ir<%B>, ir<%C>)
; CHECK-NEXT:      EMIT vp<[[VP9:%[0-9]+]]> = freeze vp<[[VP8]]>
; CHECK-NEXT:      CLONE ir<%gep.C> = getelementptr inbounds ir<%C>, vp<[[VP4]]>
; CHECK-NEXT:      WIDEN-INTRINSIC vp<[[VP10:%[0-9]+]]> = call llvm.speculative.load(ir<%gep.C>, ir<false>, vp<[[VP5]]>, vp<[[VP3]]>, ir<%A>, ir<%B>, ir<%C>)
; CHECK-NEXT:      EMIT vp<[[VP11:%[0-9]+]]> = freeze vp<[[VP10]]>
; CHECK-NEXT:      WIDEN ir<%add> = add vp<[[VP7]]>, vp<[[VP9]]>
; CHECK-NEXT:      WIDEN ir<%sub> = sub ir<%add>, vp<[[VP11]]>
; CHECK-NEXT:      WIDEN ir<%mul> = mul ir<%sub>, ir<3>
; CHECK-NEXT:      WIDEN ir<%xor> = xor ir<%mul>, ir<7>
; CHECK-NEXT:      WIDEN ir<%div> = udiv ir<%xor>, ir<3>
; CHECK-NEXT:      WIDEN ir<%fr> = freeze ir<%div>
; CHECK-NEXT:      WIDEN ir<%cmp> = icmp eq ir<%fr>, ir<0>
; CHECK-NEXT:      EMIT vp<[[VP12:%[0-9]+]]> = any-of ir<%cmp>
; CHECK-NEXT:      EMIT vp<%index.next> = add nuw vp<[[VP3]]>, vp<[[VP1]]>
...
; CHECK-EMPTY:
; CHECK-NEXT:  vector.early.exit:
; CHECK-NEXT:    EMIT vp<[[VP16:%[0-9]+]]> = first-active-lane ir<%cmp>
...
; CHECK-NEXT:    IR   %gep.A = getelementptr inbounds i8, ptr %A, i64 %iv
; CHECK-NEXT:    IR   %ld.A = load i8, ptr %gep.A, align 1
; CHECK-NEXT:    IR   %gep.B = getelementptr inbounds i8, ptr %B, i64 %iv
; CHECK-NEXT:    IR   %ld.B = load i8, ptr %gep.B, align 1
; CHECK-NEXT:    IR   %gep.C = getelementptr inbounds i8, ptr %C, i64 %iv
; CHECK-NEXT:    IR   %ld.C = load i8, ptr %gep.C, align 1
; CHECK-NEXT:    IR   %add = add i8 %ld.A, %ld.B
; CHECK-NEXT:    IR   %sub = sub i8 %add, %ld.C
; CHECK-NEXT:    IR   %mul = mul i8 %sub, 3
; CHECK-NEXT:    IR   %xor = xor i8 %mul, 7
; CHECK-NEXT:    IR   %div = udiv i8 %xor, 3
; CHECK-NEXT:    IR   %fr = freeze i8 %div
; CHECK-NEXT:    IR   %cmp = icmp eq i8 %fr, 0
; CHECK-NEXT:  No successors
; CHECK-NEXT:  }
; CHECK-EMPTY:
; CHECK:  VPlan for speculative-load oracle vp<[[VP5]]> {
...
; CHECK-NEXT:    CLONE ir<%gep.B> = getelementptr inbounds ir<%B>, vp<%adjusted.iv>
; CHECK-NEXT:    CLONE ir<%ld.B> = load ir<%gep.B>
; CHECK-NEXT:    CLONE ir<%gep.C> = getelementptr inbounds ir<%C>, vp<%adjusted.iv>
; CHECK-NEXT:    CLONE ir<%ld.C> = load ir<%gep.C>
; CHECK-NEXT:    EMIT ir<%add> = add ir<%ld.A>, ir<%ld.B>
; CHECK-NEXT:    EMIT ir<%sub> = sub ir<%add>, ir<%ld.C>
; CHECK-NEXT:    EMIT ir<%mul> = mul ir<%sub>, ir<3>
; CHECK-NEXT:    EMIT ir<%xor> = xor ir<%mul>, ir<7>
; CHECK-NEXT:    EMIT ir<%div> = udiv ir<%xor>, ir<3>
; CHECK-NEXT:    CLONE ir<%fr> = freeze ir<%div>
; CHECK-NEXT:    EMIT ir<%cmp> = icmp eq ir<%fr>, ir<0>
; CHECK-NEXT:    EMIT branch-on-cond ir<%cmp> (!vplan.prof.estimated estimated {67108864, 2080374784})
; CHECK-NEXT:  Successor(s): oracle.exit, loop.latch
; CHECK-EMPTY:
; CHECK-NEXT:  loop.latch:
...
; CHECK-NEXT:    CLONE ir<%gep.A> = getelementptr inbounds ir<%A>, vp<[[VP2]]>
; CHECK-NEXT:    CLONE ir<%ld.A> = load ir<%gep.A>
; CHECK-NEXT:    CLONE ir<%gep.B> = getelementptr inbounds ir<%B>, vp<[[VP2]]>
; CHECK-NEXT:    CLONE ir<%ld.B> = load ir<%gep.B>
; CHECK-NEXT:    EMIT ir<%cmp> = icmp ne ir<%ld.A>, ir<%ld.B>
; CHECK-NEXT:    EMIT branch-on-cond ir<%cmp> (!vplan.prof.estimated estimated {67108864, 2080374784})
; CHECK-NEXT:  Successor(s): oracle.exit, loop.latch
...
; CHECK-NEXT:      EMIT vp<[[VP9:%[0-9]+]]> = any-of ir<%cmp>
; CHECK-NEXT:      EMIT vp<%index.next> = add nuw vp<[[VP4]]>, vp<[[VP1]]>
...
; CHECK-NEXT:      EMIT vp<[[VP8:%[0-9]+]]> = any-of ir<%c>
; CHECK-NEXT:      EMIT vp<%index.next> = add nuw vp<[[VP3]]>, vp<[[VP1]]>
; CHECK-NEXT:      EMIT vp<[[VP9:%[0-9]+]]> = icmp eq vp<%index.next>, vp<[[VP2]]>
; CHECK-NEXT:      EMIT branch-on-two-conds vp<[[VP8]]>, vp<[[VP9]]>
; CHECK-NEXT:    No successors
...
; CHECK-EMPTY:
; CHECK-NEXT:  vector.early.exit:
; CHECK-NEXT:    EMIT vp<[[VP12:%[0-9]+]]> = first-active-lane ir<%c>
; CHECK-NEXT:    EMIT vp<[[VP13:%[0-9]+]]> = add vp<[[VP3]]>, vp<[[VP12]]>
; CHECK-NEXT:  Successor(s): ir-bb<early.exit>
; CHECK-EMPTY:
; CHECK-NEXT:  ir-bb<early.exit>:
...
; CHECK-NEXT:    EMIT-SCALAR vp<%index> = phi [ ir<0>, oracle.entry ], [ vp<%index.next>, loop.latch ]
; CHECK-NEXT:    EMIT vp<%adjusted.iv> = add vp<%index>, vp<[[VP1]]>
; CHECK-NEXT:    CLONE ir<%gep> = getelementptr inbounds ir<@G>, vp<%adjusted.iv>
