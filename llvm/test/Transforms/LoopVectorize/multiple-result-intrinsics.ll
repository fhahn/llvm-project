...
; CHECK:  [[VECTOR_PH:.*:]]
; CHECK:  [[VECTOR_BODY:.*:]]
; CHECK:    [[TMP1:%.*]] = call { <2 x double>, <2 x double> } @llvm.sincos.v2f64(<2 x double> [[WIDE_LOAD:%.*]])
; CHECK:    store <2 x double> zeroinitializer, ptr [[TMP2:%.*]], align 8
; CHECK:  [[IF1:.*:]]
; CHECK:    [[TMP3:%.*]] = extractvalue { <2 x double>, <2 x double> } [[TMP1]], 1
; CHECK:    [[TMP4:%.*]] = extractelement <2 x double> [[TMP3]], i64 1
