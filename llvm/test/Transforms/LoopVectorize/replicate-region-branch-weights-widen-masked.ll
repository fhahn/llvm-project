; NOTE: Assertions have been autogenerated by utils/update_test_checks.py UTC_ARGS: --filter "masked|br i1|!prof|!llvm.loop|^.*:" --version 5
; RUN: opt < %s -S -passes=loop-vectorize -mattr=+sve -force-vector-width=4 -force-vector-interleave=1 | FileCheck %s

target datalayout = "e-m:e-i8:8:32-i16:16:32-i64:64-i128:128-n32:64-S128"
target triple = "aarch64-unknown-linux-gnu"

; The branch_weights the predicator attaches to the masked recipes of a
; predicated block must not be emitted onto a widened masked memory operation.
; Here the predicated store is widened to a llvm.masked.store rather than
; scalarized into a replicate region, so the branch_weights are only valid on
; the conditional branch guarding the block. Check the masked.store/masked.load
; intrinsics carry no !prof metadata (emitting it there is invalid IR).
