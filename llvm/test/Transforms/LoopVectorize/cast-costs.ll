; NOTE: Assertions have been autogenerated by utils/update_analyze_test_checks.py UTC_ARGS: --filter "Cost.*(EMIT-SCALAR|CLONE).*(trunc|sext|zext)" --filter "Cost.*EMIT .*(trunc|sext|zext)" --filter-out-after "LV: Selecting VF" --version 6
; REQUIRES: asserts
...
; CHECK:  Cost of 1 for VF 2: EMIT vp<[[VP6:%[0-9]+]]> = trunc ir<%red.next> to i1
; CHECK:  Cost of 1 for VF 2: EMIT vp<[[VP7:%[0-9]+]]> = sext vp<[[VP6]]> to i32
; CHECK:  Cost of 1 for VF 2: EMIT-SCALAR vp<[[VP10:%[0-9]+]]> = sext vp<[[VP9:%[0-9]+]]> to i32
; CHECK:  Cost of 1 for VF 4: EMIT vp<[[VP6]]> = trunc ir<%red.next> to i1
; CHECK:  Cost of 1 for VF 4: EMIT vp<[[VP7]]> = sext vp<[[VP6]]> to i32
...
; CHECK:  Cost of 0 for VF 2: EMIT ir<%zext> = zext ir<%load> to i32
; CHECK:  Cost of 1 for VF 2: EMIT vp<[[VP7:%[0-9]+]]> = trunc ir<%red.next> to i8
; CHECK:  Cost of 1 for VF 2: EMIT vp<[[VP8:%[0-9]+]]> = zext vp<[[VP7]]> to i32
; CHECK:  Cost of 1 for VF 2: EMIT-SCALAR vp<[[VP11:%[0-9]+]]> = zext vp<[[VP10:%[0-9]+]]> to i32
; CHECK:  Cost of 0 for VF 4: EMIT ir<%zext> = zext ir<%load> to i32
; CHECK:  Cost of 1 for VF 4: EMIT vp<[[VP7]]> = trunc ir<%red.next> to i8
; CHECK:  Cost of 1 for VF 4: EMIT vp<[[VP8]]> = zext vp<[[VP7]]> to i32
