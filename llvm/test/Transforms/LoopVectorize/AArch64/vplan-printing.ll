...
; CHECK-NEXT:   WIDEN-INTRINSIC vp<[[REDUCE]]> = call llvm.vector.partial.reduce.add(ir<[[ACC]]>, ir<%mul>, ir<4>)
...
; CHECK-NEXT:   WIDEN-REDUCTION-PHI ir<%accum> = phi vp<[[RDX_START]]>, vp<[[REDUCE:%.+]]> (VF scaled by 1/4)
...
; CHECK-NEXT:   WIDEN-CAST ir<%ext.b> = zext ir<%load.b> to i32
; CHECK-NEXT:   WIDEN ir<%mul> = mul ir<%ext.b>, ir<%ext.a>
; CHECK-NEXT:   WIDEN-INTRINSIC vp<[[REDUCE]]> = call llvm.vector.partial.reduce.add(ir<%accum>, ir<%mul>, ir<4>)
...
; CHECK-NEXT:   EMIT-SCALAR vp<{{.+}}> = resume-for-epilogue vp<%vec.epilog.resume.val>
; CHECK-NEXT: Successor(s): ir-bb<for.body>
; CHECK-EMPTY:
; CHECK-NEXT: ir-bb<for.body>:
