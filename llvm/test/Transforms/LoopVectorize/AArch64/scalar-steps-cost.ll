...
; the vector body executes at most once, so the canonical IV is a constant and
; each lane's step folds away with it. At VF 2 the body runs twice and the add
; remains.
...
; The store is scalarized and predicated, so the loop contains two identical
; scalar steps recipes: one outside the replicate region, only used by a
; single-scalar GEP, and one inside it, generating a value per lane in the
; pred.store blocks.
