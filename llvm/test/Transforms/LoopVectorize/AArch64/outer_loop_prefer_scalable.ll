...
; CHECK-NEXT:    [[WIDE_LOAD:%.*]] = load <vscale x 4 x float>, ptr [[TMP14]], align 4, !llvm.access.group [[ACC_GRP0:![0-9]+]]
...
; CHECK-NEXT:    [[WIDE_MASKED_GATHER2:%.*]] = call <vscale x 4 x float> @llvm.masked.gather.nxv4f32.nxv4p0(<vscale x 4 x ptr> align 4 [[TMP8]], <vscale x 4 x i1> splat (i1 true), <vscale x 4 x float> poison), !llvm.access.group [[ACC_GRP0]]
...
; CHECK:       [[VECTOR_LATCH]]:
; CHECK-NEXT:    store <vscale x 4 x float> [[TMP9]], ptr [[TMP14]], align 4, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], [[TMP1]]
; CHECK-NEXT:    [[TMP13:%.*]] = icmp eq i64 [[INDEX_NEXT]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[TMP13]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP1:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
...
; CHECK-NEXT:    br i1 [[OUTER_EXITCOND]], label %[[EXIT]], label %[[OUTER_LOOP]], !llvm.loop [[LOOP5:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
...
outer_loop:
  ...
  %x.start = load float, ptr %arrayidx1, align 4, !llvm.access.group !3
  br label %inner_loop
...
inner_loop:
  ...
  %b = load float, ptr %arrayidx2, align 4, !llvm.access.group !3
...
outer_loop_latch:
  store float %x.next, ptr %arrayidx1, align 4, !llvm.access.group !3
