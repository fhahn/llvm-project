; NOTE: Assertions have been autogenerated by utils/update_analyze_test_checks.py UTC_ARGS: --filter "Cost of [0-9]+(.[0-9]+)? for VF 1: (EMIT(-SCALAR)? ir<%valB>|WIDEN ir<%valB>|REPLICATE ir<%valB>|WIDEN-GATHER ir<%valB>) = load" --filter "Cost of [0-9]+(.[0-9]+)? for VF [0-9]+: .* ir<%valB> = load"
