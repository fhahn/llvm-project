; NOTE: Assertions have been autogenerated by utils/update_analyze_test_checks.py UTC_ARGS: --filter "LV: Found an estimated cost of [0-9]+ for VF 1 For instruction:\s*store i16 %valB, ptr %out" --filter "Cost of [1-9][0-9]*(\.[0-9]+)? for VF [0-9]+: (profitable to scalarize\s+store i16 %valB|WIDEN store .*, ir<%valB>|REPLICATE store ir<%valB>)"
; RUN: opt -passes=loop-vectorize -vectorizer-maximize-bandwidth -S -mattr=+sse2 --debug-only=loop-vectorize --disable-output < %s 2>&1 | FileCheck %s --check-prefixes=SSE
; RUN: opt -passes=loop-vectorize -vectorizer-maximize-bandwidth -S -mattr=+sse4.2 --debug-only=loop-vectorize --disable-output < %s 2>&1 | FileCheck %s --check-prefixes=SSE
; RUN: opt -passes=loop-vectorize -vectorizer-maximize-bandwidth -S -mattr=+avx  --debug-only=loop-vectorize --disable-output < %s 2>&1 | FileCheck %s --check-prefixes=AVX1
; RUN: opt -passes=loop-vectorize -vectorizer-maximize-bandwidth -S -mattr=+avx2,-fast-gather --debug-only=loop-vectorize --disable-output < %s 2>&1 | FileCheck %s --check-prefixes=AVX2
; RUN: opt -passes=loop-vectorize -vectorizer-maximize-bandwidth -S -mattr=+avx2,+fast-gather --debug-only=loop-vectorize --disable-output < %s 2>&1 | FileCheck %s --check-prefixes=AVX2
; RUN: opt -passes=loop-vectorize -vectorizer-maximize-bandwidth -S -mattr=+avx512bw --debug-only=loop-vectorize --disable-output < %s 2>&1 | FileCheck %s --check-prefixes=AVX512

; REQUIRES: asserts

target datalayout = "e-m:e-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

@A = global [1024 x i8] zeroinitializer, align 128
@B = global [1024 x i16] zeroinitializer, align 128

define void @test(ptr %C) {
; AVX1-LABEL: 'test'
; AVX1:  LV: Found an estimated cost of 1 for VF 1 For instruction: store i16 %valB, ptr %out, align 2
; AVX1:  Cost of 2 for VF 2: REPLICATE store ir<%valB>, ir<%out> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX1:  Cost of 4 for VF 4: REPLICATE store ir<%valB>, ir<%out> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX1:  Cost of 8 for VF 8: REPLICATE store ir<%valB>, ir<%out> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX1:  Cost of 16.5 for VF 16: REPLICATE store ir<%valB>, ir<%out> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX1:  Cost of 33 for VF 32: REPLICATE store ir<%valB>, ir<%out> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
;
; AVX512-LABEL: 'test'
; AVX512:  LV: Found an estimated cost of 1 for VF 1 For instruction: store i16 %valB, ptr %out, align 2
; AVX512:  Cost of 2 for VF 2: WIDEN store vp<[[VP7:%[0-9]+]]>, ir<%valB>, ir<%canStore> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX512:  Cost of 2 for VF 4: WIDEN store vp<[[VP7]]>, ir<%valB>, ir<%canStore> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX512:  Cost of 1 for VF 8: WIDEN store vp<[[VP7]]>, ir<%valB>, ir<%canStore> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX512:  Cost of 1 for VF 16: WIDEN store vp<[[VP7]]>, ir<%valB>, ir<%canStore> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX512:  Cost of 1 for VF 32: WIDEN store vp<[[VP7]]>, ir<%valB>, ir<%canStore> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
; AVX512:  Cost of 2 for VF 64: WIDEN store vp<[[VP7]]>, ir<%valB>, ir<%canStore> (!alias.scope {{<0x[0-9a-f]+>}}, !noalias {{<0x[0-9a-f]+>}})
;
entry:
  br label %for.body

for.body:
  %iv = phi i64 [ 0, %entry ], [ %iv.next, %end ]

  %inB = getelementptr inbounds [1024 x i16], ptr @B, i64 0, i64 %iv
  %valB = load i16, ptr %inB

  %inA = getelementptr inbounds [1024 x i8], ptr @A, i64 0, i64 %iv
  %valA = load i8, ptr %inA
  %canStore = icmp ne i8 %valA, 0
  br i1 %canStore, label %store, label %mask

store:
  %out = getelementptr inbounds [1024 x i16], ptr %C, i64 0, i64 %iv
  store i16 %valB, ptr %out
  br label %end

mask:
  br label %end

end:
  %iv.next = add nuw nsw i64 %iv, 1
  %cmp = icmp ult i64 %iv.next, 1024
  br i1 %cmp, label %for.body, label %for.cond.cleanup

for.cond.cleanup:
  ret void
}
;; NOTE: These prefixes are unused and the list is autogenerated. Do not add tests below this line:
; AVX2: {{.*}}
; SSE: {{.*}}
