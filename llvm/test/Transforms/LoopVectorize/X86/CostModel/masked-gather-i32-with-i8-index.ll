; NOTE: Assertions have been autogenerated by utils/update_analyze_test_checks.py UTC_ARGS: --filter "Cost of [0-9]+(.[0-9]+)? for VF 1: (EMIT(-SCALAR)? ir<%valB.loaded>|WIDEN ir<%valB.loaded>|REPLICATE ir<%valB.loaded>|WIDEN-GATHER ir<%valB.loaded>) = load" --filter "Cost of [0-9]+(.[0-9]+)? for VF [0-9]+: .* ir<%valB.loaded> = load"
