...
; RUN: opt -S -passes=loop-vectorize -enable-vplan-native-path -mtriple x86_64 -disable-outer-loop-memory-safety-check < %s | FileCheck %s

; Exercises the debug-only verifyOuterLoopInnerIVsAreCanonical assert on a
; single-level nest with a canonical inner IV (safety check disabled as the
; symbolic trip count M needs a runtime M >= VF guard added later).
...
; CHECK:       [[INNER_BODY1]]:
; CHECK-NEXT:    [[J2:%.*]] = phi <4 x i64> [ zeroinitializer, %[[OUTER_HEADER1]] ], [ [[TMP5:%.*]], %[[INNER_BODY1]] ]
...
; CHECK-NEXT:    br i1 [[TMP8]], label %[[MIDDLE_BLOCK:.*]], label %[[OUTER_HEADER1]], !llvm.loop [[LOOP0:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
...
; CHECK-NEXT:    br i1 [[I_CMP]], label %[[EXIT]], label %[[OUTER_HEADER]], !llvm.loop [[LOOP3:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
