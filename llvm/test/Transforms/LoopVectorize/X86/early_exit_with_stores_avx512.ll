...
; RUN: opt -p loop-vectorize -force-vector-width=4 -enable-early-exit-vectorization-with-side-effects -early-exit-vectorization-bail-to-scalar -mtriple=x86_64-- -mattr=+avx512f,+avx512bw,+avx512vl -S %s | FileCheck %s

; AVX512 has a native masked <4 x i16> load/store, so the masked style is used
; even though the bail-to-scalar style is allowed.
