...
; RUN: opt -passes=loop-vectorize -mtriple=riscv64 -mattr=+v -low-trip-count-loop-body-size-limit=0 -tail-folding-policy=dont-fold-tail -S %s | FileCheck %s --check-prefix=NO-EPILOGUE
...
; The trip count (4) is a multiple of the VF, but the countable early exit
; requires the final iteration to execute in the scalar loop. Without a scalar
; epilogue, the loop must not be vectorized with VF = 4.
