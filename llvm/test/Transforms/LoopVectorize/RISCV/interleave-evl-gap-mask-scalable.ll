...
; A strided byte access forms an interleave group with gaps that requires a gap
; mask. When vectorized with scalable VFs under EVL tail folding, the gap-masked
; interleave group must NOT be lowered to a VPInterleaveEVLRecipe: gap masks are
; not supported for scalable vectors. Previously the EVL transform unconditionally
; converted the interleave recipe and tripped an assertion in the
; VPInterleaveEVLRecipe constructor. Now the conversion is skipped, so the loop is
; vectorized with a supported strategy (here a strided load + scatter) instead of
; crashing.
