...
; RUN: opt -S -pass-remarks=loop-vectorize -passes=loop-vectorize -enable-vplan-native-path -disable-outer-loop-memory-safety-check -force-target-supports-scalable-vectors < %s 2>&1 | FileCheck %s
; RUN: opt -S -pass-remarks-analysis=loop-vectorize -passes=loop-vectorize -enable-vplan-native-path -disable-outer-loop-memory-safety-check < %s 2>&1 | FileCheck %s -check-prefix=NO_SCALABLE_VECS
...
; CHECK-NEXT:    store <vscale x 4 x float> [[TMP12]], ptr [[TMP6]], align 4
