...
; CHECK-NEXT:    [[TMP1:%.*]] = getelementptr inbounds float, ptr [[B]], i64 [[INDEX]]
; CHECK-NEXT:    [[WIDE_LOAD1:%.*]] = load <4 x float>, ptr [[TMP1]], align 4
; CHECK-NEXT:    [[TMP7:%.*]] = fsub fast <4 x float> splat (float -0.000000e+00), [[WIDE_LOAD1]]
; CHECK-NEXT:    [[TMP2:%.*]] = call fast float @llvm.vector.reduce.fadd.v4f32(float 0.000000e+00, <4 x float> [[WIDE_LOAD]])
...
; CHECK-INTERLEAVED-NEXT:    [[WIDE_LOAD3:%.*]] = load <4 x float>, ptr [[TMP2]], align 4
; CHECK-INTERLEAVED-NEXT:    [[WIDE_LOAD4:%.*]] = load <4 x float>, ptr [[TMP3]], align 4
; CHECK-INTERLEAVED-NEXT:    [[TMP13:%.*]] = fsub fast <4 x float> splat (float -0.000000e+00), [[WIDE_LOAD3]]
; CHECK-INTERLEAVED-NEXT:    [[TMP14:%.*]] = fsub fast <4 x float> splat (float -0.000000e+00), [[WIDE_LOAD4]]
; CHECK-INTERLEAVED-NEXT:    [[TMP4:%.*]] = call fast float @llvm.vector.reduce.fadd.v4f32(float 0.000000e+00, <4 x float> [[WIDE_LOAD]])
; CHECK-INTERLEAVED-NEXT:    [[TMP5:%.*]] = fadd fast float [[VEC_PHI]], [[TMP4]]
...
; CHECK-INTERLEAVED-NEXT:    br i1 [[TMP12]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP35:![0-9]+]]
; CHECK-INTERLEAVED:       [[MIDDLE_BLOCK]]:
...
; CHECK-INTERLEAVED-NEXT:    br i1 [[EC]], label %[[EXIT]], label %[[LOOP]], !llvm.loop [[LOOP36:![0-9]+]]
; CHECK-INTERLEAVED:       [[EXIT]]:
...
; Negating the operand of an fsub link must preserve the sign of zero, so it
; cannot use +0.0 - X. Only reassoc is set here, not nsz.
define float @fadd_fsub_chain_inloop_reassoc_only(ptr %a, ptr %b, i64 %n) {
; CHECK-LABEL: define float @fadd_fsub_chain_inloop_reassoc_only(
; CHECK-SAME: ptr [[A:%.*]], ptr [[B:%.*]], i64 [[N:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    [[MIN_ITERS_CHECK:%.*]] = icmp ult i64 [[N]], 4
; CHECK-NEXT:    br i1 [[MIN_ITERS_CHECK]], label %[[SCALAR_PH:.*]], label %[[VECTOR_PH:.*]]
...
; CHECK-NEXT:    [[TMP1:%.*]] = getelementptr inbounds float, ptr [[B]], i64 [[INDEX]]
; CHECK-NEXT:    [[WIDE_LOAD1:%.*]] = load <4 x float>, ptr [[TMP1]], align 4
; CHECK-NEXT:    [[TMP2:%.*]] = fsub reassoc <4 x float> splat (float -0.000000e+00), [[WIDE_LOAD1]]
; CHECK-NEXT:    [[TMP3:%.*]] = call reassoc float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> [[WIDE_LOAD]])
; CHECK-NEXT:    [[TMP4:%.*]] = fadd reassoc float [[VEC_PHI]], [[TMP3]]
; CHECK-NEXT:    [[TMP5:%.*]] = call reassoc float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> [[TMP2]])
; CHECK-NEXT:    [[TMP6]] = fadd reassoc float [[TMP4]], [[TMP5]]
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], 4
...
; CHECK-NEXT:    [[GB:%.*]] = getelementptr inbounds float, ptr [[B]], i64 [[IV]]
; CHECK-NEXT:    [[Y:%.*]] = load float, ptr [[GB]], align 4
; CHECK-NEXT:    [[ADD:%.*]] = fadd reassoc float [[ACC]], [[X]]
; CHECK-NEXT:    [[SUB]] = fsub reassoc float [[ADD]], [[Y]]
; CHECK-NEXT:    [[IV_NEXT]] = add i64 [[IV]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp eq i64 [[IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EC]], label %[[EXIT]], label %[[LOOP]], !llvm.loop [[LOOP38:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    [[SUB_LCSSA:%.*]] = phi float [ [[SUB]], %[[LOOP]] ], [ [[TMP6]], %[[MIDDLE_BLOCK]] ]
; CHECK-NEXT:    ret float [[SUB_LCSSA]]
;
; CHECK-INTERLEAVED-LABEL: define float @fadd_fsub_chain_inloop_reassoc_only(
; CHECK-INTERLEAVED-SAME: ptr [[A:%.*]], ptr [[B:%.*]], i64 [[N:%.*]]) {
; CHECK-INTERLEAVED-NEXT:  [[ENTRY:.*]]:
; CHECK-INTERLEAVED-NEXT:    [[MIN_ITERS_CHECK:%.*]] = icmp ult i64 [[N]], 8
; CHECK-INTERLEAVED-NEXT:    br i1 [[MIN_ITERS_CHECK]], label %[[SCALAR_PH:.*]], label %[[VECTOR_PH:.*]]
...
; CHECK-INTERLEAVED:       [[VECTOR_BODY]]:
; CHECK-INTERLEAVED-NEXT:    [[INDEX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-INTERLEAVED-NEXT:    [[VEC_PHI:%.*]] = phi float [ -0.000000e+00, %[[VECTOR_PH]] ], [ [[TMP11:%.*]], %[[VECTOR_BODY]] ]
; CHECK-INTERLEAVED-NEXT:    [[VEC_PHI1:%.*]] = phi float [ -0.000000e+00, %[[VECTOR_PH]] ], [ [[TMP13:%.*]], %[[VECTOR_BODY]] ]
; CHECK-INTERLEAVED-NEXT:    [[TMP0:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[INDEX]]
; CHECK-INTERLEAVED-NEXT:    [[TMP1:%.*]] = getelementptr inbounds float, ptr [[TMP0]], i64 4
; CHECK-INTERLEAVED-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x float>, ptr [[TMP0]], align 4
...
; CHECK-INTERLEAVED-NEXT:    [[WIDE_LOAD3:%.*]] = load <4 x float>, ptr [[TMP2]], align 4
; CHECK-INTERLEAVED-NEXT:    [[WIDE_LOAD4:%.*]] = load <4 x float>, ptr [[TMP3]], align 4
; CHECK-INTERLEAVED-NEXT:    [[TMP4:%.*]] = fsub reassoc <4 x float> splat (float -0.000000e+00), [[WIDE_LOAD3]]
; CHECK-INTERLEAVED-NEXT:    [[TMP5:%.*]] = fsub reassoc <4 x float> splat (float -0.000000e+00), [[WIDE_LOAD4]]
; CHECK-INTERLEAVED-NEXT:    [[TMP6:%.*]] = call reassoc float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> [[WIDE_LOAD]])
; CHECK-INTERLEAVED-NEXT:    [[TMP7:%.*]] = fadd reassoc float [[VEC_PHI]], [[TMP6]]
; CHECK-INTERLEAVED-NEXT:    [[TMP8:%.*]] = call reassoc float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> [[WIDE_LOAD2]])
; CHECK-INTERLEAVED-NEXT:    [[TMP9:%.*]] = fadd reassoc float [[VEC_PHI1]], [[TMP8]]
; CHECK-INTERLEAVED-NEXT:    [[TMP10:%.*]] = call reassoc float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> [[TMP4]])
; CHECK-INTERLEAVED-NEXT:    [[TMP11]] = fadd reassoc float [[TMP7]], [[TMP10]]
; CHECK-INTERLEAVED-NEXT:    [[TMP12:%.*]] = call reassoc float @llvm.vector.reduce.fadd.v4f32(float -0.000000e+00, <4 x float> [[TMP5]])
; CHECK-INTERLEAVED-NEXT:    [[TMP13]] = fadd reassoc float [[TMP9]], [[TMP12]]
; CHECK-INTERLEAVED-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], 8
; CHECK-INTERLEAVED-NEXT:    [[TMP14:%.*]] = icmp eq i64 [[INDEX_NEXT]], [[N_VEC]]
; CHECK-INTERLEAVED-NEXT:    br i1 [[TMP14]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP37:![0-9]+]]
; CHECK-INTERLEAVED:       [[MIDDLE_BLOCK]]:
; CHECK-INTERLEAVED-NEXT:    [[BIN_RDX:%.*]] = fadd reassoc float [[TMP13]], [[TMP11]]
; CHECK-INTERLEAVED-NEXT:    [[CMP_N:%.*]] = icmp eq i64 [[N]], [[N_VEC]]
; CHECK-INTERLEAVED-NEXT:    br i1 [[CMP_N]], label %[[EXIT:.*]], label %[[SCALAR_PH]]
; CHECK-INTERLEAVED:       [[SCALAR_PH]]:
; CHECK-INTERLEAVED-NEXT:    [[BC_RESUME_VAL:%.*]] = phi i64 [ [[N_VEC]], %[[MIDDLE_BLOCK]] ], [ 0, %[[ENTRY]] ]
...
; CHECK-INTERLEAVED-NEXT:    [[GB:%.*]] = getelementptr inbounds float, ptr [[B]], i64 [[IV]]
; CHECK-INTERLEAVED-NEXT:    [[Y:%.*]] = load float, ptr [[GB]], align 4
; CHECK-INTERLEAVED-NEXT:    [[ADD:%.*]] = fadd reassoc float [[ACC]], [[X]]
; CHECK-INTERLEAVED-NEXT:    [[SUB]] = fsub reassoc float [[ADD]], [[Y]]
; CHECK-INTERLEAVED-NEXT:    [[IV_NEXT]] = add i64 [[IV]], 1
; CHECK-INTERLEAVED-NEXT:    [[EC:%.*]] = icmp eq i64 [[IV_NEXT]], [[N]]
; CHECK-INTERLEAVED-NEXT:    br i1 [[EC]], label %[[EXIT]], label %[[LOOP]], !llvm.loop [[LOOP38:![0-9]+]]
; CHECK-INTERLEAVED:       [[EXIT]]:
; CHECK-INTERLEAVED-NEXT:    [[SUB_LCSSA:%.*]] = phi float [ [[SUB]], %[[LOOP]] ], [ [[BIN_RDX]], %[[MIDDLE_BLOCK]] ]
; CHECK-INTERLEAVED-NEXT:    ret float [[SUB_LCSSA]]
;
...
loop:
  ...
  %ga = getelementptr inbounds float, ptr %a, i64 %iv
  %x = load float, ptr %ga, align 4
  %gb = getelementptr inbounds float, ptr %b, i64 %iv
  %y = load float, ptr %gb, align 4
  %add = fadd reassoc float %acc, %x
  %sub = fsub reassoc float %add, %y
  %iv.next = add i64 %iv, 1
  %ec = icmp eq i64 %iv.next, %n
  br i1 %ec, label %exit, label %loop
