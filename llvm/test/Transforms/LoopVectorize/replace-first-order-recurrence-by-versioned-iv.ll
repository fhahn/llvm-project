...
; CHECK-NEXT:    br i1 [[CMP_N]], label %[[FOR_END:.*]], label %[[SCALAR_PH]]
; CHECK:       [[SCALAR_PH]]:
; CHECK-NEXT:    [[BC_RESUME_VAL:%.*]] = phi i32 [ [[TMP10]], %[[MIDDLE_BLOCK]] ], [ 0, %[[ENTRY]] ], [ 0, %[[VECTOR_SCEVCHECK]] ], [ 0, %[[VECTOR_MEMCHECK]] ]
; CHECK-NEXT:    [[SCALAR_RECUR_INIT:%.*]] = phi i64 [ [[N_VEC]], %[[MIDDLE_BLOCK]] ], [ 0, %[[ENTRY]] ], [ 0, %[[VECTOR_SCEVCHECK]] ], [ 0, %[[VECTOR_MEMCHECK]] ]
; CHECK-NEXT:    br label %[[FOR_BODY:.*]]
; CHECK:       [[FOR_BODY]]:
; CHECK-NEXT:    [[PHI32:%.*]] = phi i32 [ [[BC_RESUME_VAL]], %[[SCALAR_PH]] ], [ [[I32NEXT:%.*]], %[[FOR_BODY]] ]
...
; The FOR's user is sitofp (not a GEP), so sinking succeeds and the FOR is
; kept. The attached induction (with its i32->i64 zext predicate) is erased,
; removing the SCEV overflow check that would otherwise be needed. The BTC
; and memory accesses use a separate i64 IV, so the main PSE has no predicates.
...
; CHECK-NEXT:    [[PHI64:%.*]] = phi i64 [ [[BC_RESUME_VAL]], %[[SCALAR_PH]] ], [ [[I64NEXT:%.*]], %[[FOR_BODY]] ]
...
; The i16 induction needs a nuw predicate for the zext to i64. The FOR's user
; is a GEP, so the induction is kept and the predicate must be preserved as a
; SCEV overflow check.
...
; Same loop as above, but the FOR's user is sitofp (not a GEP), so sinking
; succeeds and the FOR is kept. The attached i16 induction (with its nuw
; predicate) is erased, removing the SCEV overflow check.
