...
; RUN: opt -passes=loop-vectorize -enable-vplan-native-path -enable-outer-loop-safety-checks -force-vector-width=4 -S %s | FileCheck %s
...
; CHECK-NEXT:    [[IDX:%.*]] = add nsw i64 [[OUTER_IV_MUL_M]], [[INNER_IV]]
...
; CHECK-NEXT:    br i1 [[OUTER_IV_CMP]], label %[[EXIT:.*]], label %[[OUTER_HEADER]], !llvm.loop [[LOOP8:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    [[A_PTR:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[IDX]]
; CHECK-NEXT:    [[A_VAL:%.*]] = load float, ptr [[A_PTR]], align 4
; CHECK-NEXT:    [[C_PTR:%.*]] = getelementptr inbounds float, ptr [[C]], i64 [[IDX]]
; CHECK-NEXT:    [[C_VAL:%.*]] = load float, ptr [[C_PTR]], align 4
; CHECK-NEXT:    [[ADD:%.*]] = fadd float [[A_VAL]], [[C_VAL]]
; CHECK-NEXT:    [[SUM_NEXT]] = fadd float [[SUM]], [[ADD]]
; CHECK-NEXT:    [[INNER_IV_NEXT]] = add nuw nsw i64 [[INNER_IV]], 1
; CHECK-NEXT:    [[INNER_IV_CMP:%.*]] = icmp eq i64 [[INNER_IV_NEXT]], [[M]]
; CHECK-NEXT:    br i1 [[INNER_IV_CMP]], label %[[OUTER_LATCH]], label %[[INNER_BODY]]
; CHECK:       [[OUTER_LATCH]]:
...
; CHECK-NEXT:    [[OUTER_IV_NEXT]] = add nuw nsw i64 [[OUTER_IV]], 1
; CHECK-NEXT:    [[OUTER_IV_CMP:%.*]] = icmp eq i64 [[OUTER_IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[OUTER_IV_CMP]], label %[[EXIT:.*]], label %[[OUTER_HEADER]], !llvm.loop [[LOOP8]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
...
; CHECK:       [[INNER_BODY]]:
; CHECK-NEXT:    [[INNER_IV:%.*]] = phi i64 [ 0, %[[OUTER_HEADER]] ], [ [[INNER_IV_NEXT:%.*]], %[[INNER_BODY]] ]
; CHECK-NEXT:    [[INNER_IV_SCALED:%.*]] = mul i64 [[INNER_IV]], 9223372036854775807
; CHECK-NEXT:    [[IDX:%.*]] = add i64 [[OUTER_IV]], [[INNER_IV_SCALED]]
; CHECK-NEXT:    [[A_PTR:%.*]] = getelementptr inbounds i8, ptr [[A]], i64 [[IDX]]
; CHECK-NEXT:    [[INNER_IV_NEXT]] = add nuw nsw i64 [[INNER_IV]], 1
; CHECK-NEXT:    [[INNER_IV_CMP:%.*]] = icmp eq i64 [[INNER_IV_NEXT]], 3
; CHECK-NEXT:    br i1 [[INNER_IV_CMP]], label %[[OUTER_LATCH]], label %[[INNER_BODY]]
; CHECK:       [[OUTER_LATCH]]:
; CHECK-NEXT:    [[A_PTR_LCSSA:%.*]] = phi ptr [ [[A_PTR]], %[[INNER_BODY]] ]
; CHECK-NEXT:    [[A_VAL:%.*]] = load i8, ptr [[A_PTR_LCSSA]], align 1
; CHECK-NEXT:    [[NEG_OUTER_IV:%.*]] = sub i64 0, [[OUTER_IV]]
; CHECK-NEXT:    [[B_PTR:%.*]] = getelementptr inbounds i8, ptr [[B]], i64 [[NEG_OUTER_IV]]
; CHECK-NEXT:    store i8 [[A_VAL]], ptr [[B_PTR]], align 1
; CHECK-NEXT:    [[OUTER_IV_NEXT]] = add nuw nsw i64 [[OUTER_IV]], 1
...
; CHECK-NEXT:    br i1 [[TMP4]], label %[[OUTER_LATCH3:.*]], label %[[INNER_BODY1]], !llvm.loop [[LOOP10:![0-9]+]]
; CHECK:       [[OUTER_LATCH3]]:
...
; CHECK-NEXT:    [[SUM:%.*]] = phi float [ 0.000000e+00, %[[OUTER_HEADER]] ], [ [[SUM_NEXT:%.*]], %[[INNER_LATCH]] ]
; CHECK-NEXT:    br i1 [[COND]], label %[[INNER_THEN:.*]], label %[[INNER_LATCH]]
; CHECK:       [[INNER_THEN]]:
; CHECK-NEXT:    [[IDX:%.*]] = add nsw i64 [[OUTER_IV_MUL_M]], [[INNER_IV]]
; CHECK-NEXT:    [[A_PTR:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[IDX]]
; CHECK-NEXT:    [[A_VAL:%.*]] = load float, ptr [[A_PTR]], align 4
...
; CHECK:       [[INNER_LATCH]]:
; CHECK-NEXT:    [[VAL:%.*]] = phi float [ [[A_VAL]], %[[INNER_THEN]] ], [ 0.000000e+00, %[[INNER_HEADER]] ]
; CHECK-NEXT:    [[SUM_NEXT]] = fadd float [[SUM]], [[VAL]]
; CHECK-NEXT:    [[INNER_IV_NEXT]] = add nuw nsw i64 [[INNER_IV]], 1
; CHECK-NEXT:    [[INNER_IV_CMP:%.*]] = icmp eq i64 [[INNER_IV_NEXT]], [[M]]
; CHECK-NEXT:    br i1 [[INNER_IV_CMP]], label %[[OUTER_LATCH]], label %[[INNER_HEADER]]
; CHECK:       [[OUTER_LATCH]]:
; CHECK-NEXT:    [[SUM_NEXT_LCSSA:%.*]] = phi float [ [[SUM_NEXT]], %[[INNER_LATCH]] ]
; CHECK-NEXT:    [[B_PTR:%.*]] = getelementptr inbounds float, ptr [[B]], i64 [[OUTER_IV]]
; CHECK-NEXT:    store float [[SUM_NEXT_LCSSA]], ptr [[B_PTR]], align 4
; CHECK-NEXT:    [[OUTER_IV_NEXT]] = add nuw nsw i64 [[OUTER_IV]], 1
; CHECK-NEXT:    [[OUTER_IV_CMP:%.*]] = icmp eq i64 [[OUTER_IV_NEXT]], [[N]]
...
; CHECK:       [[OUTER_LATCH]]:
; CHECK-NEXT:    [[SUM_NEXT_LCSSA:%.*]] = phi float [ [[SUM_NEXT]], %[[INNER_BODY]] ]
; CHECK-NEXT:    [[B_PTR:%.*]] = getelementptr float, ptr [[B]], i64 [[OUTER_IV]]
; CHECK-NEXT:    store float [[SUM_NEXT_LCSSA]], ptr [[B_PTR]], align 4
; CHECK-NEXT:    [[OUTER_IV_NEXT]] = add nuw nsw i64 [[OUTER_IV]], 1
; CHECK-NEXT:    [[OUTER_IV_CMP:%.*]] = icmp eq i64 [[OUTER_IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[OUTER_IV_CMP]], label %[[EXIT:.*]], label %[[OUTER_HEADER]], !llvm.loop [[LOOP8]]
...
; CHECK:       [[INNER_BODY]]:
; CHECK-NEXT:    [[INNER_IV:%.*]] = phi i64 [ 0, %[[OUTER_HEADER]] ], [ [[INNER_IV_NEXT:%.*]], %[[INNER_BODY]] ]
; CHECK-NEXT:    [[INNER_IV_MUL_2:%.*]] = mul nsw i64 [[INNER_IV]], 2
; CHECK-NEXT:    [[IDX:%.*]] = add nsw i64 [[OUTER_IV]], [[INNER_IV_MUL_2]]
; CHECK-NEXT:    [[A_PTR:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[IDX]]
; CHECK-NEXT:    [[A_VAL:%.*]] = load float, ptr [[A_PTR]], align 4
; CHECK-NEXT:    [[OUTER_IV_FP:%.*]] = sitofp i64 [[OUTER_IV]] to float
; CHECK-NEXT:    [[SUB:%.*]] = fsub float [[OUTER_IV_FP]], [[A_VAL]]
...
; CHECK:       [[OUTER_HEADER]]:
; CHECK-NEXT:    [[OUTER_IV:%.*]] = phi i64 [ 0, %[[ENTRY]] ], [ [[OUTER_IV_NEXT:%.*]], %[[OUTER_LATCH:.*]] ]
; CHECK-NEXT:    [[OUTER_IV_MUL_8:%.*]] = mul nsw i64 [[OUTER_IV]], 8
; CHECK-NEXT:    br label %[[INNER_BODY:.*]]
; CHECK:       [[INNER_BODY]]:
; CHECK-NEXT:    [[INNER_IV:%.*]] = phi i64 [ 0, %[[OUTER_HEADER]] ], [ [[INNER_IV_NEXT:%.*]], %[[INNER_BODY]] ]
; CHECK-NEXT:    [[IDX:%.*]] = add nsw i64 [[OUTER_IV_MUL_8]], [[INNER_IV]]
; CHECK-NEXT:    [[A_PTR:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[IDX]]
; CHECK-NEXT:    [[A_VAL:%.*]] = load float, ptr [[A_PTR]], align 4
; CHECK-NEXT:    [[OUTER_IV_FP:%.*]] = sitofp i64 [[OUTER_IV]] to float
; CHECK-NEXT:    [[SUB:%.*]] = fsub float [[OUTER_IV_FP]], [[A_VAL]]
...
; CHECK:       [[INNER_BODY]]:
; CHECK-NEXT:    [[INNER_IV:%.*]] = phi i64 [ 0, %[[OUTER_HEADER]] ], [ [[INNER_IV_NEXT:%.*]], %[[INNER_BODY]] ]
; CHECK-NEXT:    [[INNER_IV_MUL_4:%.*]] = mul nsw i64 [[INNER_IV]], 4
; CHECK-NEXT:    [[IDX:%.*]] = add nsw i64 [[OUTER_IV]], [[INNER_IV_MUL_4]]
; CHECK-NEXT:    [[A_PTR:%.*]] = getelementptr inbounds i8, ptr [[A]], i64 [[IDX]]
; CHECK-NEXT:    [[A_VAL:%.*]] = load float, ptr [[A_PTR]], align 1
; CHECK-NEXT:    [[OUTER_IV_FP:%.*]] = sitofp i64 [[OUTER_IV]] to float
; CHECK-NEXT:    [[SUB:%.*]] = fsub float [[OUTER_IV_FP]], [[A_VAL]]
; CHECK-NEXT:    store float [[SUB]], ptr [[A_PTR]], align 1
; CHECK-NEXT:    [[INNER_IV_NEXT]] = add nuw nsw i64 [[INNER_IV]], 1
; CHECK-NEXT:    [[INNER_IV_CMP:%.*]] = icmp eq i64 [[INNER_IV_NEXT]], 4
; CHECK-NEXT:    br i1 [[INNER_IV_CMP]], label %[[OUTER_LATCH]], label %[[INNER_BODY]]
; CHECK:       [[OUTER_LATCH]]:
; CHECK-NEXT:    [[OUTER_IV_NEXT]] = add nuw nsw i64 [[OUTER_IV]], 1
; CHECK-NEXT:    [[OUTER_IV_CMP:%.*]] = icmp eq i64 [[OUTER_IV_NEXT]], [[N]]
