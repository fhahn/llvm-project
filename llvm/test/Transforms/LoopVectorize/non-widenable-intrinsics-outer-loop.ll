...
; recipe and do not crash the VPlan native path. The array consists of disjoint
; rows of n elements (n > 0); the noncanonical IV skips two rows per iteration.
; Lifetime intrinsics conservatively prevent outer-loop vectorization.
...
; CHECK-NEXT:    [[TMP0:%.*]] = icmp sge <2 x i64> [[VEC_IND]], zeroinitializer
...
; CHECK-NEXT:    [[TMP10:%.*]] = mul nuw nsw <2 x i64> [[VEC_IND]], [[BROADCAST_SPLAT]]
; CHECK-NEXT:    [[TMP2:%.*]] = add nuw nsw <2 x i64> [[TMP10]], [[TMP1]]
; CHECK-NEXT:    [[TMP3:%.*]] = getelementptr inbounds i32, ptr [[ARR]], <2 x i64> [[TMP2]]
; CHECK-NEXT:    call void @llvm.masked.scatter.v2i32.v2p0(<2 x i32> splat (i32 1), <2 x ptr> align 4 [[TMP3]], <2 x i1> splat (i1 true)), !llvm.access.group [[ACC_GRP0:![0-9]+]]
...
; CHECK-NEXT:    br i1 [[TMP7]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP1:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
...
; CHECK-NEXT:    [[CMP:%.*]] = icmp sge i64 [[J]], 0
; CHECK-NEXT:    call void @llvm.assume(i1 [[CMP]])
; CHECK-NEXT:    br label %[[INNER:.*]]
; CHECK:       [[INNER]]:
; CHECK-NEXT:    [[K:%.*]] = phi i64 [ 0, %[[OUTER_HEADER]] ], [ [[K_NEXT:%.*]], %[[INNER]] ]
; CHECK-NEXT:    [[ROW:%.*]] = mul nuw nsw i64 [[J]], [[N]]
; CHECK-NEXT:    [[IDX:%.*]] = add nuw nsw i64 [[ROW]], [[K]]
; CHECK-NEXT:    [[PTR:%.*]] = getelementptr inbounds i32, ptr [[ARR]], i64 [[IDX]]
; CHECK-NEXT:    store i32 1, ptr [[PTR]], align 4, !llvm.access.group [[ACC_GRP0]]
...
outer.header:
  ...
  %cmp = icmp sge i64 %j, 0
  call void @llvm.assume(i1 %cmp)
  br label %inner

inner:
  %k = phi i64 [ 0, %outer.header ], [ %k.next, %inner ]
  %row = mul nuw nsw i64 %j, %n
  %idx = add nuw nsw i64 %row, %k
  %ptr = getelementptr inbounds i32, ptr %arr, i64 %idx
  store i32 1, ptr %ptr, align 4, !llvm.access.group !3
  %k.next = add i64 %k, 1
...
; CHECK-NEXT:    [[TMP14:%.*]] = mul nuw nsw <2 x i64> [[VEC_IND]], [[BROADCAST_SPLAT]]
; CHECK-NEXT:    [[TMP6:%.*]] = add nuw nsw <2 x i64> [[TMP14]], [[TMP5]]
; CHECK-NEXT:    [[TMP7:%.*]] = getelementptr inbounds i32, ptr [[ARR]], <2 x i64> [[TMP6]]
; CHECK-NEXT:    [[WIDE_MASKED_GATHER:%.*]] = call <2 x i32> @llvm.masked.gather.v2i32.v2p0(<2 x ptr> align 4 [[TMP7]], <2 x i1> splat (i1 true), <2 x i32> poison), !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    call void @llvm.masked.scatter.v2i32.v2p0(<2 x i32> [[TMP9]], <2 x ptr> align 4 [[TMP7]], <2 x i1> splat (i1 true)), !llvm.access.group [[ACC_GRP0]]
...
; CHECK:       [[INNER]]:
; CHECK-NEXT:    [[K:%.*]] = phi i64 [ 0, %[[OUTER_HEADER]] ], [ [[K_NEXT:%.*]], %[[INNER]] ]
; CHECK-NEXT:    [[ROW:%.*]] = mul nuw nsw i64 [[M]], [[N]]
; CHECK-NEXT:    [[IDX:%.*]] = add nuw nsw i64 [[ROW]], [[K]]
; CHECK-NEXT:    [[PTR:%.*]] = getelementptr inbounds i32, ptr [[ARR]], i64 [[IDX]]
; CHECK-NEXT:    [[VAL:%.*]] = load i32, ptr [[PTR]], align 4, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    [[INC:%.*]] = select i1 [[CMP]], i32 1, i32 -1
; CHECK-NEXT:    [[VAL2:%.*]] = add i32 [[VAL]], [[INC]]
; CHECK-NEXT:    store i32 [[VAL2]], ptr [[PTR]], align 4, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    [[K_NEXT]] = add i64 [[K]], 1
; CHECK-NEXT:    [[INNER_COND:%.*]] = icmp eq i64 [[K_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[INNER_COND]], label %[[OUTER_LATCH]], label %[[INNER]]
; CHECK:       [[OUTER_LATCH]]:
...
inner:
  %k = phi i64 [ 0, %outer.header ], [ %k.next, %inner ]
  %row = mul nuw nsw i64 %m, %n
  %idx = add nuw nsw i64 %row, %k
  %ptr = getelementptr inbounds i32, ptr %arr, i64 %idx
  %val = load i32, ptr %ptr, align 4, !llvm.access.group !3
  %inc = select i1 %cmp, i32 1, i32 -1
  %val2 = add i32 %val, %inc
  store i32 %val2, ptr %ptr, align 4, !llvm.access.group !3
  %k.next = add i64 %k, 1
  %inner.cond = icmp eq i64 %k.next, %n
  br i1 %inner.cond, label %outer.latch, label %inner
...
; CHECK-NEXT:    [[J_NEXT]] = add i64 [[J]], 1
; CHECK-NEXT:    [[OUTER_COND:%.*]] = icmp eq i64 [[J_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[OUTER_COND]], label %[[EXIT:.*]], label %[[OUTER_HEADER]], !llvm.loop [[LOOP9:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
...
; CHECK-NEXT:    [[TMP7:%.*]] = mul nuw nsw <2 x i64> [[VEC_IND]], [[BROADCAST_SPLAT]]
; CHECK-NEXT:    [[TMP1:%.*]] = add nuw nsw <2 x i64> [[TMP7]], [[TMP0]]
; CHECK-NEXT:    [[TMP2:%.*]] = getelementptr inbounds i32, ptr [[ARR]], <2 x i64> [[TMP1]]
; CHECK-NEXT:    call void @llvm.masked.scatter.v2i32.v2p0(<2 x i32> splat (i32 1), <2 x ptr> align 4 [[TMP2]], <2 x i1> splat (i1 true)), !llvm.access.group [[ACC_GRP0]]
