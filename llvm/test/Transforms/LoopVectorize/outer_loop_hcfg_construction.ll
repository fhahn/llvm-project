...
; The parallel-access contract requires disjoint rows a[i], each separate from
; the pointer array a. Only accesses in the selected loop are annotated.
;
...
; CHECK-NEXT:    [[WIDE_MASKED_GATHER:%.*]] = load <4 x ptr>, ptr [[TMP8]], align 8, !llvm.access.group [[ACC_GRP0:![0-9]+]]
...
; CHECK-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[BROADCAST_SPLAT]], <4 x ptr> align 4 [[TMP4]], <4 x i1> splat (i1 true)), !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    [[TMP11:%.*]] = load ptr, ptr [[ARRAYIDX11_US_US]], align 8, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    br label %[[INNERMOST_LOOP:.*]]
; CHECK:       [[INNERMOST_LOOP]]:
...
; CHECK-NEXT:    store i32 [[TMP2]], ptr [[ARRAYIDX13_US_US]], align 4, !llvm.access.group [[ACC_GRP0]]
...
; CHECK:       [[MIDDLE_LOOP_LATCH]]:
; CHECK-NEXT:    [[I_NEXT]] = add nuw nsw i64 [[I]], 1
; CHECK-NEXT:    [[EXITCOND41_NOT:%.*]] = icmp eq i64 [[I_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EXITCOND41_NOT]], label %[[OUTERMOST_LOOP_LATCH]], label %[[MIDDLE_LOOP]], !llvm.loop [[LOOP5:![0-9]+]]
; CHECK:       [[OUTERMOST_LOOP_LATCH]]:
...
middle.loop:
  ...
  %3 = load ptr, ptr %arrayidx11.us.us, align 8, !llvm.access.group !7
...
innermost.loop:
  ...
  store i32 %2, ptr %arrayidx13.us.us, align 4, !llvm.access.group !7
...
; CHECK-NEXT:    [[WIDE_MASKED_GATHER:%.*]] = call <4 x ptr> @llvm.masked.gather.v4p0.v4p0(<4 x ptr> align 8 [[TMP3]], <4 x i1> splat (i1 true), <4 x ptr> poison), !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    [[TMP12:%.*]] = load ptr, ptr [[GEP]], align 8, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    [[ARRAYIDX17:%.*]] = getelementptr inbounds i32, ptr [[TMP12]], i64 [[J0]]
; CHECK-NEXT:    store i32 [[CONV]], ptr [[ARRAYIDX17]], align 4, !llvm.access.group [[ACC_GRP0]]
...
innermost.loop:
  ...
  %2 = load ptr, ptr %gep, align 8, !llvm.access.group !7
  %arrayidx17 = getelementptr inbounds i32, ptr %2, i64 %j0
  store i32 %conv, ptr %arrayidx17, align 4, !llvm.access.group !7
