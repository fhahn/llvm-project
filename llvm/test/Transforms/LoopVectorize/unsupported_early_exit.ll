...
; The induction step is computed at runtime, so its defining recipe lives in
; the entry block the oracle plan disconnects.
define i64 @early_exit_runtime_induction_step(ptr %A, i64 %n, i64 %s) {
; CHECK-LABEL: define i64 @early_exit_runtime_induction_step(
; CHECK-SAME: ptr [[A:%.*]], i64 [[N:%.*]], i64 [[S:%.*]]) {
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[STEP:%.*]] = mul i64 [[S]], 4
; CHECK-NEXT:    br label [[LOOP:%.*]]
; CHECK:       loop:
; CHECK-NEXT:    [[IV:%.*]] = phi i64 [ 0, [[ENTRY:%.*]] ], [ [[IV_NEXT:%.*]], [[LOOP_LATCH:%.*]] ]
...
; CHECK-NEXT:    br i1 [[EC]], label [[EXIT:%.*]], label [[LOOP]]
; CHECK:       early.exit:
; CHECK-NEXT:    [[J_LCSSA:%.*]] = phi i64 [ [[J]], [[LOOP]] ]
