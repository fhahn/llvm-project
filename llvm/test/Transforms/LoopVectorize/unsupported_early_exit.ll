...
; CHECK-NEXT:    [[BAD_CALL:%.*]] = call i32 @foo(i32 [[LD1]]) #[[ATTR4:[0-9]+]]
...
; CHECK:       oracle.loop:
; CHECK-NEXT:    [[TMP5:%.*]] = phi i64 [ 0, [[VECTOR_BODY]] ], [ [[INDEX_NEXT6:%.*]], [[LOOP_INC5:%.*]] ]
; CHECK-NEXT:    [[TMP2:%.*]] = add i64 [[TMP5]], [[INDEX1]]
; CHECK-NEXT:    [[TMP6:%.*]] = add i64 3, [[TMP2]]
; CHECK-NEXT:    [[TMP7:%.*]] = getelementptr i8, ptr [[MASK]], i64 [[TMP6]]
; CHECK-NEXT:    [[TMP23:%.*]] = load i8, ptr [[TMP7]], align 1
; CHECK-NEXT:    [[TMP9:%.*]] = icmp ne i8 [[TMP23]], 0
; CHECK-NEXT:    br i1 [[TMP9]], label [[LOOP_SEARCH4:%.*]], label [[LOOP_INC5]]
; CHECK:       oracle.loop.search:
; CHECK-NEXT:    [[TMP10:%.*]] = getelementptr i8, ptr [[P1]], i64 [[TMP6]]
; CHECK-NEXT:    [[TMP11:%.*]] = load i8, ptr [[TMP10]], align 1
; CHECK-NEXT:    [[TMP27:%.*]] = getelementptr i8, ptr [[P2]], i64 [[TMP6]]
; CHECK-NEXT:    [[TMP28:%.*]] = load i8, ptr [[TMP27]], align 1
; CHECK-NEXT:    [[TMP29:%.*]] = icmp eq i8 [[TMP11]], [[TMP28]]
; CHECK-NEXT:    br i1 [[TMP29]], label [[LOOP_INC5]], label [[ORACLE_EXIT:%.*]]
; CHECK:       oracle.loop.inc:
; CHECK-NEXT:    [[INDEX_NEXT6]] = add nuw i64 [[TMP5]], 1
; CHECK-NEXT:    [[TMP15:%.*]] = icmp eq i64 [[INDEX_NEXT6]], 4
; CHECK-NEXT:    br i1 [[TMP15]], label [[ORACLE_EXIT]], label [[LOOP4]]
; CHECK:       oracle.oracle.exit:
; CHECK-NEXT:    [[TMP16:%.*]] = add i64 [[TMP5]], 1
; CHECK-NEXT:    br label [[LOOP_SPLIT:%.*]]
; CHECK:       vector.body.split:
; CHECK-NEXT:    [[TMP17:%.*]] = getelementptr inbounds i8, ptr [[MASK]], i64 [[OFFSET_IDX]]
; CHECK-NEXT:    [[TMP18:%.*]] = call <4 x i8> @llvm.speculative.load.v4i8.p0(ptr [[TMP17]], i64 [[TMP16]])
; CHECK-NEXT:    [[TMP19:%.*]] = icmp ne <4 x i8> [[TMP18]], zeroinitializer
; CHECK-NEXT:    [[TMP20:%.*]] = getelementptr inbounds i8, ptr [[P1]], i64 [[OFFSET_IDX]]
; CHECK-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x i8>, ptr [[TMP20]], align 1
; CHECK-NEXT:    [[TMP21:%.*]] = getelementptr inbounds i8, ptr [[P2]], i64 [[OFFSET_IDX]]
; CHECK-NEXT:    [[WIDE_LOAD7:%.*]] = load <4 x i8>, ptr [[TMP21]], align 1
; CHECK-NEXT:    [[TMP22:%.*]] = icmp ne <4 x i8> [[WIDE_LOAD]], [[WIDE_LOAD7]]
; CHECK-NEXT:    [[TMP8:%.*]] = select <4 x i1> [[TMP22]], <4 x i1> [[TMP19]], <4 x i1> zeroinitializer
; CHECK-NEXT:    [[INDEX_NEXT3]] = add nuw i64 [[INDEX1]], 4
; CHECK-NEXT:    [[TMP24:%.*]] = freeze <4 x i1> [[TMP8]]
; CHECK-NEXT:    [[TMP25:%.*]] = call i1 @llvm.vector.reduce.or.v4i1(<4 x i1> [[TMP24]])
; CHECK-NEXT:    [[TMP26:%.*]] = icmp eq i64 [[INDEX_NEXT3]], 64
; CHECK-NEXT:    br i1 [[TMP25]], label [[VECTOR_EARLY_EXIT:%.*]], label [[VECTOR_BODY_INTERIM]]
; CHECK:       vector.body.interim:
; CHECK-NEXT:    br i1 [[TMP26]], label [[MIDDLE_BLOCK:%.*]], label [[VECTOR_BODY]], !llvm.loop [[LOOP0:![0-9]+]]
; CHECK:       middle.block:
; CHECK-NEXT:    br label [[LOOP_END:%.*]]
; CHECK:       vector.early.exit:
; CHECK-NEXT:    [[TMP30:%.*]] = call i64 @llvm.experimental.cttz.elts.i64.v4i1(<4 x i1> [[TMP8]], i1 false)
; CHECK-NEXT:    [[TMP31:%.*]] = add i64 [[INDEX1]], [[TMP30]]
; CHECK-NEXT:    [[TMP32:%.*]] = add i64 3, [[TMP31]]
; CHECK-NEXT:    br label [[LOOP_END]]
; CHECK:       scalar.ph:
...
; CHECK:       loop.end:
; CHECK-NEXT:    [[RETVAL:%.*]] = phi i64 [ [[INDEX]], [[LOOP_SEARCH]] ], [ 67, [[LOOP_INC]] ], [ 67, [[MIDDLE_BLOCK]] ], [ [[TMP32]], [[VECTOR_EARLY_EXIT]] ]
; CHECK-NEXT:    ret i64 [[RETVAL]]
;
entry:
  %p1 = alloca [1024 x i8]
