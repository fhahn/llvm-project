...
; RUN: opt -p loop-vectorize -force-vector-width=4 -enable-early-exit-vectorization-with-side-effects -early-exit-vectorization-bail-to-scalar -S %s | FileCheck %s
...
; CHECK-NEXT:    [[TMP22:%.*]] = call i1 @llvm.vector.reduce.or.v4i1(<4 x i1> [[TMP2]])
...
; CHECK-NEXT:    [[TMP25:%.*]] = load <4 x i16>, ptr [[TMP6]], align 2
; CHECK-NEXT:    [[TMP26:%.*]] = add nsw <4 x i16> [[TMP25]], splat (i16 1)
; CHECK-NEXT:    store <4 x i16> [[TMP26]], ptr [[TMP6]], align 2
; CHECK-NEXT:    br label %[[PRED_LOAD_IF5]]
; CHECK:       [[PRED_LOAD_IF5]]:
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], 4
; CHECK-NEXT:    [[TMP33:%.*]] = icmp eq i64 [[INDEX_NEXT]], 20
; CHECK-NEXT:    [[TMP7:%.*]] = or i1 [[TMP22]], [[TMP33]]
; CHECK-NEXT:    br i1 [[TMP7]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP0:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP5:%.*]] = select i1 [[TMP22]], i64 0, i64 4
...
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], %[[PRED_STORE_IF5:.*]] ]
...
; CHECK-NEXT:    [[TMP17:%.*]] = call i1 @llvm.vector.reduce.or.v4i1(<4 x i1> [[TMP3]])
...
; CHECK-NEXT:    [[TMP7:%.*]] = or i1 [[TMP17]], [[TMP24]]
; CHECK-NEXT:    br i1 [[TMP7]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP4:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP2:%.*]] = select i1 [[TMP17]], i64 0, i64 4
...
; The store is in the latch block, i.e. after the early-exit check. The latch
; still runs when bailing, so the masking style is used.
...
; A value defined in the body (%data) is live out of the loop, so the body
; cannot be branched around and the masking style is used.
...
; CHECK-NEXT:    [[TMP1:%.*]] = icmp sgt <4 x i16> [[WIDE_LOAD]], splat (i16 500)
; CHECK-NEXT:    [[TMP3:%.*]] = freeze <4 x i1> [[TMP1]]
; CHECK-NEXT:    [[TMP24:%.*]] = call i1 @llvm.vector.reduce.or.v4i1(<4 x i1> [[TMP3]])
...
; CHECK-NEXT:    [[TMP4:%.*]] = icmp sgt <4 x i16> [[WIDE_LOAD]], splat (i16 500)
; CHECK-NEXT:    [[TMP2:%.*]] = freeze <4 x i1> [[TMP4]]
; CHECK-NEXT:    [[TMP34:%.*]] = call i1 @llvm.vector.reduce.or.v4i1(<4 x i1> [[TMP2]])
; CHECK-NEXT:    br i1 [[TMP34]], label %[[PRED_STORE_CONTINUE12]], label %[[VECTOR_BODY_NONBAILING:.*]]
...
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP5:%.*]] = select i1 [[TMP34]], i64 0, i64 4
...
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i32 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], %[[PRED_LOAD_IF5:.*]] ]
; CHECK-NEXT:    [[TMP3:%.*]] = getelementptr inbounds nuw i16, ptr [[PRED]], i32 [[INDEX]]
; CHECK-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x i16>, ptr [[TMP3]], align 2
; CHECK-NEXT:    [[TMP4:%.*]] = icmp sgt <4 x i16> [[WIDE_LOAD]], splat (i16 500)
; CHECK-NEXT:    [[TMP2:%.*]] = freeze <4 x i1> [[TMP4]]
; CHECK-NEXT:    [[TMP23:%.*]] = call i1 @llvm.vector.reduce.or.v4i1(<4 x i1> [[TMP2]])
; CHECK-NEXT:    br i1 [[TMP23]], label %[[PRED_LOAD_IF5]], label %[[PRED_LOAD_CONTINUE6:.*]]
; CHECK:       [[PRED_LOAD_CONTINUE6]]:
; CHECK-NEXT:    [[TMP5:%.*]] = getelementptr inbounds nuw i16, ptr [[ARRAY]], i32 [[INDEX]]
; CHECK-NEXT:    [[TMP26:%.*]] = load <4 x i16>, ptr [[TMP5]], align 2
; CHECK-NEXT:    [[TMP27:%.*]] = add nsw <4 x i16> [[TMP26]], splat (i16 1)
; CHECK-NEXT:    store <4 x i16> [[TMP27]], ptr [[TMP5]], align 2
; CHECK-NEXT:    br label %[[PRED_LOAD_IF5]]
; CHECK:       [[PRED_LOAD_IF5]]:
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i32 [[INDEX]], 4
; CHECK-NEXT:    [[TMP34:%.*]] = icmp eq i32 [[INDEX_NEXT]], 20
; CHECK-NEXT:    [[TMP7:%.*]] = or i1 [[TMP23]], [[TMP34]]
; CHECK-NEXT:    br i1 [[TMP7]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP16:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP6:%.*]] = select i1 [[TMP23]], i32 0, i32 4
...
; There is control flow between the header and the latch, so the body cannot be
; branched around as a single block and the masking style is used.
