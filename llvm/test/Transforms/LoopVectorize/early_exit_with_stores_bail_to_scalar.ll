...
define void @two_stores(ptr noalias %a, ptr noalias %b, ptr align 2 dereferenceable(40) readonly %pred) {
; CHECK-LABEL: define void @two_stores(
; CHECK-SAME: ptr noalias [[A:%.*]], ptr noalias [[B:%.*]], ptr readonly align 2 dereferenceable(40) [[PRED:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    br label %[[VECTOR_PH:.*]]
; CHECK:       [[VECTOR_PH]]:
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
...
define void @store_in_latch(ptr noalias %array, ptr align 2 dereferenceable(40) readonly %pred) {
...
define i16 @body_live_out(ptr noalias %array, ptr align 2 dereferenceable(40) readonly %pred) {
...
define void @nondivisible_trip_count(ptr noalias %array, ptr align 2 dereferenceable(42) readonly %pred) {
; CHECK-LABEL: define void @nondivisible_trip_count(
; CHECK-SAME: ptr noalias [[ARRAY:%.*]], ptr readonly align 2 dereferenceable(42) [[PRED:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    br label %[[VECTOR_PH:.*]]
; CHECK:       [[VECTOR_PH]]:
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
...
; a constant.
define void @offset_store_address(ptr noalias %array, ptr align 2 dereferenceable(40) readonly %pred) {
...
; A load from a loop-invariant address (%cfg) in the body. It must not be
; hoisted out of the vector loop once the body is only conditionally executed.
define void @single_store_with_invariant_load(ptr noalias %array, ptr align 2 dereferenceable(40) readonly %pred, ptr noalias readonly %cfg) {
; CHECK-LABEL: define void @single_store_with_invariant_load(
; CHECK-SAME: ptr noalias [[ARRAY:%.*]], ptr readonly align 2 dereferenceable(40) [[PRED:%.*]], ptr noalias readonly [[CFG:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    br label %[[VECTOR_PH:.*]]
; CHECK:       [[VECTOR_PH]]:
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
...
; The store is in its own block between the early-exit check and the latch. The
; conversion is not supported for loop bodies spanning more than one block.
define void @store_in_separate_block(ptr noalias %array, ptr align 2 dereferenceable(40) readonly %pred) {
; CHECK-LABEL: define void @store_in_separate_block(
; CHECK-SAME: ptr noalias [[ARRAY:%.*]], ptr readonly align 2 dereferenceable(40) [[PRED:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    br label %[[VECTOR_PH:.*]]
; CHECK:       [[VECTOR_PH]]:
...
; CHECK-NEXT:    [[EE_VAL:%.*]] = load i16, ptr [[EE_ADDR]], align 2
; CHECK-NEXT:    [[EE_COND:%.*]] = icmp sgt i16 [[EE_VAL]], 500
; CHECK-NEXT:    br i1 [[EE_COND]], label %[[EXIT]], label %[[LOOP_BODY:.*]]
; CHECK:       [[LOOP_BODY]]:
; CHECK-NEXT:    [[ST_ADDR:%.*]] = getelementptr inbounds nuw i16, ptr [[ARRAY]], i64 [[IV]]
; CHECK-NEXT:    [[DATA:%.*]] = load i16, ptr [[ST_ADDR]], align 2
; CHECK-NEXT:    [[INC:%.*]] = add nsw i16 [[DATA]], 1
...
loop.header:
  ...
  %ee.val = load i16, ptr %ee.addr, align 2
  %ee.cond = icmp sgt i16 %ee.val, 500
  br i1 %ee.cond, label %exit, label %loop.body
...
; The uncountable and the counted exit leave to different blocks.
define i16 @distinct_exit_blocks(ptr noalias %array, ptr align 2 dereferenceable(40) readonly %pred) {
; CHECK-LABEL: define i16 @distinct_exit_blocks(
; CHECK-SAME: ptr noalias [[ARRAY:%.*]], ptr readonly align 2 dereferenceable(40) [[PRED:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*:]]
; CHECK-NEXT:    br label %[[VECTOR_PH:.*]]
; CHECK:       [[VECTOR_PH]]:
...
; CHECK-NEXT:    [[EE_VAL:%.*]] = load i16, ptr [[EE_ADDR]], align 2
; CHECK-NEXT:    [[EE_COND:%.*]] = icmp sgt i16 [[EE_VAL]], 500
; CHECK-NEXT:    br i1 [[EE_COND]], label %[[EARLY_EXIT:.*]], label %[[LOOP_LATCH]]
; CHECK:       [[LOOP_LATCH]]:
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[COUNTED_COND:%.*]] = icmp eq i64 [[IV_NEXT]], 20
; CHECK-NEXT:    br i1 [[COUNTED_COND]], label %[[EXIT]], label %[[LOOP_HEADER]], !llvm.loop [[LOOP21:![0-9]+]]
; CHECK:       [[EARLY_EXIT]]:
...
loop.header:
  ...
  %ee.val = load i16, ptr %ee.addr, align 2
  %ee.cond = icmp sgt i16 %ee.val, 500
  br i1 %ee.cond, label %early.exit, label %loop.latch
