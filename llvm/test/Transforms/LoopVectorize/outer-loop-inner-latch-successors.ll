...
; CHECK-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x i64>, ptr [[TMP5]], align 4, !llvm.access.group [[ACC_GRP0:![0-9]+]]
; CHECK-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[BROADCAST_SPLAT]], <4 x ptr> align 4 [[TMP0]], <4 x i1> splat (i1 true)), !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    [[WIDE_MASKED_GATHER5:%.*]] = call <4 x i64> @llvm.masked.gather.v4i64.v4p0(<4 x ptr> align 4 [[TMP1]], <4 x i1> splat (i1 true), <4 x i64> poison), !llvm.access.group [[ACC_GRP0]]
...
; CHECK:       [[VECTOR_LATCH]]:
; CHECK-NEXT:    store <4 x i64> [[TMP3]], ptr [[TMP5]], align 4, !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    [[RED_START:%.*]] = load i64, ptr [[GEP_A]], align 4, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    store i32 [[C]], ptr [[GEP_A]], align 4, !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    [[TMP8:%.*]] = load i64, ptr [[GEP_B]], align 4, !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    store i64 [[RED_NEXT_LCSSA]], ptr [[GEP_A]], align 4, !llvm.access.group [[ACC_GRP0]]
...
outer.header:
  ...
  %red.start = load i64 , ptr %gep.A, align 4, !llvm.access.group !4
  store i32 %c, ptr %gep.A, align 4, !llvm.access.group !4
  br label %inner
...
inner:
  ...
  %2 = load i64, ptr %gep.B, align 4, !llvm.access.group !4
...
outer.latch:
  store i64 %red.next, ptr %gep.A, align 4, !llvm.access.group !4
...
; CHECK-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x i64>, ptr [[TMP9]], align 4, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[BROADCAST_SPLAT]], <4 x ptr> align 4 [[TMP0]], <4 x i1> splat (i1 true)), !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    br label %[[INNER3:.*]]
; CHECK:       [[INNER3]]:
; CHECK-NEXT:    [[VEC_PHI:%.*]] = phi <4 x i64> [ zeroinitializer, %[[VECTOR_BODY]] ], [ [[TMP4:%.*]], %[[INNER3]] ]
...
; CHECK:       [[VECTOR_LATCH]]:
; CHECK-NEXT:    store <4 x i64> [[TMP3]], ptr [[TMP9]], align 4, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], 4
; CHECK-NEXT:    [[VEC_IND_NEXT]] = add nuw nsw <4 x i64> [[VEC_IND]], splat (i64 4)
; CHECK-NEXT:    [[TMP7:%.*]] = icmp eq i64 [[INDEX_NEXT]], [[N_VEC]]
...
; CHECK-NEXT:    [[IV_OUTER_NEXT]] = add nuw nsw i64 [[IV_OUTER]], 1
; CHECK-NEXT:    [[OUTER_EC:%.*]] = icmp eq i64 [[IV_OUTER_NEXT]], [[M]]
; CHECK-NEXT:    br i1 [[OUTER_EC]], label %[[EXIT]], label %[[OUTER_HEADER]], !llvm.loop [[LOOP7:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
...
!4 = distinct !{}
!5 = !{!"llvm.loop.parallel_accesses", !4}
