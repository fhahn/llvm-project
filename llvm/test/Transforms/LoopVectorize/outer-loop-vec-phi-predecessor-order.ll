...
; Test from PR45958. The parallel-access contract requires 0 < n <= 2000,
; so each outer iteration updates a separate row.
...
; CHECK-NEXT:    [[WIDE_MASKED_GATHER:%.*]] = call <4 x i32> @llvm.masked.gather.v4i32.v4p0(<4 x ptr> align 4 [[TMP0]], <4 x i1> splat (i1 true), <4 x i32> poison), !llvm.access.group [[ACC_GRP0:![0-9]+]]
; CHECK-NEXT:    [[TMP1:%.*]] = mul nsw <4 x i32> [[WIDE_MASKED_GATHER]], splat (i32 10)
; CHECK-NEXT:    call void @llvm.masked.scatter.v4i32.v4p0(<4 x i32> [[TMP1]], <4 x ptr> align 4 [[TMP0]], <4 x i1> splat (i1 true)), !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    [[L1:%.*]] = load i32, ptr [[GEP_SRC]], align 4, !llvm.access.group [[ACC_GRP0]]
; CHECK-NEXT:    [[MUL:%.*]] = mul nsw i32 [[L1]], 10
; CHECK-NEXT:    store i32 [[MUL]], ptr [[GEP_SRC]], align 4, !llvm.access.group [[ACC_GRP0]]
...
; CHECK-NEXT:    br i1 [[EC_1]], label %[[EXIT]], label %[[LOOP_1_HEADER]], !llvm.loop [[LOOP5:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
...
loop.3:
  ...
  %l1 = load i32, ptr %gep.src, align 4, !llvm.access.group !3
  %mul = mul nsw i32 %l1, 10
  store i32 %mul, ptr %gep.src, align 4, !llvm.access.group !3
