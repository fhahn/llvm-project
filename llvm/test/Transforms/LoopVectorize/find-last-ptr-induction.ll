...
; Find-last reductions whose index is a pointer. A pointer cannot be reduced
; with min/max, so a fresh 0-based unit-step integer IV is reduced instead and
; the pointer rebuilt from the reduced index in the middle block. That also
; removes the only vector use of the pointer induction, so it collapses to a
; scalar one.
...
; CHECK-NEXT:    [[VEC_PHI:%.*]] = phi <4 x i64> [ splat (i64 -9223372036854775808), %[[VECTOR_PH]] ], [ [[TMP5:%.*]], %[[VECTOR_BODY]] ]
; CHECK-NEXT:    [[TMP3:%.*]] = shl i64 [[INDEX]], 2
; CHECK-NEXT:    [[NEXT_GEP:%.*]] = getelementptr i8, ptr [[A]], i64 [[TMP3]]
; CHECK-NEXT:    [[WIDE_LOAD:%.*]] = load <4 x i32>, ptr [[NEXT_GEP]], align 4
; CHECK-NEXT:    [[TMP4:%.*]] = icmp sgt <4 x i32> [[WIDE_LOAD]], splat (i32 3)
; CHECK-NEXT:    [[TMP5]] = select <4 x i1> [[TMP4]], <4 x i64> [[VEC_IND]], <4 x i64> [[VEC_PHI]]
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[INDEX]], 4
; CHECK-NEXT:    [[VEC_IND_NEXT]] = add nuw <4 x i64> [[VEC_IND]], splat (i64 4)
; CHECK-NEXT:    [[TMP6:%.*]] = icmp eq i64 [[INDEX_NEXT]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[TMP6]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP0:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP7:%.*]] = call i64 @llvm.vector.reduce.smax.v4i64(<4 x i64> [[TMP5]])
; CHECK-NEXT:    [[TMP8:%.*]] = shl i64 [[TMP7]], 2
; CHECK-NEXT:    [[TMP9:%.*]] = getelementptr i8, ptr [[A]], i64 [[TMP8]]
; CHECK-NEXT:    [[TMP10:%.*]] = icmp ne i64 [[TMP7]], -9223372036854775808
; CHECK-NEXT:    [[TMP11:%.*]] = select i1 [[TMP10]], ptr [[TMP9]], ptr null
...
; CHECK-NEXT:    [[BEST:%.*]] = phi ptr [ [[BC_MERGE_RDX]], %[[SCALAR_PH]] ], [ [[BEST_NEXT:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    [[L:%.*]] = load i32, ptr [[P]], align 4
; CHECK-NEXT:    [[CMP:%.*]] = icmp sgt i32 [[L]], 3
; CHECK-NEXT:    [[BEST_NEXT]] = select i1 [[CMP]], ptr [[P]], ptr [[BEST]]
; CHECK-NEXT:    [[P_NEXT]] = getelementptr inbounds i32, ptr [[P]], i64 1
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp eq i64 [[IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EC]], label %[[EXIT]], label %[[LOOP]], !llvm.loop [[LOOP3:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    [[R:%.*]] = phi ptr [ [[BEST_NEXT]], %[[LOOP]] ], [ [[TMP11]], %[[MIDDLE_BLOCK]] ]
...
loop:
  ...
  %best = phi ptr [ null, %entry ], [ %best.next, %loop ]
  %l = load i32, ptr %p
...
exit:
  %r = phi ptr [ %best.next, %loop ]
  ret ptr %r
}

; The pointer induction and the loaded array are independent, and the start
; value of the reduction is a non-null loop-invariant pointer.
...
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP7:%.*]] = call i64 @llvm.vector.reduce.smax.v4i64(<4 x i64> [[TMP5]])
; CHECK-NEXT:    [[TMP8:%.*]] = mul i64 [[TMP7]], 24
; CHECK-NEXT:    [[TMP9:%.*]] = getelementptr i8, ptr [[B]], i64 [[TMP8]]
; CHECK-NEXT:    [[TMP10:%.*]] = icmp ne i64 [[TMP7]], -9223372036854775808
; CHECK-NEXT:    [[TMP11:%.*]] = select i1 [[TMP10]], ptr [[TMP9]], ptr [[INIT]]
; CHECK-NEXT:    [[CMP_N:%.*]] = icmp eq i64 [[N]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[CMP_N]], label %[[EXIT:.*]], label %[[SCALAR_PH]]
; CHECK:       [[SCALAR_PH]]:
; CHECK-NEXT:    [[BC_RESUME_VAL:%.*]] = phi i64 [ [[N_VEC]], %[[MIDDLE_BLOCK]] ], [ 0, %[[ENTRY]] ]
...
loop:
  ...
  %best = phi ptr [ %init, %entry ], [ %best.next, %loop ]
...
; Negative test: the selected pointer is not a pointer induction, so the
; generic extract.last.active lowering is used unchanged.
...
; CHECK-NEXT:    [[BEST_NEXT]] = select i1 [[CMP]], ptr [[L]], ptr [[BEST]]
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp eq i64 [[IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EC]], label %[[EXIT]], label %[[LOOP]], !llvm.loop [[LOOP7:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    [[R:%.*]] = phi ptr [ [[BEST_NEXT]], %[[LOOP]] ], [ [[TMP9]], %[[MIDDLE_BLOCK]] ]
; CHECK-NEXT:    ret ptr [[R]]
;
entry:
  br label %loop
...
loop:
  ...
  %best.next = select i1 %cmp, ptr %l, ptr %best
  %iv.next = add nuw nsw i64 %iv, 1
  %ec = icmp eq i64 %iv.next, %n
  br i1 %ec, label %exit, label %loop
...
; Negative test: the pointer index is paired with a min/max reduction, i.e. it
; is an argmax. handleMultiUseReductions cannot pair an integer-typed index
; reduction with a pointer result, so the pointer index is left alone.
declare i32 @llvm.smax.i32(i32, i32)

define ptr @argmax_ptr_index(ptr %a, i64 %n) {
...
; CHECK-NEXT:    [[BP:%.*]] = phi ptr [ null, %[[ENTRY]] ], [ [[BP_NEXT:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    [[GEP:%.*]] = getelementptr inbounds i32, ptr [[A]], i64 [[IV]]
; CHECK-NEXT:    [[L:%.*]] = load i32, ptr [[GEP]], align 4
; CHECK-NEXT:    [[CMP:%.*]] = icmp sgt i32 [[L]], [[BEST]]
; CHECK-NEXT:    [[BP_NEXT]] = select i1 [[CMP]], ptr [[P]], ptr [[BP]]
...
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    [[R:%.*]] = phi ptr [ [[BP_NEXT]], %[[LOOP]] ]
; CHECK-NEXT:    ret ptr [[R]]
;
entry:
  %a0 = load i32, ptr %a
...
loop:
  ...
  %bp = phi ptr [ null, %entry ], [ %bp.next, %loop ]
  %gep = getelementptr inbounds i32, ptr %a, i64 %iv
  %l = load i32, ptr %gep
  %cmp = icmp sgt i32 %l, %best
  %bp.next = select i1 %cmp, ptr %p, ptr %bp
...
exit:
  %r = phi ptr [ %bp.next, %loop ]
  ret ptr %r
}

; Negative test: the pointer induction's step is not the canonical IV type, so
; the reduced index cannot be multiplied by it without a cast.
...
; CHECK-NEXT:    [[LAST:%.*]] = phi ptr addrspace(1) [ [[BC_MERGE_RDX]], %[[SCALAR_PH]] ], [ [[SEL:%.*]], %[[LOOP]] ]
; CHECK-NEXT:    [[L:%.*]] = load i32, ptr addrspace(1) [[PTR_IV]], align 4
; CHECK-NEXT:    [[C:%.*]] = icmp eq i32 [[L]], 3
; CHECK-NEXT:    [[SEL]] = select i1 [[C]], ptr addrspace(1) [[PTR_IV]], ptr addrspace(1) [[LAST]]
; CHECK-NEXT:    [[PTR_IV_NEXT]] = getelementptr inbounds i32, ptr addrspace(1) [[PTR_IV]], i32 1
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp eq i64 [[IV_NEXT]], [[N]]
; CHECK-NEXT:    br i1 [[EC]], label %[[EXIT]], label %[[LOOP]], !llvm.loop [[LOOP9:![0-9]+]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    [[SEL_LCSSA:%.*]] = phi ptr addrspace(1) [ [[SEL]], %[[LOOP]] ], [ [[TMP12]], %[[MIDDLE_BLOCK]] ]
; CHECK-NEXT:    ret ptr addrspace(1) [[SEL_LCSSA]]
;
entry:
  br label %loop
...
loop:
  ...
  %last = phi ptr addrspace(1) [ null, %entry ], [ %sel, %loop ]
  %l = load i32, ptr addrspace(1) %ptr.iv, align 4
  %c = icmp eq i32 %l, 3
  %sel = select i1 %c, ptr addrspace(1) %ptr.iv, ptr addrspace(1) %last
