...
; CHECK-NEXT:    [[INDEX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT4:%.*]], %[[FOR_BODY_SPLIT_INTERIM:.*]] ]
; CHECK-NEXT:    br label %[[FOR_BODY1:.*]]
; CHECK:       [[FOR_BODY1]]:
; CHECK-NEXT:    [[TMP2:%.*]] = phi i64 [ 0, %[[FOR_BODY_SPLIT]] ], [ [[INDEX_NEXT:%.*]], %[[IF_ELSE3:.*]] ]
; CHECK-NEXT:    [[TMP3:%.*]] = getelementptr [4 x i8], ptr [[A]], i64 [[TMP2]]
; CHECK-NEXT:    [[TMP15:%.*]] = getelementptr [4 x i8], ptr [[TMP3]], i64 [[INDEX]]
; CHECK-NEXT:    [[TMP4:%.*]] = load i32, ptr [[TMP15]], align 4
; CHECK-NEXT:    [[TMP5:%.*]] = icmp eq i32 [[TMP4]], [[K]]
; CHECK-NEXT:    br i1 [[TMP5]], label %[[ORACLE_EXIT:.*]], label %[[IF_ELSE3]]
; CHECK:       [[IF_ELSE3]]:
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i64 [[TMP2]], 1
; CHECK-NEXT:    [[TMP6:%.*]] = icmp eq i64 [[INDEX_NEXT]], 4
; CHECK-NEXT:    br i1 [[TMP6]], label %[[ORACLE_EXIT]], label %[[FOR_BODY1]]
; CHECK:       [[ORACLE_EXIT]]:
...
; CHECK-NEXT:    [[TMP10:%.*]] = call <4 x i32> @llvm.speculative.load.v4i32.p0(ptr [[TMP9]], i64 [[TMP8]])
...
; CHECK-NEXT:    br i1 [[DOTNOT]], label %[[FOR_BODY_SPLIT_INTERIM]], label %[[VECTOR_EARLY_EXIT:.*]]
; CHECK:       [[FOR_BODY_SPLIT_INTERIM]]:
; CHECK-NEXT:    [[INDEX_NEXT4]] = add nuw i64 [[INDEX]], 4
; CHECK-NEXT:    [[TMP14:%.*]] = icmp eq i64 [[INDEX_NEXT4]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[TMP14]], label %[[MIDDLE_BLOCK:.*]], label %[[FOR_BODY_SPLIT]], !llvm.loop [[LOOP4:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
...
; CHECK-NEXT:    [[TMP2:%.*]] = phi i64 [ 0, %[[FOR_BODY_SPLIT]] ], [ [[INDEX_NEXT:%.*]], %[[IF_ELSE3:.*]] ]
; CHECK-NEXT:    [[TMP3:%.*]] = getelementptr [4 x i8], ptr [[A]], i64 [[TMP2]]
; CHECK-NEXT:    [[TMP17:%.*]] = getelementptr [4 x i8], ptr [[TMP3]], i64 [[INDEX]]
...
; CHECK-NEXT:    [[INDEX_NEXT4]] = add nuw i64 [[INDEX]], 4
; CHECK-NEXT:    [[TMP14:%.*]] = icmp eq i64 [[INDEX_NEXT4]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[TMP14]], label %[[MIDDLE_BLOCK:.*]], label %[[FOR_BODY_SPLIT]], !llvm.loop [[LOOP8:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[CMP_N:%.*]] = icmp eq i64 [[N_VEC]], [[TMP0]]
; CHECK-NEXT:    br i1 [[CMP_N]], label %[[END_LOOPEXIT:.*]], label %[[SCALAR_PH]]
; CHECK:       [[VECTOR_EARLY_EXIT]]:
...
; CHECK:       [[END_LOOPEXIT]]:
; CHECK-NEXT:    [[RESULT_LCSSA:%.*]] = phi i64 [ [[INDVARS_IV]], %[[FOR_BODY]] ], [ -1, %[[IF_ELSE]] ], [ -1, %[[MIDDLE_BLOCK]] ], [ [[TMP16]], %[[VECTOR_EARLY_EXIT]] ]
