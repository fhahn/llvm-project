...
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    [[CMP8:%.*]] = icmp sgt i32 [[LENGTH]], 0
; CHECK-NEXT:    br i1 [[CMP8]], label %[[SCALAR_PH:.*]], label %[[END:.*]]
; CHECK:       [[SCALAR_PH]]:
...
; CHECK-NEXT:    br i1 [[TMP1]], label %[[VECTOR_PH:.*]], label %[[SCALAR_PH1]]
; CHECK:       [[VECTOR_PH]]:
...
; CHECK-NEXT:    [[TMP10:%.*]] = call <4 x i32> (ptr, ...) @llvm.speculative.load.v4i32.p0(ptr [[TMP9]], ptr nonnull @speculativeLoadOracle, i64 [[INDEX]], ptr [[A]], i32 [[K]], i32 [[LENGTH]])
; CHECK-NEXT:    [[TMP11:%.*]] = icmp eq <4 x i32> [[TMP10]], [[BROADCAST_SPLAT]]
; CHECK-NEXT:    [[TMP12:%.*]] = freeze <4 x i1> [[TMP11]]
; CHECK-NEXT:    [[TMP13:%.*]] = bitcast <4 x i1> [[TMP12]] to i4
; CHECK-NEXT:    [[DOTNOT:%.*]] = icmp eq i4 [[TMP13]], 0
...
; CHECK-NEXT:    br i1 [[TMP14]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY_SPLIT]], !llvm.loop [[LOOP4:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[CMP_N:%.*]] = icmp eq i64 [[N_VEC]], [[TMP0]]
; CHECK-NEXT:    br i1 [[CMP_N]], label %[[END_LOOPEXIT:.*]], label %[[SCALAR_PH1]]
...
; CHECK:       [[FOR_BODY1]]:
; CHECK-NEXT:    [[INDVARS_IV:%.*]] = phi i64 [ [[INDVARS_IV_NEXT:%.*]], %[[IF_ELSE:.*]] ], [ [[BC_RESUME_VAL]], %[[SCALAR_PH1]] ]
; CHECK-NEXT:    [[ARRAYIDX:%.*]] = getelementptr inbounds nuw [4 x i8], ptr [[A]], i64 [[INDVARS_IV]]
...
; CHECK-NEXT:    br i1 [[CMP]], label %[[FOR_BODY1]], label %[[END_LOOPEXIT]], !llvm.loop [[LOOP7:![0-9]+]]
; CHECK:       [[END_LOOPEXIT]]:
; CHECK-NEXT:    [[RESULT_LCSSA:%.*]] = phi i32 [ 1, %[[FOR_BODY1]] ], [ 0, %[[IF_ELSE]] ], [ 0, %[[MIDDLE_BLOCK]] ], [ 1, %[[VECTOR_EARLY_EXIT]] ]
; CHECK-NEXT:    br label %[[END]]
; CHECK:       [[END]]:
...
; CHECK-NEXT:    [[INDEX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT2:%.*]], %[[VECTOR_BODY_INTERIM:.*]] ]
; CHECK-NEXT:    [[TMP9:%.*]] = getelementptr inbounds [4 x i8], ptr [[A]], i64 [[INDEX]]
; CHECK-NEXT:    [[TMP10:%.*]] = call <4 x i32> (ptr, ...) @llvm.speculative.load.v4i32.p0(ptr [[TMP9]], ptr nonnull @speculativeLoadOracle.1, i64 [[INDEX]], ptr [[A]], i32 [[K]], i32 [[LENGTH]])
; CHECK-NEXT:    [[TMP11:%.*]] = icmp eq <4 x i32> [[TMP10]], [[BROADCAST_SPLAT]]
; CHECK-NEXT:    [[TMP12:%.*]] = freeze <4 x i1> [[TMP11]]
; CHECK-NEXT:    [[TMP13:%.*]] = bitcast <4 x i1> [[TMP12]] to i4
; CHECK-NEXT:    [[DOTNOT:%.*]] = icmp eq i4 [[TMP13]], 0
...
; CHECK-NEXT:    [[INDEX_NEXT2]] = add nuw i64 [[INDEX]], 4
; CHECK-NEXT:    [[TMP14:%.*]] = icmp eq i64 [[INDEX_NEXT2]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[TMP14]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY_SPLIT]], !llvm.loop [[LOOP8:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[CMP_N:%.*]] = icmp eq i64 [[N_VEC]], [[TMP0]]
; CHECK-NEXT:    br i1 [[CMP_N]], label %[[END_LOOPEXIT:.*]], label %[[SCALAR_PH1]]
; CHECK:       [[VECTOR_EARLY_EXIT]]:
...
; CHECK-NEXT:    [[TRUNC:%.*]] = trunc i64 [[INDVARS_IV_NEXT]] to i32
; CHECK-NEXT:    [[CMP:%.*]] = icmp sgt i32 [[LENGTH]], [[TRUNC]]
; CHECK-NEXT:    br i1 [[CMP]], label %[[FOR_BODY1]], label %[[END_LOOPEXIT]], !llvm.loop [[LOOP9:![0-9]+]]
; CHECK:       [[END_LOOPEXIT]]:
; CHECK-NEXT:    [[RESULT_LCSSA:%.*]] = phi i64 [ [[INDVARS_IV]], %[[FOR_BODY1]] ], [ -1, %[[IF_ELSE]] ], [ -1, %[[MIDDLE_BLOCK]] ], [ [[TMP16]], %[[VECTOR_EARLY_EXIT]] ]
...
; CHECK-NEXT:    [[TMP9:%.*]] = getelementptr [4 x i8], ptr [[A]], i64 [[INDEX]]
...
; CHECK-NEXT:    [[TMP12:%.*]] = getelementptr [4 x i8], ptr [[A]], i64 [[INDEX]]
