...
; CHECK:       [[LOOP_HEADER1]]:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT4:%.*]], %[[MIDDLE_BLOCK:.*]] ]
; CHECK-NEXT:    br label %[[LOOP_HEADER3:.*]]
; CHECK:       [[LOOP_HEADER3]]:
; CHECK-NEXT:    [[INDEX2:%.*]] = phi i64 [ 0, %[[LOOP_HEADER1]] ], [ [[INDEX_NEXT1:%.*]], %[[LOOP_LATCH3:.*]] ]
; CHECK-NEXT:    [[TMP14:%.*]] = add i64 [[INDEX2]], [[INDEX]]
; CHECK-NEXT:    [[TMP6:%.*]] = getelementptr i8, ptr [[A]], i64 [[TMP14]]
; CHECK-NEXT:    [[TMP7:%.*]] = load i8, ptr [[TMP6]], align 1
; CHECK-NEXT:    [[TMP17:%.*]] = getelementptr i8, ptr [[B]], i64 [[TMP14]]
; CHECK-NEXT:    [[TMP9:%.*]] = load i8, ptr [[TMP17]], align 1
; CHECK-NEXT:    [[TMP10:%.*]] = icmp eq i8 [[TMP7]], [[TMP9]]
; CHECK-NEXT:    br i1 [[TMP10]], label %[[LOOP_LATCH3]], label %[[ORACLE_EXIT:.*]]
; CHECK:       [[LOOP_LATCH3]]:
; CHECK-NEXT:    [[INDEX_NEXT1]] = add nuw i64 [[INDEX2]], 1
; CHECK-NEXT:    [[TMP11:%.*]] = icmp eq i64 [[INDEX_NEXT1]], 4
; CHECK-NEXT:    br i1 [[TMP11]], label %[[ORACLE_EXIT]], label %[[LOOP_HEADER3]]
; CHECK:       [[ORACLE_EXIT]]:
; CHECK-NEXT:    [[TMP21:%.*]] = add i64 [[INDEX2]], 1
; CHECK-NEXT:    br label %[[LOOP_HEADER_SPLIT:.*]]
; CHECK:       [[LOOP_HEADER_SPLIT]]:
; CHECK-NEXT:    [[TMP13:%.*]] = getelementptr inbounds i8, ptr [[A]], i64 [[INDEX]]
; CHECK-NEXT:    [[TMP5:%.*]] = call <4 x i8> @llvm.speculative.load.v4i8.p0(ptr [[TMP13]], i64 [[TMP21]])
; CHECK-NEXT:    [[TMP15:%.*]] = getelementptr inbounds i8, ptr [[B]], i64 [[INDEX]]
; CHECK-NEXT:    [[TMP16:%.*]] = call <4 x i8> @llvm.speculative.load.v4i8.p0(ptr [[TMP15]], i64 [[TMP21]])
; CHECK-NEXT:    [[TMP8:%.*]] = icmp ne <4 x i8> [[TMP5]], [[TMP16]]
; CHECK-NEXT:    [[INDEX_NEXT4]] = add nuw i64 [[INDEX]], 4
; CHECK-NEXT:    [[TMP18:%.*]] = freeze <4 x i1> [[TMP8]]
; CHECK-NEXT:    [[TMP19:%.*]] = call i1 @llvm.vector.reduce.or.v4i1(<4 x i1> [[TMP18]])
; CHECK-NEXT:    [[TMP20:%.*]] = icmp eq i64 [[INDEX_NEXT4]], 64
; CHECK-NEXT:    br i1 [[TMP19]], label %[[VECTOR_EARLY_EXIT:.*]], label %[[MIDDLE_BLOCK]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    br i1 [[TMP20]], label %[[MIDDLE_BLOCK1:.*]], label %[[LOOP_HEADER1]], !llvm.loop [[LOOP6:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK1]]:
...
; CHECK-NEXT:    [[TMP22:%.*]] = extractelement <4 x i8> [[TMP5]], i64 [[FIRST_ACTIVE_LANE]]
; CHECK-NEXT:    br label %[[LOOP_END:.*]]
; CHECK:       [[SCALAR_PH1]]:
; CHECK-NEXT:    [[BC_RESUME_VAL:%.*]] = phi i64 [ 64, %[[MIDDLE_BLOCK1]] ], [ 0, %[[LOOP]] ]
...
; CHECK-NEXT:    [[IV_NEXT]] = add i64 [[IV]], 1
; CHECK-NEXT:    [[EC:%.*]] = icmp ne i64 [[IV_NEXT]], 67
; CHECK-NEXT:    br i1 [[EC]], label %[[LOOP_HEADER2]], label %[[LOOP_END]], !llvm.loop [[LOOP7:![0-9]+]]
; CHECK:       [[LOOP_END]]:
; CHECK-NEXT:    [[RETVAL:%.*]] = phi i8 [ [[LD1]], %[[LOOP_HEADER2]] ], [ 0, %[[LOOP_LATCH]] ], [ [[TMP22]], %[[VECTOR_EARLY_EXIT]] ]
; CHECK-NEXT:    ret i8 [[RETVAL]]
;
entry:
  br label %loop.header
