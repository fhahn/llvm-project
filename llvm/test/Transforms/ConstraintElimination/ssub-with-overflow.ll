...
define i8 @ssub_no_overflow_due_to_non_positive_operands(i8 %a, i8 %b) {
; CHECK-LABEL: @ssub_no_overflow_due_to_non_positive_operands(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[C_1:%.*]] = icmp sle i8 [[B:%.*]], 0
; CHECK-NEXT:    [[C_2:%.*]] = icmp sle i8 [[A:%.*]], [[B]]
; CHECK-NEXT:    [[AND:%.*]] = and i1 [[C_1]], [[C_2]]
; CHECK-NEXT:    br i1 [[AND]], label [[MATH:%.*]], label [[EXIT_FAIL:%.*]]
; CHECK:       math:
; CHECK-NEXT:    [[TMP0:%.*]] = sub nsw i8 [[A]], [[B]]
; CHECK-NEXT:    br i1 false, label [[EXIT_FAIL]], label [[EXIT_OK:%.*]]
; CHECK:       exit.ok:
; CHECK-NEXT:    ret i8 [[TMP0]]
; CHECK:       exit.fail:
...
math:
  %op = tail call { i8, i1 } @llvm.ssub.with.overflow.i8(i8 %a, i8 %b)
  %status = extractvalue { i8, i1 } %op, 1
  br i1 %status, label %exit.fail, label %exit.ok
...
define i8 @ssub_may_overflow_only_second_operand_non_positive(i8 %a, i8 %b) {
; CHECK-LABEL: @ssub_may_overflow_only_second_operand_non_positive(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    [[C_1:%.*]] = icmp sle i8 [[B:%.*]], 0
; CHECK-NEXT:    br i1 [[C_1]], label [[MATH:%.*]], label [[EXIT_FAIL:%.*]]
; CHECK:       math:
; CHECK-NEXT:    [[OP:%.*]] = tail call { i8, i1 } @llvm.ssub.with.overflow.i8(i8 [[A:%.*]], i8 [[B]])
; CHECK-NEXT:    [[STATUS:%.*]] = extractvalue { i8, i1 } [[OP]], 1
; CHECK-NEXT:    br i1 [[STATUS]], label [[EXIT_FAIL]], label [[EXIT_OK:%.*]]
; CHECK:       exit.ok:
; CHECK-NEXT:    [[RES:%.*]] = extractvalue { i8, i1 } [[OP]], 0
