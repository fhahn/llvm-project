...
; Mirror of @postinc_signed_facts_when_unsigned_overflows: here the signed sum
; StartValue + Step (127 + 1) overflows signed but not unsigned, so the signed
; facts must be suppressed (deriving PN s>= 127 or PN s< -100 would be wrong,
; e.g. at %iv == -128) while the unsigned fact PN u< -100 is still derived.
