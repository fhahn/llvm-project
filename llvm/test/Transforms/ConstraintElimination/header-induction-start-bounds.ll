...
; Checked variant of @relational_exit_cond: no no-wrap flags, so SCEV is used.
...
; The step of a checked add is its constant operand, as for a plain add.
...
; Only sadd.with.overflow is matched, not uadd.with.overflow.
define void @checked_uadd_must_not_fold(i32 %n) {
; CHECK-LABEL: define void @checked_uadd_must_not_fold(
; CHECK-SAME: i32 [[N:%.*]]) {
; CHECK-NEXT:  [[ENTRY:.*]]:
; CHECK-NEXT:    br label %[[LOOP_HEADER:.*]]
; CHECK:       [[LOOP_HEADER]]:
