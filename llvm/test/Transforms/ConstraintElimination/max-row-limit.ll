...
; RUN: opt -passes=constraint-elimination -constraint-elimination-max-rows=14 -S %s | FileCheck --check-prefixes=COMMON,SIMP %s
; RUN: opt -passes=constraint-elimination -constraint-elimination-max-rows=13 -S %s | FileCheck --check-prefixes=COMMON,NOSIMP %s
