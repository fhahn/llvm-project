...
; CHECK-NEXT:    [[TMP0:%.*]] = add i16 [[START]], 1
; CHECK-NEXT:    [[TMP1:%.*]] = tail call i16 @llvm.smax.i16(i16 [[N]], i16 [[TMP0]])
; CHECK-NEXT:    [[TMP2:%.*]] = xor i16 [[START]], -1
; CHECK-NEXT:    [[TMP3:%.*]] = add i16 [[TMP1]], [[TMP2]]
; CHECK-NEXT:    [[TMP4:%.*]] = zext i16 [[TMP3]] to i32
; CHECK-NEXT:    [[TMP5:%.*]] = add nuw nsw i32 [[TMP4]], 1
; CHECK-NEXT:    [[MIN_ITERS_CHECK:%.*]] = icmp ult i16 [[TMP3]], 15
; CHECK-NEXT:    br i1 [[MIN_ITERS_CHECK]], label %[[LOOP_PREHEADER1:.*]], label %[[VECTOR_SCEVCHECK:.*]]
; CHECK:       [[VECTOR_SCEVCHECK]]:
; CHECK-NEXT:    [[TMP6:%.*]] = add i16 [[START]], 1
; CHECK-NEXT:    [[SMAX:%.*]] = tail call i16 @llvm.smax.i16(i16 [[N]], i16 [[TMP6]])
; CHECK-NEXT:    [[TMP7:%.*]] = xor i16 [[START]], -1
; CHECK-NEXT:    [[TMP8:%.*]] = add i16 [[SMAX]], [[TMP7]]
; CHECK-NEXT:    [[TMP9:%.*]] = sext i16 [[V]] to i32
; CHECK-NEXT:    [[TMP10:%.*]] = zext i16 [[TMP8]] to i32
; CHECK-NEXT:    [[TMP11:%.*]] = xor i16 [[START]], -1
; CHECK-NEXT:    [[TMP12:%.*]] = sext i16 [[TMP11]] to i32
; CHECK-NEXT:    [[TMP13:%.*]] = add nsw i32 [[TMP12]], [[TMP9]]
; CHECK-NEXT:    [[TMP14:%.*]] = icmp ult i32 [[TMP13]], [[TMP10]]
; CHECK-NEXT:    br i1 [[TMP14]], label %[[LOOP_PREHEADER1]], label %[[VECTOR_PH:.*]]
; CHECK:       [[VECTOR_PH]]:
; CHECK-NEXT:    [[N_VEC:%.*]] = and i32 [[TMP5]], 131068
; CHECK-NEXT:    [[TMP15:%.*]] = trunc i32 [[N_VEC]] to i16
; CHECK-NEXT:    [[TMP16:%.*]] = add i16 [[START]], [[TMP15]]
; CHECK-NEXT:    [[INVARIANT_OP:%.*]] = sub i16 [[START]], [[V]]
; CHECK-NEXT:    br label %[[VECTOR_BODY:.*]]
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[INDEX:%.*]] = phi i32 [ 0, %[[VECTOR_PH]] ], [ [[INDEX_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-NEXT:    [[TMP17:%.*]] = trunc i32 [[INDEX]] to i16
; CHECK-NEXT:    [[DOTREASS:%.*]] = add i16 [[TMP17]], [[INVARIANT_OP]]
; CHECK-NEXT:    [[TMP18:%.*]] = sext i16 [[DOTREASS]] to i64
; CHECK-NEXT:    [[TMP19:%.*]] = and i64 [[TMP18]], 4294967295
; CHECK-NEXT:    [[TMP20:%.*]] = getelementptr inbounds nuw i8, ptr [[A]], i64 [[TMP19]]
; CHECK-NEXT:    store <4 x i8> splat (i8 1), ptr [[TMP20]], align 1
; CHECK-NEXT:    [[INDEX_NEXT]] = add nuw i32 [[INDEX]], 4
; CHECK-NEXT:    [[TMP21:%.*]] = icmp eq i32 [[INDEX_NEXT]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[TMP21]], label %[[MIDDLE_BLOCK:.*]], label %[[VECTOR_BODY]], !llvm.loop [[LOOP0:![0-9]+]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[CMP_N:%.*]] = icmp eq i32 [[TMP5]], [[N_VEC]]
; CHECK-NEXT:    br i1 [[CMP_N]], label %[[EXIT]], label %[[LOOP_PREHEADER1]]
; CHECK:       [[LOOP_PREHEADER1]]:
; CHECK-NEXT:    [[IV_PH:%.*]] = phi i16 [ [[START]], %[[VECTOR_SCEVCHECK]] ], [ [[START]], %[[LOOP_PREHEADER]] ], [ [[TMP16]], %[[MIDDLE_BLOCK]] ]
; CHECK-NEXT:    br label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i16 [ [[IV_NEXT:%.*]], %[[LOOP]] ], [ [[IV_PH]], %[[LOOP_PREHEADER1]] ]
; CHECK-NEXT:    [[OFF:%.*]] = sub nsw i16 [[IV]], [[V]]
...
; CHECK-NEXT:    [[EC:%.*]] = icmp slt i16 [[IV_NEXT]], [[N]]
...
; CHECK-NEXT:    br i1 [[PRE]], label %[[EXIT:.*]], label %[[LOOP:.*]]
; CHECK:       [[LOOP]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i32 [ [[IV_NEXT:%.*]], %[[LOOP]] ], [ [[START]], %[[ENTRY]] ]
; CHECK-NEXT:    [[OFF:%.*]] = sub nsw i32 [[IV]], [[V]]
; CHECK-NEXT:    [[TMP0:%.*]] = sext i32 [[OFF]] to i64
; CHECK-NEXT:    [[GEP:%.*]] = getelementptr inbounds [4 x i8], ptr [[A]], i64 [[TMP0]]
...
loop:
  ...
  %gep = getelementptr inbounds i32, ptr %A, i32 %off
