...
; CE does derive nuw for the shl from the bound on the decrementing induction:
; the systems bound it by the lshr start value, which in turn is bounded by
; 2^31 - 1. IndVars then widens the induction to i64, keeping the flags on the
; wide `shl nuw nsw i64` and its `trunc nuw`, and InstCombine now preserves nuw
; when narrowing `trunc(shl X, C)` back to `shl(trunc X), C`.
...
; CHECK-NEXT:    [[INDVARS_IV_TR:%.*]] = trunc nuw i64 [[EXT]] to i32
; CHECK-NEXT:    [[SHL:%.*]] = shl nuw i32 [[INDVARS_IV_TR]], 1
