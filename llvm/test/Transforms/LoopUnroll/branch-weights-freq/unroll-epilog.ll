...
; UR10-EUR: - do.body.epil: float = 0.93725,
; UR10-EUR: - do.body.epil.1: float = 0.82639,
; UR10-EUR: - do.body.epil.2: float = 0.63739,
; UR10-EUR: - do.body.epil.3: float = 0.49161,
; UR10-EUR: - do.body.epil.4: float = 0.37917,
; UR10-EUR: - do.body.epil.5: float = 0.29245,
; UR10-EUR: - do.body.epil.6: float = 0.22557,
; UR10-EUR: - do.body.epil.7: float = 0.17398,
; UR10-EUR: - do.body.epil.8: float = 0.13419,
;
; Unrolled loop guard, body, and latch.
...
; UR10-EUR: call void @f
; UR10-EUR: br i1 %{{.*}}, label %do.body.epil.2, label %do.end.epilog-lcssa, !prof !7
; UR10-EUR: call void @f
; UR10-EUR: br i1 %{{.*}}, label %do.body.epil.3, label %do.end.epilog-lcssa, !prof !7
; UR10-EUR: call void @f
; UR10-EUR: br i1 %{{.*}}, label %do.body.epil.4, label %do.end.epilog-lcssa, !prof !7
; UR10-EUR: call void @f
; UR10-EUR: br i1 %{{.*}}, label %do.body.epil.5, label %do.end.epilog-lcssa, !prof !7
; UR10-EUR: call void @f
; UR10-EUR: br i1 %{{.*}}, label %do.body.epil.6, label %do.end.epilog-lcssa, !prof !7
; UR10-EUR: call void @f
; UR10-EUR: br i1 %{{.*}}, label %do.body.epil.7, label %do.end.epilog-lcssa, !prof !7
; UR10-EUR: call void @f
; UR10-EUR: br i1 %{{.*}}, label %do.body.epil.8, label %do.end.epilog-lcssa, !prof !7
; UR10-EUR: call void @f
;
; Unrolled loop metadata.
...
;
; Non-unrolled epilogue loop metadata.
; UR10-ELP: !6 = !{!"branch_weights", i32 1656332913, i32 491150735}
; UR10-ELP: !7 = distinct !{!7, !3, !4}
;
; Completely unrolled epilogue loop metadata: as for -unroll-count=8, it loses
; its backedge, so the first latch's weights are adjusted and the rest keep the
; non-unrolled epilogue weights, and it has no llvm.loop.estimated_trip_count.
; UR10-EUR: !6 = !{!"branch_weights", i32 1893470673, i32 254012975}
; UR10-EUR: !7 = !{!"branch_weights", i32 1656332913, i32 491150735}
...
; UR11-EUR: - do.body.epil: float = 0.94604,
; UR11-EUR: - do.body.epil.1: float = 0.84529,
; UR11-EUR: - do.body.epil.2: float = 0.66641,
; UR11-EUR: - do.body.epil.3: float = 0.52539,
; UR11-EUR: - do.body.epil.4: float = 0.4142,
; UR11-EUR: - do.body.epil.5: float = 0.32655,
; UR11-EUR: - do.body.epil.6: float = 0.25745,
; UR11-EUR: - do.body.epil.7: float = 0.20297,
; UR11-EUR: - do.body.epil.8: float = 0.16001,
; UR11-EUR: - do.body.epil.9: float = 0.12615,
;
; Unrolled loop guard, body, and latch.
...
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.2, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.3, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.4, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.5, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.6, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.7, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.8, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
; UR11-EUR: br i1 %{{.*}}, label %do.body.epil.9, label %do.end.epilog-lcssa, !prof !7
; UR11-EUR: call void @f
;
; Unrolled loop metadata.
...
;
; Non-unrolled epilogue loop metadata.  It has an llvm.loop.estimated_trip_count
; different than the unrolled loop's, so it has its own metadata node.
; UR11-ELP: !6 = !{!"branch_weights", i32 1693034047, i32 454449601}
; UR11-ELP: !7 = distinct !{!7, !8, !4}
; UR11-ELP: !8 = !{!"llvm.loop.estimated_trip_count", i32 0}
;
; Completely unrolled epilogue loop metadata: as for -unroll-count=8, it loses
; its backedge, so the first latch's weights are adjusted and the rest keep the
; non-unrolled epilogue weights, and it has no llvm.loop.estimated_trip_count.
; UR11-EUR: !6 = !{!"branch_weights", i32 1918796287, i32 228687361}
; UR11-EUR: !7 = !{!"branch_weights", i32 1693034047, i32 454449601}
...
; UR12-EUR: - do.body.epil: float = 0.95324,
; UR12-EUR: - do.body.epil.1: float = 0.86034,
; UR12-EUR: - do.body.epil.2: float = 0.69045,
; UR12-EUR: - do.body.epil.3: float = 0.55411,
; UR12-EUR: - do.body.epil.4: float = 0.44469,
; UR12-EUR: - do.body.epil.5: float = 0.35687,
; UR12-EUR: - do.body.epil.6: float = 0.2864,
; UR12-EUR: - do.body.epil.7: float = 0.22985,
; UR12-EUR: - do.body.epil.8: float = 0.18446,
; UR12-EUR: - do.body.epil.9: float = 0.14803,
; UR12-EUR: - do.body.epil.10: float = 0.1188,
;
; Unrolled loop guard, body, and latch.
...
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.2, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.3, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.4, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.5, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.6, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.7, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.8, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.9, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
; UR12-EUR: br i1 %{{.*}}, label %do.body.epil.10, label %do.end.epilog-lcssa, !prof !7
; UR12-EUR: call void @f
;
; Unrolled loop metadata.
...
;
; Non-unrolled epilogue loop metadata.  Its llvm.loop.estimated_trip_count
; differs from the unrolled loop's, so they have distinct metadata nodes.
; UR12-ELP: !6 = !{!"branch_weights", i32 1723419551, i32 424064097}
...
; its backedge, so the first latch's weights are adjusted and the rest keep the
; non-unrolled epilogue weights, and it has no llvm.loop.estimated_trip_count.
; UR12-EUR: !6 = !{!"branch_weights", i32 1938208533, i32 209275115}
; UR12-EUR: !7 = !{!"branch_weights", i32 1723419551, i32 424064097}
