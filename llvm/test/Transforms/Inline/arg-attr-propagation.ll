...
; The callee promises the argument is dereferenceable(32); inlining preserves
; that as an llvm.assume operand bundle on the caller value (see
; AddDereferenceableAssumptions in InlineFunction.cpp). The noundef on the other
; argument is only retained with --enable-knowledge-retention.

define i32 @caller1(ptr %t1, i32 %t2) {
; NO_ASSUME-LABEL: define {{[^@]+}}@caller1
; NO_ASSUME-SAME: (ptr [[T1:%.*]], i32 [[T2:%.*]]) {
; NO_ASSUME-NEXT:    call void @llvm.assume(i1 true) [ "dereferenceable"(ptr [[T1]], i64 32) ]
...
; CHECK-SAME: (ptr dereferenceable(31) [[T1:%.*]]) {
; CHECK-NEXT:    call void @llvm.assume(i1 true) [ "dereferenceable"(ptr [[T1]], i64 32) ]
