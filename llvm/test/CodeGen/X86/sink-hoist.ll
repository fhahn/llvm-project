...
; CHECK: movsd
; CHECK: align
define void @bar(ptr nocapture %p, i64 %n) nounwind {
entry:
  %0 = icmp sgt i64 %n, 0
...
bb:
  ...
  %scevgep = getelementptr double, ptr %p, i64 %i.03
  %1 = load double, ptr %scevgep, align 8
  %2 = fdiv double 3.200000e+00, %1
  store double %2, ptr %scevgep, align 8
...
@_minusZero.6007 = internal constant <4 x float> <float -0.000000e+00, float -0.000000e+00, float -0.000000e+00, float -0.000000e+00> ; <ptr> [#uses=0]
@twoTo23.6008 = internal constant <4 x float> <float 8.388608e+06, float 8.388608e+06, float 8.388608e+06, float 8.388608e+06> ; <ptr> [#uses=0]

define void @vv(ptr %y, ptr %x, ptr %n) nounwind ssp {
entry:
  br label %bb60

bb:                                               ; preds = %bb60
  %i.0 = phi i32 [ 0, %bb60 ]                    ; <i32> [#uses=2]
  %0 = load <4 x float>, ptr %x_addr.0, align 16             ; <<4 x float>> [#uses=4]
  %tmp20 = bitcast <4 x float> %0 to <4 x i32>    ; <<4 x i32>> [#uses=1]
  ...
  %tmp25 = bitcast <4 x float> %0 to <4 x i32>    ; <<4 x i32>> [#uses=1]
  ...
  %tmp45 = fadd <4 x float> %0, %tmp43            ; <<4 x float>> [#uses=1]
  %tmp47 = fsub <4 x float> %tmp45, %tmp43        ; <<4 x float>> [#uses=2]
  %tmp49 = call <4 x float> @llvm.x86.sse.cmp.ps(<4 x float> %0, <4 x float> %tmp47, i8 1) ; <<4 x float>> [#uses=1]
  %1 = bitcast <4 x float> %tmp49 to <4 x i32>    ; <<4 x i32>> [#uses=1]
  %2 = call <4 x float> @llvm.x86.sse2.cvtdq2ps(<4 x i32> %1) nounwind readnone ; <<4 x float>> [#uses=1]
  %tmp53 = fadd <4 x float> %tmp47, %2            ; <<4 x float>> [#uses=1]
  ...
  store <4 x float> %tmp58, ptr %y_addr.0, align 16
  %3 = getelementptr float, ptr %x_addr.0, i64 4      ; <ptr> [#uses=1]
  %4 = getelementptr float, ptr %y_addr.0, i64 4      ; <ptr> [#uses=1]
  %5 = add i32 %i.0, 4                            ; <i32> [#uses=1]
  %6 = load i32, ptr %n, align 4                      ; <i32> [#uses=1]
  %7 = icmp sgt i32 %6, %5                        ; <i1> [#uses=1]
  br i1 %7, label %bb60, label %return

bb60:                                             ; preds = %bb, %entry
  %x_addr.0 = phi ptr [ %x, %entry ], [ %3, %bb ] ; <ptr> [#uses=2]
  %y_addr.0 = phi ptr [ %y, %entry ], [ %4, %bb ] ; <ptr> [#uses=2]
  br label %bb
...
@cl_options_count = external dso_local constant i32         ; <ptr> [#uses=2]

define void @default_get_pch_validity() nounwind {
entry:
  %tmp4 = load i32, ptr @cl_options_count, align 4    ; <i32> [#uses=1]
