...
; More than one index, so this goes the general route.
...
; The index is not loop-invariant, so this goes the general route.
...
; The getelementptr does not use the PHI as its base, so the PHI is not an
; induction variable.
