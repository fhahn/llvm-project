...
; CHECK-NEXT:    --> {1,+,1}<nuw><%outer> U: [1,0) S: [1,0) Exits: (1 + %outer_l)<nuw> LoopDispositions: { %outer: Computable, %inner: Invariant }
...
; CHECK-NEXT:    --> {1,+,1}<nuw><%inner> U: [1,0) S: [1,0) Exits: (1 + %inner_l)<nuw> LoopDispositions: { %inner: Computable, %outer: Uniform }
