...
; CHECK-NEXT:    --> (-83 %u {1,+,1}<nuw><nsw><%loop>) U: [0,1) S: [0,1) Exits: 0 LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %zext = zext i8 %urem to i32
; CHECK-NEXT:    --> (173 %u (zext i8 {1,+,1}<nuw><nsw><%loop> to i32)) U: [0,1) S: [0,1) Exits: 0 LoopDispositions: { %loop: Computable }
; CHECK-NEXT:    %sub = sub i32 0, %zext
; CHECK-NEXT:    --> (-1 * (173 %u (zext i8 {1,+,1}<nuw><nsw><%loop> to i32)))<nuw><nsw> U: [0,1) S: [0,1) Exits: 0 LoopDispositions: { %loop: Computable }
