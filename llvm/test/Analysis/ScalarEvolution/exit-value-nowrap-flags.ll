...
; The %up recurrence has nuw, so the closed form of its exit value does not
; unsigned-wrap. The flag is attached to the use, and stays attached where that
; use becomes the start operand of the %down recurrence.
...
; Negative test: without a no-wrap flag on the recurrence there is nothing to
; carry over, so neither the exit value nor the %down recurrence's start operand
; gets one.
...
; The signed half of the new condition: with nsw and both %start and %step known
; non-negative, Step * It cannot leave the signed range, so the closed form keeps
; nsw.
...
; Negative test: %start is non-negative and %step is non-positive, so Step * It is
; not bounded by the recurrence's own range and nsw must not carry over. For
; example i8 {-100,+,50} is nsw for four iterations while 4 * 50 = 200 is not
; representable.
...
; The iteration count also has to be non-negative as a signed value of the
; recurrence's type before nsw can go on the multiply. On the non-positive arm the
; recurrence's own nsw only bounds the count by 2^(W-1), and at exactly that count
; BinomialCoefficient yields SMIN, so Step * SMIN leaves the signed range even
; though the recurrence never wrapped. Here the i8 backedge-taken count is 128,
; i.e. SMIN, and (-128 * %s) must not be marked nsw: for %s == -1 the loop is
; poison-free and the exit value is -128, while the signed product 128 does not
; fit in i8.
