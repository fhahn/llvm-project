...
const SCEV *ScalarEvolution::computeMaxBECountForLT(const SCEV *Start,
  ...
  // ~Start and ~End, while Stride is the negated step and the minimum and
  // maximum are complements of each other (~SMIN == SMAX).
  ...
  MaxEnd = IsSigned ? APIntOps::smin(MaxEnd, Limit)
                    : APIntOps::umin(MaxEnd, Limit);
...
ScalarEvolution::howManyLessThans(const SCEV *LHS, const SCEV *RHS,
  ...
  if (!isLoopInvariant(RHS, L)) {
    ...
    // Translate "~A - k" and "~B - ~A" into terms of A and B; see the note on
    // Invert at the top of this function.
    ...
    if ((!AddingStrideMinusOneMayOverflow ||
      ...
      // our preconditions. Inverted, the proof applies to ~Start and ~RHS.
  ...
  if (isa<SCEVConstant>(BECount)) {
    ...
    ConstantMaxBECount = computeMaxBECountForLT(
        Start, Stride, RHS, getTypeSizeInBits(LHS->getType()), IsSigned, Invert);
  }
