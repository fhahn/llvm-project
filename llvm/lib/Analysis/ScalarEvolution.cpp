...
std::optional<ScalarEvolution::SimpleAffineStep>
ScalarEvolution::matchSimpleAffineStep(const Loop *L, PHINode *PN,
                                       Value *BEValueV) {
  if (auto BO = MatchBinaryOp(BEValueV, getDataLayout(), AC, DT, PN)) {
    ...
    return SimpleAffineStep{StepV, /*GEP=*/nullptr, Flags};
  }
...
const SCEV *ScalarEvolution::createSimpleAffineAddRec(PHINode *PN,
  ...
  std::optional<SimpleAffineStep> Step =
      matchSimpleAffineStep(L, PN, BEValueV);
  if (!Step)
    return nullptr;

  const SCEV *Accum = getSCEV(Step->StepV);
  SCEV::NoWrapFlags Flags = Step->Flags;
  if (GEPOperator *GEP = Step->GEP) {
    // The step is the index scaled by the source element size. Getelementptr
    // indices are signed.
...
const SCEV *ScalarEvolution::finishAddRecFromPHI(PHINode *PN) {
  ...
  // The symbolic name must already be in place; don't create one here, as the
  // SCEVs computed below are only valid with respect to it.
  const SCEV *SymbolicName = getExistingSCEV(PN);
  assert(SymbolicName && SymbolicName == getUnknown(PN) &&
...
const SCEV *ScalarEvolution::createSCEVIter(Value *V) {
  ...
  using StackEntry = PointerIntPair<Value *, 2, State>;
  SmallVector<StackEntry> Stack;
