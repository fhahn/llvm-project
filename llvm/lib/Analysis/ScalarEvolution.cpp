...
const SCEV *ScalarEvolution::createSimpleAffineAddRec(PHINode *PN,
  ...
  if (auto BO = MatchBinaryOp(BEValueV, getDataLayout(), AC, DT, PN)) {
    ...
    // The getelementptr adds the same offset on every iteration, so this is an
    // affine add recurrence stepping by that offset. Only a single index is
    // handled; anything else takes the general route.
    ...
    // The step is the index scaled by the source element size, as in
    // getGEPExpr. Getelementptr indices are signed.
    ...
    // Derive the nowrap flags with the same rules createAddRecFromPHI uses for
    // this shape. Note that isSCEVExprNeverPoison cannot be used here: it walks
    // the operands of the increment, which recurses back into PN before it has
    // a SCEV.
    GEPNoWrapFlags NW = GEP->getNoWrapFlags();
