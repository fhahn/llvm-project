...
/// Exact mul nuw region for single element RHS. Always inlined, as
/// makeNoWrapRegion() below gives it two call sites.
...
namespace {
/// Stands in for the single-element range holding \p C: all its bounds are C.
...
/// SingleElementBounds. Templated and always inlined so that each caller only
/// materializes the bounds it asks for, folded to the constant in the latter
/// case.
...
ConstantRange ConstantRange::makeExactNoWrapRegion(Instruction::BinaryOps BinOp,
                                                   const APInt &Other,
                                                   unsigned NoWrapKind) {
  // The region is exact for single-element ranges, as "for all" and "for any"
