...
public:
  ...
  void optimizeVectorOps(ArrayRef<Instruction *> MatrixOps,
    ...
    for (Instruction *I : VectorOps) {
      ...
      while (!Worklist.empty()) {
        ...
        } else if (match(Curr, m_Intrinsic<Intrinsic::matrix_transpose>())) {
          NotSupported = true;
        } else if (CI) {
      ...
      for (Value *I : ToFlatten) {
        if (I->getType()->isVoidTy())
          continue;
        Value *Op;
        if (match(I, m_Intrinsic<Intrinsic::matrix_transpose>(m_Value(Op)))) {
