...
/// Returns true if \p Opcode applied to \p Op0 and \p Op1 with \p
/// NoWrapFlags is known to not wrap in signed or unsigned, depending on \p
/// Signed.
...
static Decomposition decompose(Value *V, const ConstraintInfo &Info,
  ...
  if (match(V, m_AddLike(m_Value(Op0), m_Value(Op1)))) {
    // In the unsigned system, `%x + -C` has two possible decompositions: the
    // exact one, if the add does not wrap unsigned, and `%x - C`, if the
    // subtraction does not wrap. A single query decides which applies, as
    // `%x >=u C` is exactly the negation of the region in which the add does
    // not wrap unsigned. Note that the constant has to be decomposed as
    // signed for the subtraction, to get the negative offset.
    if (!IsSigned && match(Op1, m_ConstantInt(CI)) && CI->isNegative() &&
        canUseSExt(CI) && !match(V, m_NUWAddLike(m_Value(), m_Value()))) {
      std::optional<bool> AddWraps = Info.evaluate(
          CmpInst::ICMP_UGE, Op0,
          ConstantInt::get(Op0->getType(), -CI->getSExtValue()));
      if (AddWraps)
        if (auto Decomp = MergeResults(Op0, CI, /*IsSignedB=*/*AddWraps))
          return *Decomp;
      return V;
    }

    // The decomposition is exact if the add does not wrap.
...
static FlagsToStrengthen getFlagsToStrengthen(Instruction *I) {
  // There is no rule to infer nsw for a trunc.
  ...
  case Instruction::Sub:
    ...
    // 0 <=s B <=s A or A <=s B <=s 0. Both rules apply to any operands, so
    // either flag can be strengthened as long as it is not set yet.
    return {/*NSW=*/!BO->hasNoSignedWrap(), /*NUW=*/!BO->hasNoUnsignedWrap()};
...
void State::addInfoFor(BasicBlock &BB) {
  ...
  for (Instruction &I : BB) {
    ...
    // Enqueue the overflow and saturating intrinsics for simplification.
...
/// Try to replace \p SI by the plain binary operation it performs, if \p Info
/// proves that it cannot saturate. Returns true if \p SI was replaced.
static bool
checkAndReplaceSaturating(SaturatingInst *SI, ConstraintInfo &Info,
                          SmallVectorImpl<Instruction *> &ToRemove) {
  // A saturating intrinsic saturates exactly when the plain operation wraps.
  Instruction::BinaryOps Opcode = SI->getBinaryOp();
  Value *A = SI->getLHS(), *B = SI->getRHS();
  bool Signed = SI->isSigned();
  if (!isKnownNoWrap(Opcode, A, B, /*NoWrapFlags=*/0, Info, Signed))
    return false;

  // The no-wrap flag for the other signedness may hold as well. It is worth
  // the extra queries here, as the intrinsic is known to be simplified
  // already.
  bool HasNSW = Signed || isKnownNoWrap(Opcode, A, B, /*NoWrapFlags=*/0, Info,
                                        /*Signed=*/true);
  bool HasNUW =
      !Signed ||
      isKnownNoWrap(Opcode, A, B,
                    HasNSW ? OverflowingBinaryOperator::NoSignedWrap : 0, Info,
                    /*Signed=*/false);
