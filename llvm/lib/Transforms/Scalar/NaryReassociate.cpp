...
NaryReassociatePass::tryReassociateGEPAtIndex(GetElementPtrInst *GEP,
  ...
  SCEVUse CandidateExpr = SE->getGEPExpr(cast<GEPOperator>(GEP),
                                             IndexExprs);
...
SCEVUse NaryReassociatePass::getBinarySCEV(BinaryOperator *I,
                                               SCEVUse LHS,
                                               SCEVUse RHS) {
  switch (I->getOpcode()) {
  case Instruction::Add:
...
Value *NaryReassociatePass::tryReassociateMinOrMax(Instruction *I,
  ...
  auto tryCombination = [&](Value *A, SCEVUse AExpr, Value *B,
                            SCEVUse BExpr, Value *C,
                            SCEVUse CExpr) -> Value * {
    ...
    SmallVector<SCEVUse, 2> Ops2{SE->getUnknown(C),
