...
Value *SCEVExpander::visitAddExpr(SCEVUseT<const SCEVAddExpr *> S) {
  ...
  for (auto I = OpsAndLoops.begin(), E = OpsAndLoops.end(); I != E;) {
    ...
    if (isa<PointerType>(Sum->getType())) {
      ...
      // A single operand is already the offset; going through getAddExpr would
      // return it as a bare SCEV and drop its use-specific flags.
      SCEVUse Offset =
          NewOps.size() == 1 ? NewOps[0] : SCEVUse(SE.getAddExpr(NewOps));
      Sum = expandAddToGEP(Offset, Sum, S.getNoWrapFlags());
