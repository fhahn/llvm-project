...
VPIRAttributes::VPIRAttributes(CallInst &CI, const Function &Variant) {
  ...
  auto CombineAttrs = [&](Attribute CallAttr, Attribute VariantAttr) -> Attribute {
    ...
    case Attribute::NoFPClass:
      return Attribute::getWithNoFPClass(
          Ctx, CallAttr.getNoFPClass() | VariantAttr.getNoFPClass());
  ...
  for (unsigned I = 0, E = std::min<unsigned>(CI.arg_size(),
    ...
    if (!PA.hasAttributes())
      continue;
    ArgAttrs[I] = Filter(VariantTy->getParamType(I), PA,
                         VariantAttrs.getParamAttrs(I));
  }
