...
static VPSpeculativeLoadOracleRecipe *buildOraclePlan(VPlan &Plan,
  ...
  // The plan stays in plain-CFG form; VPSpeculativeLoadOracleRecipe::execute
  ...
  // Rebase to the global lane index. The canonical IV does not exist yet; use
  // the plan's unused vector-trip-count symbol, mapped to an arg in execute().
  ...
  // A recipe-defined trip count now lives in the unreachable original entry;
  // reset it so Plan::duplicate() can map it.
...
bool VPlanTransforms::replaceUnsafeLoadsWithSpeculative(
  ...
  // an element type occupying exactly EltStoreSize bytes, a power of 2.
  ...
  // Replace the loads with speculative_load(ptr, from_end=false, @oracle,
  // args...). Lanes past the early exit are poison; freeze them to keep it out
  // of the early-exit lane index (cttz.elts of an unfrozen cmp is poison).
...
void VPlanTransforms::materializeSpeculativeLoadOracleCanonicalIV(VPlan &Plan) {
  ...
  // The oracle and its speculative loads were created before the canonical IV
  // existed and use the plan's vector trip count as a placeholder for it.
...
void VPlanTransforms::attachSpeculativeLoadChecks(
  ...
  for (VPUser *U : Oracle->users()) {
    ...
    // base: @llvm.can.load.speculatively only certifies addresses of the form
    // `start + I * num_bytes`, so an offset between the two (e.g. `A[i+1]`,
    // based on `%A` but starting at `%A+1`) would slip through its check.
