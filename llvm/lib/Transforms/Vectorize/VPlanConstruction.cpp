...
/// Clone \p Plan into a speculative-load oracle plan: a scalar loop (step 1,
/// trip count VF) replaying the exit conditions of the loop with header \p
/// OrigHeader lane-by-lane. The oracle returns the number of valid *bytes* from
/// the start of the loaded vector, i.e. the number of safe lanes times \p
/// EltStoreSize, the per-lane store size shared by all speculative loads. The
/// oracle's live-in operands are added to \p Plan. Returns nullptr if the loop
/// body cannot be replayed.
static VPSpeculativeLoadOracleRecipe *
buildOraclePlan(VPlan &Plan, VPBasicBlock *OrigHeader, uint64_t EltStoreSize) {
  // Bail out before cloning, to avoid a wasted Plan::duplicate(). Integer
  // div/rem is unsafe: a speculative load's trailing lanes are frozen poison,
  // which may be zero, so a widened div/rem could divide by zero in a lane the
  // loop never executed. The other opcodes have no scalar code-gen in
  // VPInstruction::execute, and pointer inductions or steps defined outside the
  // replayed body cannot be rebuilt from the oracle's own IV.
  ...
  if (any_of(vp_rpo_plain_cfg_loop_body(OrigHeader),
  ...
  // The oracle plan stays in plain-CFG form, as it is code-generated by RPOT
  // traversal in VPSpeculativeLoadOracleRecipe::execute; it needs no loop
  // regions or canonical IV. Build the scalar IV (phi 0, step 1) and
  // BranchOnCount latch (trip count VF) directly on the plain CFG.
  auto [HeaderVPBB, LatchVPBB] =
      VPBlockUtils::getPlainCFGHeaderAndLatch(*OraclePlan);
  SmallVector<VPBasicBlock *> BodyVPBBs =
      vp_rpo_plain_cfg_loop_body(HeaderVPBB);
  ...
  VPValue *LatchExitCond = LatchTerm->getOperand(0);
  LatchTerm->eraseFromParent();

  // Make the header the oracle plan's entry's single successor. The original
  // entry and the whole vector skeleton become unreachable and are not cloned.
  ...
  // canonIV. The outer canonical IV does not exist yet, so use the oracle
  // plan's unused vector-trip-count symbol, which execute() maps to an oracle
  // argument.
  VPBuilder Builder(HeaderVPBB, HeaderVPBB->getFirstNonPhi());
  auto *AdjustedIV =
      Builder.createAdd(ScalarIVPHI, &OraclePlan->getVectorTripCount(),
                        DebugLoc::getUnknown(), "adjusted.iv");

  // Replace wide inductions with scalar equivalents based on adjusted.iv.
  for (VPRecipeBase &R : make_early_inc_range(HeaderVPBB->phis())) {
  ...
  // Replicate Load/GEP/Freeze recipes, so the underlying scalar IR instruction
  // is cloned during oracle execution. Freeze must be replicated too: as a
  // VPInstruction it would hit the epilogue-only underlying-value side-channel
  // in VPInstruction::execute and assert.
  for (VPBasicBlock *VPBB : BodyVPBBs)
    for (VPRecipeBase &R : make_early_inc_range(*VPBB)) {
      ...
      auto *Replicate = VPBuilder::createSingleScalarOp(
          Opc, to_vector(VPI->operands()), /*Mask=*/nullptr, *VPI, *VPI,
  ...
  // block) to a single new exit block computing the return value.
  ...
  for (auto [Pred, EarlyExitVPBB] :
    ...
    VPBlockUtils::connectBlocks(Pred, ExitVPBB, /*PredIdx=*/-1u,
                                Pred->getIndexForSuccessor(EarlyExitVPBB));
  VPBlockUtils::disconnectBlocks(LatchVPBB, MiddleVPBB);
  VPBlockUtils::connectBlocks(LatchVPBB, ExitVPBB);
  LatchVPBB->swapSuccessors();
  ...
  // A recipe-defined trip count is unreachable now, reset it so
  // Plan::duplicate() can map it. This also drops the dead latch condition's use
  // of it, keeping the deletion below from freeing it.
  if (VPValue *TC = OraclePlan->getTripCount(); TC->getDefiningRecipe()) {
    TC->replaceAllUsesWith(Zero);
    OraclePlan->resetTripCount(Zero);
  }
  // Delete the dead latch condition, so the values it used (like a live-in trip
  // count) do not become spurious oracle parameters.
  vputils::recursivelyDeleteDeadRecipes(LatchExitCond);

  VPlanTransforms::convertToConcreteRecipes(*OraclePlan);

  // Collect the operands passed to the oracle function: the lane-0 canonical IV
  // (still a placeholder, see above), followed by the live-ins of the
  // code-generated blocks. Constants are materialized directly in the oracle.
  ...
  for (VPBasicBlock *VPBB : BodyVPBBs)
    for (VPRecipeBase &R : *VPBB)
      for (VPValue *Op : R.operands())
        if (auto *IRV = dyn_cast<VPIRValue>(Op); IRV && !isa<VPConstant>(IRV))
          Operands.insert(Plan.getOrAddLiveIn(IRV->getValue()));
...
bool VPlanTransforms::replaceUnsafeLoadsWithSpeculative(
  ...
  // Collect the loads that are not known to be dereferenceable.
  ...
  // The oracle returns a single byte count shared by all speculative loads,
  // assuming lane K occupies bytes [K * EltStoreSize, (K + 1) * EltStoreSize)
  // of the loaded vector. All loads must hence have the same element type, with
  // a power-of-2 store size matching its type size, as @llvm.speculative.load
  // requires. Pointers (primitive size 0) have no fixed layout and are
  // excluded.
  Type *EltTy = UnsafeLoads.front()->getScalarType();
  uint64_t EltStoreSize = DL.getTypeStoreSize(EltTy).getFixedValue();
  if (EltTy->getPrimitiveSizeInBits().getKnownMinValue() == 0 ||
      !DL.typeSizeEqualsStoreSize(EltTy) || !isPowerOf2_64(EltStoreSize) ||
      any_of(UnsafeLoads, [EltTy](VPInstruction *VPI) {
        return VPI->getScalarType() != EltTy;
      })) {
    LLVM_DEBUG(dbgs() << "LV: Not vectorizing: unsupported element type for "
                         "speculative loads\n");
    return false;
  }

  // Only a single early exit is supported, and all speculative loads must
  // execute unconditionally before it.
  auto EarlyExits =
      vputils::getEarlyExits(Plan, VPBlockUtils::getPlainCFGMiddleBlock(Plan));
  VPDominatorTree VPDT(Plan);
  if (EarlyExits.size() != 1 ||
      any_of(UnsafeLoads, [&](VPInstruction *VPI) {
        return VPDT.properlyDominates(EarlyExits[0].first, VPI->getParent());
      })) {
    LLVM_DEBUG(dbgs() << "LV: Not vectorizing: multiple early exits or "
                         "non-dereferenceable load after the early exit\n");
    return false;
  }

  auto *Oracle = buildOraclePlan(Plan, HeaderVPBB, EltStoreSize);
  if (!Oracle)
    return false;
  Oracle->insertBefore(*HeaderVPBB, HeaderVPBB->getFirstNonPhi());

  // Replace the loads with speculative_load(ptr, from_end, @oracle, args...),
  // with from_end false, as the oracle counts valid bytes from the start.
  // Everything past the lane taking the early exit is poison; freeze it to keep
  // it out of the early-exit lane-index computation (cttz.elts of an unfrozen
  // compare would be poison), leaving the valid leading lanes unchanged.
...
void VPlanTransforms::materializeSpeculativeLoadOracleCanonicalIV(VPlan &Plan) {
  ...
  // Operand 0 of the oracle and operand 3 of each speculative load using it are
  // placeholders for the canonical IV, which did not exist when they were
  // created.
  VPValue *CanonicalIV = Plan.getVectorLoopRegion()->getCanonicalIV();
  Oracle->setOperand(0, CanonicalIV);
  for (VPUser *U : Oracle->users())
    U->setOperand(3, CanonicalIV);
}
...
void VPlanTransforms::attachSpeculativeLoadChecks(
  ...
  // The oracle is only used by the speculative loads it guards.
  for (VPUser *U : Oracle->users()) {
    ...
    // Check the pointer at iteration 0 (the AddRec's start), not its underlying
    // SCEVUnknown base: @llvm.can.load.speculatively only certifies addresses
    // of the form `start + I * num_bytes`, so an offset between the base and
    // the iteration-0 pointer (e.g. `A[i+1]`, based on `%A` but starting at
    // `%A+1`) would slip through the alignment check, and the vectorized load
    // could span a page boundary the original program never accessed.
