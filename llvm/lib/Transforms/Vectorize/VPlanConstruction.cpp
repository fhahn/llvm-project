  ...
  for (VPRecipeBase &R : Header->phis()) {
    ...
    for (VPSingleDefRecipe *CurrentLink : drop_begin(Worklist)) {
      ...
      if (IsFMulAdd) {
        ...
        // Carry the link's fast-math flags onto the negation.
        Type *PhiTy = PhiR->getScalarType();
        auto *Zero = Plan.getOrAddLiveIn(ConstantFP::get(PhiTy, 0.0));
        auto *FNeg = new VPInstruction(
            Instruction::FSub, {Zero, CurrentLink->getOperand(1)},
