...
void VPlanTransforms::createLoopRegions(VPlan &Plan, DebugLoc DL) {
  ...
  // Transfer the temporary header mask to the new loop region.
  auto HeaderMask = Header->getFirstNonPhi();
  if (HeaderMask != Header->end() &&
      match(&*HeaderMask, m_VPInstruction<VPInstruction::HeaderMask>())) {
    HeaderMask->getVPSingleValue()->replaceAllUsesWith(
...
void VPlanTransforms::foldTailByMasking(VPlan &Plan) {
  ...
  // condition now: it may depend on recipes in the body, which the header
  // mask will bypass.
  assert(match(OrigLatch->getTerminator(), m_BranchOnCond()) &&
         "expected a single countable latch exit");
  VPBasicBlock *Latch =
  ...
  // createLoopRegions transfers this mask to the loop region.
