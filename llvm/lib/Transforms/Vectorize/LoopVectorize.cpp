...
public:
  ...
  /// predicated or its probability is unknown. Used to seed VPlan construction,
  /// where they are recorded on the block and later scale its cost.
...
MDNode *LoopVectorizationCostModel::getEstimatedPredBlockWeights(
    const BasicBlock *BB) {
...
VPCostContext::getPredBlockCostDivisor(const VPBasicBlock *VPBB) const {
  ...
  if (CostKind == TTI::TCK_CodeSize)
    return 1;
  // The block's estimated branch weights encode a conditional entry as
  // {TakenWeight, NotTakenWeight}, so the reciprocal entry probability relative
  // to the loop header is their sum. They are recorded on the block at VPlan0
  // construction and carried onto the replicate region's ".if" block. Missing
  // weights leave the cost unscaled.
