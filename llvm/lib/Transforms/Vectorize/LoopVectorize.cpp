...
void LoopVectorizationPlanner::addMinimumIterationCheck(
    ...
    ElementCount MinProfitableTripCount, bool ForceEmitCheck) const {
...
bool LoopVectorizePass::processLoop(Loop *L) {
  ...
  if (EpilogueVF.Width.isVector()) {
    ...
    // Add iteration count checks to the main plan. The epilogue VF check
    // (TC < EpilogueVF * EpilogueUF) is added to Entry. The main VF check
    // (TC < MainVF * MainUF) is added as a new block before VectorPreheader.
    LVP.addMinimumIterationCheck(*BestMainPlan, EPI.EpilogueVF,
                                 EPI.EpilogueUF,
    ...
    // When the epilogue check is known to never bypass (constant false
    // condition), the entry block has an unconditional branch to the main
    // check block. Otherwise successor(1) is the main check block.
    if (auto *CondBI = dyn_cast<CondBrInst>(EntryBB->getTerminator()))
      EPI.MainLoopIterationCountCheck = CondBI->getSuccessor(1);
    else
      EPI.MainLoopIterationCountCheck = EntryBB->getSingleSuccessor();

    // Move the main loop iteration count check after runtime check blocks.
    // After executePlan the order is:
    //   Entry -> MainCheck -> [SCEV] -> [Mem] -> VecPH
    // Reorder to:
    //   Entry -> [SCEV] -> [Mem] -> MainCheck -> VecPH
    BasicBlock *MainCheck = EPI.MainLoopIterationCountCheck;
    BasicBlock *SCEVBlock = Checks.getSCEVChecks().second;
    BasicBlock *MemBlock = Checks.getMemRuntimeChecks().second;
    if (SCEVBlock || MemBlock) {
      ...
      LastRT->getTerminator()->replaceUsesOfWith(VecPH, MainCheck);

      DomTreeUpdater DTU(DT, DomTreeUpdater::UpdateStrategy::Eager);
      DTU.applyUpdates(
          {{DominatorTree::Delete, EntryBB, MainCheck},
           {DominatorTree::Insert, EntryBB, FirstRT},
           {DominatorTree::Delete, MainCheck, FirstRT},
           {DominatorTree::Insert, MainCheck, VecPH},
           {DominatorTree::Delete, LastRT, VecPH},
           {DominatorTree::Insert, LastRT, MainCheck}});
    }
