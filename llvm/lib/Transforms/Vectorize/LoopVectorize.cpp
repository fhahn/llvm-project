...
struct MainLoopResumeValues {
  ...
  /// Markers for the resume values of the scalar loop's header phis, keyed by
  /// the header phi they belong to.
  DenseMap<PHINode *, VPInstruction *> HeaderPhis;

  /// Markers for the values reductions accumulating in a domain of their own
  /// resume accumulating from, keyed by the header phi they belong to. Their
  /// bypass value is the value the reduction starts accumulating from.
  DenseMap<PHINode *, VPInstruction *> Accumulators;
...
static MainLoopResumeValues preparePlanForMainVectorLoop(VPlan &MainPlan,
  ...
  MainLoopResumeValues ResumeValues;
  auto AddFreezeForFindLastIVReductions = [&ResumeValues](VPlan &Plan,
                                                          bool IsMainPlan) {
  ...
  // Map the phis of the scalar loop to the reductions computing them, to be
  // able to find the value each reduction accumulates in.
  DenseMap<PHINode *, VPReductionPHIRecipe *> IRPhiToRedPhiR;
  for (VPRecipeBase &R :
       MainPlan.getVectorLoopRegion()->getEntryBasicBlock()->phis())
    if (auto *RedPhiR = dyn_cast<VPReductionPHIRecipe>(&R))
      if (auto *IRPhi =
              dyn_cast_or_null<PHINode>(RedPhiR->getUnderlyingValue()))
        IRPhiToRedPhiR[IRPhi] = RedPhiR;
  ...
  for (VPRecipeBase &R : MainPlan.getScalarHeader()->phis()) {
    ...
    ResumeValues.HeaderPhis[IRPhi] = ResumeBuilder.createNaryOp(
        VPInstruction::ResumeForEpilogue,
    ...
    // value, while the scalar loop resumes from the lowered one. Create a
    // marker for the accumulated value as well; a single one cannot serve both.
    auto *RedPhiR = IRPhiToRedPhiR.lookup(IRPhi);
    ...
    ResumeValues.Accumulators[IRPhi] = ResumeBuilder.createNaryOp(
...
static void preparePlanForEpilogueVectorLoop(VPlan &Plan,
  ...
  for (VPRecipeBase &R : Header->phis()) {
    ...
    if (auto *ReductionPhi = dyn_cast<VPReductionPHIRecipe>(&R)) {
      ...
      if (accumulatesInOwnDomain(ReductionPhi->getRecurrenceKind())) {
        cast<VPHeaderPHIRecipe>(&R)->setStartValue(GetResumeValue(
            ResumeValues.Accumulators.at(IRPhi), "bc.merge.rdx"));
        continue;
      }

      StartVal =
          GetResumeValue(ResumeValues.HeaderPhis.at(IRPhi), "bc.merge.rdx");
