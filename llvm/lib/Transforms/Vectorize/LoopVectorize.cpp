...
/// Markers in the middle block of the main plan keeping the resume values
/// computed by the main vector loop available for the epilogue plan. After
/// executing it, each marker's underlying value is the generated IR value and
/// its second operand the value to resume at when the loop is bypassed.
struct MainLoopResumeValues {
  /// The markers created for one of the scalar loop's header phis.
  struct Markers {
    /// Marker for the value the scalar loop resumes from.
    VPInstruction *ScalarResume = nullptr;

    /// Marker for the value the epilogue vector loop resumes accumulating
    /// from, for reductions accumulating in a domain of their own, null for
    /// all other header phis. Its bypass value is the value the reduction
    /// starts accumulating from.
    VPInstruction *Accumulated = nullptr;
  };

  /// Marker for the canonical IV's resume value, i.e. the vector trip count.
  VPInstruction *CanonicalIV = nullptr;

  /// The markers for the scalar loop's header phis, keyed by the header phi
  /// they belong to.
  /// TODO: Replace the IR PHI key.
  DenseMap<PHINode *, Markers> HeaderPhis;

  /// The start values frozen by the main plan, keyed by the value being frozen,
  /// so the epilogue plan can re-use them.
  DenseMap<Value *, VPInstruction *> Frozen;
};
...
struct EpilogueLoopVectorizationInfo {
  ...
  MainLoopResumeValues ResumeValues;
...
BasicBlock *EpilogueVectorizerEpilogueLoop::createVectorizedLoopSkeleton() {
  ...
  // heading the chain.
  RUN_VPLAN_PASS(VPlanTransforms::modelGeneratedMainLoopBlocks, Plan,
                 EPI.MainPlan, NewEntry, EPI.MainLoopCheck);

  // When the epilogue vector loop is skipped, the scalar loop resumes at the
  // values the main vector loop computed. Update the scalar preheader's phis
  // for the edge from the plan's entry accordingly; so far they hold the
  ...
  for (VPRecipeBase &R : Plan.getScalarHeader()->phis()) {
    // Skip header phis whose resume phi has been folded away, e.g. because the
    // scalar preheader has a single predecessor; they resume at a value
    ...
    VPInstruction *ResumeForEpi =
        EPI.ResumeValues.HeaderPhis.at(&cast<VPIRPhi>(R).getIRPhi())
            .ScalarResume;
    ResumePhi->setIncomingValueForBlock(
        NewEntry, Plan.getOrAddLiveIn(ResumeForEpi->getUnderlyingValue()));
  }
...
/// Returns true if reductions of kind \p RK accumulate in a value domain of
/// their own, i.e. the middle block lowers the accumulated value before the
/// scalar loop resumes from it: AnyOf accumulates a boolean, FindIV a
/// sentinel-encoded IV. Both lowerings depend on the recurrence kind only, not
/// on the VF, so the main and epilogue vector loops agree on the domain and the
/// epilogue can continue accumulating where the main vector loop left off.
static bool accumulatesInOwnDomain(RecurKind RK) {
...
/// vectorization.
static MainLoopResumeValues preparePlanForMainVectorLoop(VPlan &MainPlan,
                                                         VPlan &EpiPlan) {
  using namespace VPlanPatternMatch;
  ...
  auto AddFreezeForFindLastIVReductions = [&](VPlan &Plan) {
    ...
    for (VPRecipeBase &R : *Plan.getMiddleBlock()) {
      ...
      if (!IsMainPlan)
        continue;
      ResumeValues.Frozen[OrigStart->getLiveInIRValue()] = Freeze;
      OrigStart->replaceUsesWithIf(Freeze, [Freeze](VPUser &U, unsigned) {
        return Freeze != &U && isa<VPPhi>(&U);
      });
    }
  };
  AddFreezeForFindLastIVReductions(MainPlan);
  AddFreezeForFindLastIVReductions(EpiPlan);
  ...
  // Create ResumeForEpilogue markers keeping the resume values computed by the
  // main vector loop available for the epilogue plan. They are placed in the
  // middle block, which is where the values leave the main vector loop, before
  // its terminator if it has one, i.e. unless it has a single successor.
  ...
  VPBuilder ResumeBuilder(MiddleVPBB, MiddleTerm ? MiddleTerm->getIterator()
                                                 : MiddleVPBB->end());
  ResumeValues.CanonicalIV = ResumeBuilder.createNaryOp(
  ...
  // own. That lets the scalar loop resume at its original start values until
  // the epilogue plan is executed.
  for (VPRecipeBase &R : MainPlan.getScalarHeader()->phis()) {
    ...
    assert(ResumePhi->getParent() == MainPlan.getScalarPreheader() &&
           ...
           "resume phi must be in the scalar preheader, with its first "
           "incoming value coming from the middle block");
    auto *IRPhi = &cast<VPIRPhi>(R).getIRPhi();
    ResumeValues.HeaderPhis[IRPhi].ScalarResume = ResumeBuilder.createNaryOp(
        VPInstruction::ResumeForEpilogue,
  ...
  // Reductions accumulating in a domain of their own continue accumulating in
  // the epilogue vector loop, which therefore resumes from the accumulated
  // value, while the scalar loop resumes from the lowered one. Create a marker
  // for the accumulated value as well; a single one cannot serve both.
  for (VPRecipeBase &R :
       MainPlan.getVectorLoopRegion()->getEntryBasicBlock()->phis()) {
    auto *RedPhiR = dyn_cast<VPReductionPHIRecipe>(&R);
    if (!RedPhiR || !accumulatesInOwnDomain(RedPhiR->getRecurrenceKind()))
      continue;
    auto *IRPhi = dyn_cast_if_present<PHINode>(RedPhiR->getUnderlyingValue());
    auto It = IRPhi ? ResumeValues.HeaderPhis.find(IRPhi)
                    : ResumeValues.HeaderPhis.end();
    if (It == ResumeValues.HeaderPhis.end())
      continue;
    VPInstruction *Accumulated = vputils::findComputeReductionResult(RedPhiR);
    assert(Accumulated && "reduction must have a ComputeReductionResult");
    VPValue *Start = RedPhiR->getStartValue();
    [[maybe_unused]] auto *StartVPI = dyn_cast<VPInstruction>(Start);
    assert((!StartVPI ||
            StartVPI->getOpcode() != VPInstruction::ReductionStartVector) &&
           "expected a scalar start value");
    It->second.Accumulated = ResumeBuilder.createNaryOp(
        VPInstruction::ResumeForEpilogue, {Accumulated, Start});
  }
  return ResumeValues;
}
...
static void preparePlanForEpilogueVectorLoop(VPlan &Plan,
                                             ...
                                             VFSelectionContext &Config,
                                             ScalarEvolution &SE) {
  const MainLoopResumeValues &ResumeValues = EPI.ResumeValues;
  ...
  // at the vector trip count of the main vector loop, or at zero if the main
  // vector loop has been bypassed. Add the phi merging both as offset to the
  // canonical IV of the epilogue loop.
  ...
  for (VPRecipeBase &R : Header->phis()) {
    ...
    if (auto *ReductionPhi = dyn_cast<VPReductionPHIRecipe>(&R)) {
      auto *IRPhi = cast<PHINode>(ReductionPhi->getUnderlyingInstr());
      const auto &Markers = ResumeValues.HeaderPhis.at(IRPhi);
      // Reductions accumulating in a domain of their own resume accumulating
      // where the main vector loop left off, rather than from the value the
      // scalar loop resumes from.
      if (Markers.Accumulated) {
        ReductionPhi->setStartValue(
            GetResumeValue(Markers.Accumulated, "bc.merge.rdx"));
        continue;
      }

      StartVal = GetResumeValue(Markers.ScalarResume, "bc.merge.rdx");
      if (auto *VPI = dyn_cast<VPInstruction>(ReductionPhi->getStartValue())) {
        ...
        // Whether the final subtraction is present depends on the VF, so the
        // epilogue cannot resume from the accumulated value; update the operand
        // the main vector loop contributes to instead.
        if (ReductionPhi->getVFScaleFactor() > 1 &&
          ...
          VPInstruction *RdxResult =
              vputils::findComputeReductionResult(ReductionPhi);
          assert(RdxResult && "reduction must have a ComputeReductionResult");
          ...
          [[maybe_unused]] auto StartValueIsIdentity = [&] {
            Value *IdentityValue = getRecurrenceIdentity(
                ReductionPhi->getRecurrenceKind(), StartVal->getScalarType(),
      ...
      StartVal = GetResumeValue(ResumeValues.HeaderPhis.at(IndPhi).ScalarResume,
                                "bc.resume.val");
    }
  ...
  for (auto &R : make_early_inc_range(*Plan.getEntry())) {
    ...
    if (VPI && VPI->getOpcode() == Instruction::Freeze) {
      VPInstruction *MainFreeze =
          ResumeValues.Frozen.lookup(VPI->getOperand(0)->getLiveInIRValue());
      assert(MainFreeze && "main plan must have frozen the same value");
      VPI->replaceAllUsesWith(
          Plan.getOrAddLiveIn(MainFreeze->getUnderlyingValue()));
      continue;
    }
  ...
  RUN_VPLAN_PASS(VPlanTransforms::addMinimumVectorEpilogueIterationCheck, Plan,
                 MainVectorTripCount, Plan.requiresScalarEpilogue(),
...
bool LoopVectorizePass::processLoop(Loop *L) {
  ...
  if (EpiPlan) {
    ...
    EpilogueLoopVectorizationInfo EPI(VF.Width, IC, EpilogueVF, 1,
                                      BestMainPlan);
    EPI.ResumeValues = preparePlanForMainVectorLoop(BestMainPlan, BestEpiPlan);
    ...
    // Collect the blocks bypassing both vector loops: the entry block and the
    ...
    // The block holding the iteration count check for the main vector loop is
    // the predecessor of the scalar preheader that branches to it as its first
    // successor without bypassing both vector loops; it branches to the
