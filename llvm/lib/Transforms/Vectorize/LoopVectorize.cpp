...
public:
  ...
  /// Returns true if \p Op should be considered invariant and if it is trivially hoistable.
      ...
      for (VFInfo &Info : VFDatabase::getMappings(*CI)) {
        ...
        for (VFParameter Param : Info.Shape.Parameters) {
          ...
          case VFParamKind::OMP_Uniform: {
            ...
            if (!shouldConsiderInvariant(ScalarParam))
              ParamsOk = false;
            break;
          }
...
LoopVectorizationCostModel::getInstructionCost(Instruction *I,
  ...
  case Instruction::Xor: {
    ...
    auto Op2Info = TTI.getOperandInfo(Op2);
    if (Op2Info.Kind == TargetTransformInfo::OK_AnyValue && shouldConsiderInvariant(Op2))
      Op2Info.Kind = TargetTransformInfo::OK_UniformValue;
