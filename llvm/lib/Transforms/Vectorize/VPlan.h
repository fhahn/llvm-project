...
class VPSpeculativeLoadOracleRecipe : public VPSingleDefRecipe {
  ...
  /// The type of the canonical IV in the oracle plan's scalar loop.
  Type *IVTy;

public:
  VPSpeculativeLoadOracleRecipe(std::unique_ptr<VPlan> OraclePlan, Type *IVTy,
                                ArrayRef<VPValue *> ExternalOperands)
      : VPSingleDefRecipe(VPRecipeBase::VPSpeculativeLoadOracleSC,
                          ExternalOperands, {}),
        OraclePlan(std::move(OraclePlan)), IVTy(IVTy) {}
  ...
  Type *getIVTy() const { return IVTy; }
