...
void VPlanTransforms::convertEVLExitCond(VPlan &Plan) {
  ...
  assert((match(LatchBr, m_BranchOnCond()) ||
          match(LatchBr, m_BranchOnTwoConds())) &&
         "expected a conditional latch branch");
  ...
  assert(FoundIncrement &&
         ...
         "Expected countable exit condition comparing CanIV + VFxUF with "
         "vector trip count");
