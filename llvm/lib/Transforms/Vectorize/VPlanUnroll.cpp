...
static void addLaneToStartIndex(VPScalarIVStepsRecipe *Steps, unsigned Lane,
  ...
  VPIRFlags Flags;
  // The lane offset is a positive position within the vector group and is
  // always *added* to the start index, independent of the induction direction
  // (the induction opcode is applied later, when combining BaseIV with
  // StartIndex * Step in VPScalarIVStepsRecipe::execute).
  if (BaseIVTy->isFloatingPointTy()) {
