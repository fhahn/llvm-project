  ...
  bool match(const VPRecipeBase *R) {
    if (!detail::MatchRecipeAndOpcode<Opcode, RecipeTys...>::match(R)) {
      ...
      return LHS.match(R->getOperand(0)) && RHS.match(R->getOperand(1)) &&
             m_SpecificInt(0).match(R->getOperand(2));
    }
    assert(R->getNumOperands() == 2 &&
           "recipe with matched opcode does not have 2 operands");
    return LHS.match(R->getOperand(0)) && RHS.match(R->getOperand(1));
...
template <typename Op0_t, typename Op1_t>
inline LogicalRecipe_match<Op0_t, Op1_t, Instruction::And, VPWidenRecipe,
                           VPReplicateRecipe, VPWidenCastRecipe, VPInstruction>
m_LogicalAnd(const Op0_t &Op0, const Op1_t &Op1) {
  return LogicalRecipe_match<Op0_t, Op1_t, Instruction::And, VPWidenRecipe,
