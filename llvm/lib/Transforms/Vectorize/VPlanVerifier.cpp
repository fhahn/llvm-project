...
/// Returns the number of elements \p V's result is explicitly widened to, or
/// std::nullopt if it is widened to the plan's VF (see VPValue::getWideType).
static std::optional<ElementCount> getWideResultVF(const VPValue *V) {
  const auto *RV = dyn_cast<VPRecipeValue>(V);
  auto *VecTy =
      RV ? dyn_cast_if_present<VectorType>(RV->getResultType()) : nullptr;
  if (!VecTy)
    return std::nullopt;
...
static bool verifyWideningVF(const VPRecipeBase &R) {
  for (const VPValue *Def : R.definedValues()) {
    std::optional<ElementCount> WideVF = getWideResultVF(Def);
    if (!WideVF)
      continue;
    if (all_of(R.operands(), [&](const VPValue *Op) {
          return R.usesScalars(Op) || getWideResultVF(Op) == WideVF;
        }))
      continue;
