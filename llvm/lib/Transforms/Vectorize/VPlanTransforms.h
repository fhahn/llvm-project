...
#define RUN_VPLAN_PASS_NO_VERIFY(PASS, ...)                                    \
  ...
  /// EarlyExitingVPBB to \p EarlyExitVPBB by updating the condition exiting
  /// the loop via the latch to include the early exit condition and splitting
  /// the original middle block to branch to the early exit block conditionally
  /// according to the early exit condition. Any load in the loop that is not
  /// provably dereferenceable will be converted to a speculative load
  /// intrinsic.
