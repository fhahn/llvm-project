...
static VPValue *simplifyRecipe(VPSingleDefRecipe *Def) {
  ...
  if (match(Def, m_Not(m_VPValue(A)))) {
    ...
    if (match(A, m_Cmp(Pred, m_VPValue(), m_VPValue()))) {
      ...
      // solely as its condition. If a select also uses the cmp as a value
      // operand, inverting the predicate would flip that value use, which the
      // condition/operand swap below cannot compensate for.
  ...
  return all_of(Info.Shape.Parameters, [&](VFParameter Param) {
    ...
    case VFParamKind::OMP_Uniform: {
      if (!SE->isSCEVable(Args[Param.ParamPos]->getScalarType()))
        return false;
      // isLoopInvariant asserts on SCEVCouldNotCompute, which
      // getSCEVExprForVPValue returns e.g. for a widened load. Such a value is
      // not loop-invariant.
      const SCEV *S =
...
static Function *findVectorVariant(CallInst *CI, ArrayRef<VPValue *> Args,
  ...
  for (const VFInfo &Info : VFDatabase::getMappings(*CI)) {
    if (Info.Shape.VF != VF || (MaskRequired && !Info.isMasked()) ||
      ...
      continue;
    // Skip variants whose declaration does not match the shape, e.g. a masked
    // variant without a trailing mask parameter.
    if (Function *Variant =
