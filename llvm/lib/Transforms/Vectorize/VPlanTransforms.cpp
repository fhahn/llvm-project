...
static void simplifyRecipe(VPSingleDefRecipe *Def) {
  ...
  if (match(Def, m_Add(m_VPValue(IVInc, m_Add(m_VPValue(X), m_VPValue())),
    ...
    bool AvailableAtStart =
        isa<VPIRValue>(Y) || (YR && YR->getParent() == Phi->getIncomingBlock(0));
...
static bool simplifyBranchConditionForVFAndUF(VPlan &Plan, ElementCount BestVF,
  ...
  bool MatchedCanIVInc = match(
      Term, m_BranchOnCount(
                m_CombineOr(m_CanIVInc, m_c_Add(m_CanIVInc, m_VPValue(Offset))),
                m_VPValue()));
  if (MatchedCanIVInc && Offset) {
    VPRecipeBase *OffsetR = Offset->getDefiningRecipe();
    MatchedCanIVInc =
        !OffsetR || OffsetR->getParent()->getParent() != VectorRegion;
  }
  if (MatchedCanIVInc ||
      match(Term,
