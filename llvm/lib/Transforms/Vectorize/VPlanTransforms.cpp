...
void VPlanTransforms::tryToRealignLoop(VPlan &Plan) {
  ...
  for (VPRecipeBase &R : *Header) {
    if (isa<VPReductionPHIRecipe, VPFirstOrderRecurrencePHIRecipe,
            VPWidenIntOrFpInductionRecipe, VPWidenPointerInductionRecipe>(&R))
      return;
  ...
  auto IsSupportedPtr = [&Plan](VPValue *V) -> VPValue * {
    ...
    VPValue *Base, *Offset;
    if (match(RepR, m_Binary<Instruction::GetElementPtr>(m_VPValue(Base),
                                                         m_VPValue(Offset))) &&
        isa<VPScalarIVStepsRecipe>(Offset) &&
        cast<VPScalarIVStepsRecipe>(Offset)->getOperand(0) ==
            Plan.getCanonicalIV())
      return Base;
    return nullptr;
  };
  ...
  VPValue *LastIter =
      BPH.createSelect(NeedsAlign, &Plan.getVectorTripCount(),
                       Plan.getOrAddLiveIn(ConstantInt::get(
                           Plan.getCanonicalIV()->getScalarType(), -1)));
  ...
  VPInstruction *Sub = BPH.createNaryOp(Instruction::Sub, {&Plan.getVFxUF(), Plan.getOrAddLiveIn(ConstantInt::get(Plan.getCanonicalIV()->getScalarType(), 1))}, {}, "rnd.up.tc");
  ...
  /*VPValue *AlignIter = B.createNaryOp(Instruction::And, {NeedsAlign,
   * IsLastIter});*/
  ...
  cast<VPInstruction>(Header->getTerminator())
      ->setOperand(1, Plan.getTripCount());
  CanIV->setOperand(1, Sel);

  /*  CanIV->replaceUsesWithIf(Sel, [CanIV, RemIters, Sel](VPUser &U, unsigned)
   * {*/
  /*return !isa<VPInstruction>(&U) || (cast<VPInstruction>(&U) !=
   * CanIV->getBackedgeValue() && cast<VPInstruction>(&U) != RemIters&&
   * cast<VPInstruction>(&U) != Sel);*/
  /*});*/

  BranchOnCond->setOperand(
      0, Plan.getOrAddLiveIn(ConstantInt::getTrue(
