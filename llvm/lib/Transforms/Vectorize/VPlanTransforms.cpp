...
/// Returns true if \p R is a widening recipe or a wide (vector-producing) cast,
/// i.e. a flag-carrying recipe widening a single underlying operation.
static bool isWidenOrWidenCast(const VPRecipeBase *R) {
  return isa<VPWidenRecipe>(R) || match(R, m_WidenCast());
}
