...
void VPlanTransforms::simplifyRecipes(VPlan &Plan) {
  ...
  while (!Worklist.empty()) {
    ...
    // simplifyRecipe inserts the recipes it creates directly before \p Def.
    // Remember \p Def's predecessor, so a freshly created replacement can be
    // told apart from a pre-existing one below.
    VPBasicBlock *VPBB = Def->getParent();
    auto PredOfDef = [&]() -> VPRecipeBase * {
      auto It = Def->getIterator();
      return It == VPBB->begin() ? nullptr : &*std::prev(It);
    };
    VPRecipeBase *PrevR = PredOfDef();

    VPValue *New = simplifyRecipe(Def);
    if (!New)
      continue;
    if (New != Def) {
      // Only revisit the replacement if simplifyRecipe created it. A
      // pre-existing replacement is not changed by the substitution, so
      // re-simplifying it cannot expose anything new.
      auto *NewR =
          dyn_cast_if_present<VPSingleDefRecipe>(New->getDefiningRecipe());
      if (NewR && NewR != PrevR && NewR == PredOfDef())
