...
getOpcodeOrIntrinsicID(const VPSingleDefRecipe *R) {
  return TypeSwitch<const VPSingleDefRecipe *,
      ...
      .Case<VPInstruction, VPWidenRecipe, VPWidenRecipe, VPWidenGEPRecipe,
            VPReplicateRecipe>(
...
void VPlanTransforms::truncateToMinimalBitwidths(
  ...
  // typed.
  DenseMap<VPValue *, VPWidenRecipe *> ProcessedTruncs;
  VPBasicBlock *PH = Plan.getVectorPreheader();
  SmallVector<VPRecipeBase *> ToErase;
  for (VPBasicBlock *VPBB : VPBlockUtils::blocksOnly<VPBasicBlock>(
           vp_depth_first_deep(Plan.getVectorLoopRegion()))) {
    for (VPRecipeBase &R : make_early_inc_range(*VPBB)) {
      if (!isa<VPWidenRecipe, VPReplicateRecipe,
      ...
      if (isa<VPReplicateRecipe>(&R) || R.isWidenCast())
        continue;
      ...
      auto InsertExtAfter = [&](VPValue *Source,
                                VPRecipeBase *After) -> VPWidenRecipe * {
        assert(OldResSizeInBits > NewResSizeInBits && "Nothing to shrink?");
        auto *Ext = new VPWidenRecipe(
...
static void fixupVFUsersForEVL(VPlan &Plan, VPValue &EVL) {
  ...
  Type *IdxTy = Plan.getVF().getScalarType();
  Type *EVLTy = EVL.getScalarType();
...
tryToMatchAndCreateExtendedReduction(VPReductionRecipe *Red, VPCostContext &Ctx,
    ...
    return LoopVectorizationPlanner::getDecisionAndClampRange(
        [&](ElementCount VF) {
          ...
          InstructionCost ExtCost =
              cast<VPWidenRecipe>(VecOp)->computeCost(VF, Ctx);
  ...
  VPValue *A;
  // Match reduce(ext)).
  if (match(VecOp, m_Isa<VPWidenRecipe>(m_ZExtOrSExt(m_VPValue(A)))) &&
      IsExtendedRedValidAndClampRange(
          RecurrenceDescriptor::getOpcode(Red->getRecurrenceKind()),
          Instruction::CastOps(cast<VPWidenRecipe>(VecOp)->getOpcode()),
          A->getScalarType()))
    return new VPExpressionRecipe(cast<VPWidenRecipe>(VecOp), Red);
...
tryToMatchAndCreateMulAccumulateReduction(VPReductionRecipe *Red,
  ...
  auto IsMulAccValidAndClampRange =
      [&](VPWidenRecipe *Mul, VPWidenRecipe *Ext0, VPWidenRecipe *Ext1,
          VPWidenRecipe *OuterExt) -> bool {
    return LoopVectorizationPlanner::getDecisionAndClampRange(
        [&](ElementCount VF) {
          TTI::TargetCostKind CostKind = TTI::TCK_RecipThroughput;
  ...
  auto ExtendAndReplaceConstantOp = [](VPWidenRecipe *ExtA,
                                       VPWidenRecipe *&ExtB, VPValue *&ValB,
                                       VPWidenRecipe *Mul) {
    if (!ExtA || !Instruction::isCast(ExtA->getOpcode()) || ExtB ||
        !isa<VPIRValue>(ValB))
      return;
    Type *NarrowTy = ExtA->getOperand(0)->getScalarType();
    Instruction::CastOps ExtOpc = Instruction::CastOps(ExtA->getOpcode());
    const APInt *Const;
  ...
  if (match(VecOp, m_Mul(m_VPValue(A), m_VPValue(B)))) {
    auto *RecipeA = dyn_cast<VPWidenRecipe>(A);
    auto *RecipeB = dyn_cast<VPWidenRecipe>(B);
  ...
  if (match(VecOp, m_ZExtOrSExt(m_Mul(m_VPValue(A), m_VPValue(B))))) {
    auto *Ext = cast<VPWidenRecipe>(VecOp);
    auto *Mul = cast<VPWidenRecipe>(Ext->getOperand(0));
    auto *Ext0 = dyn_cast<VPWidenRecipe>(A);
    auto *Ext1 = dyn_cast<VPWidenRecipe>(B);
...
collectComplementaryPredicatedMemOps(VPlan &Plan,
  ...
  auto GetLoadStoreValueType = [&](VPReplicateRecipe *Recipe) {
    return (IsLoad ? Recipe : Recipe->getOperand(0))->getScalarType();
  };
...
optimizeExtendsForPartialReduction(VPSingleDefRecipe *Op) {
  ...
  if (match(Op, m_Mul(m_ZExtOrSExt(m_VPValue()), m_APInt(Const)))) {
    auto *ExtA = cast<VPWidenRecipe>(Op->getOperand(0));
    Instruction::CastOps ExtOpc = Instruction::CastOps(ExtA->getOpcode());
    Type *NarrowTy = ExtA->getOperand(0)->getScalarType();
    if (!Op->hasOneUse() ||
        !llvm::canConstantBeExtended(
  ...
  if (match(Op, m_WidenIntrinsic<Intrinsic::abs>(m_Sub(
    ...
    auto *Ext = cast<VPWidenRecipe>(Sub->getOperand(0));
    assert(Ext->getOpcode() ==
               cast<VPWidenRecipe>(Sub->getOperand(1))->getOpcode() &&
  ...
  if (match(Op, m_ZExtOrSExt(m_Mul(m_ZExtOrSExt(m_VPValue()),
    ...
    auto *Ext = cast<VPWidenRecipe>(Op);
    auto *Mul = cast<VPWidenRecipe>(Ext->getOperand(0));
    auto *MulLHS = cast<VPWidenRecipe>(Mul->getOperand(0));
    auto *MulRHS = cast<VPWidenRecipe>(Mul->getOperand(1));
    if (!Mul->hasOneUse() ||
...
createPartialReductionExpression(VPReductionRecipe *Red) {
  ...
  if (match(VecOp, m_FMul(m_FPExt(m_VPValue()), m_FPExt(m_VPValue()))) ||
    ...
    auto *Mul = cast<VPWidenRecipe>(VecOp);
    auto *ExtA = cast<VPWidenRecipe>(Mul->getOperand(0));
    auto *ExtB = cast<VPWidenRecipe>(Mul->getOperand(1));
...
static ExtendKind getPartialReductionExtendKind(VPWidenRecipe *Cast) {
  return TTI::getPartialReductionExtendKind(
      Instruction::CastOps(Cast->getOpcode()));
}
...
matchExtendedReductionOperand(VPWidenRecipe *UpdateR, VPValue *Op) {
  ...
  if (Op->hasOneUse() &&
    ...
    auto *LHSExt = cast<VPWidenRecipe>(Sub->getOperand(0));
    auto *RHSExt = cast<VPWidenRecipe>(Sub->getOperand(1));
  ...
  if (match(Op, m_WidenAnyExtend(m_VPValue()))) {
    auto *CastRecipe = cast<VPWidenRecipe>(Op);
  ...
  auto *LHSCast = cast<VPWidenRecipe>(LHS);
  ...
  VPWidenRecipe *RHSCast = nullptr;
  if (match(RHS, m_WidenAnyExtend(m_VPValue())))
    RHSCast = cast<VPWidenRecipe>(RHS);
  ...
  for (VPWidenRecipe *Cast : {LHSCast, RHSCast})
...
void VPlanTransforms::createPartialReductions(VPlan &Plan,
  ...
  auto ExtendUsersValid = [&](VPValue *Ext) {
    auto *WR = dyn_cast<VPWidenRecipe>(Ext);
    return !WR || !Instruction::isCast(WR->getOpcode()) ||
  ...
  auto IsProfitablePartialReductionChainForVF =
    ...
    for (const VPPartialReductionChain &Link : Chain) {
      ...
      for (VPValue *Op : ExtendedOp.ExtendsUser->operands())
        if (auto *Extend = dyn_cast<VPWidenRecipe>(Op))
          if (Instruction::isCast(Extend->getOpcode()))
