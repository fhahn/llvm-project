...
void VPlanTransforms::optimizeFindIVReductions(VPlan &Plan,
  ...
  // Helper lambda to get the RecurKind for FindIV based on direction and
  // signedness.
  auto GetFindIVRecurKind = [](bool IsFindLast, bool Signed) -> RecurKind {
    if (IsFindLast)
      return Signed ? RecurKind::FindLastIVSMax : RecurKind::FindLastIVUMax;
    return Signed ? RecurKind::FindFirstIVSMin : RecurKind::FindFirstIVUMin;
  };
  ...
  for (VPRecipeBase &Phi :
    ...
    auto *AR = dyn_cast<SCEVAddRecExpr>(IVSCEV);
    if (!AR || !SE.isKnownNonZero(Step))
      continue;

    bool IsFindLast = SE.isKnownPositive(Step);

    // Try sentinel-based approach first (signed, then unsigned).
    bool UseSigned = true;
    ...
    RecurKind NewRK = GetFindIVRecurKind(IsFindLast, UseSigned);

    RecurKind MinMaxKind = IsFindLast
                               ? (UseSigned ? RecurKind::SMax : RecurKind::UMax)
    ...
    auto *NewPhiR = new VPReductionPHIRecipe(
        cast<PHINode>(PhiR->getUnderlyingInstr()), NewRK, *StartVPV,
