...
static VPIRMetadata getCommonMetadata(ArrayRef<VPReplicateRecipe *> Recipes) {
  ...
  // The recipe the common metadata is used for is not predicated, so drop !prof.
...
void VPlanTransforms::dropBranchWeightsFromUnguardedRecipes(VPlan &Plan) {
  for (VPBasicBlock *VPBB : VPBlockUtils::blocksOnly<VPBasicBlock>(
    ...
    for (VPRecipeBase &R : *VPBB) {
      ...
      if (!MD)
        continue;
      // Only the execution probability the predicator records on masked recipes
      // is to be dropped here. Terminators carry the branch weights of the
      // branch they will be lowered to, which the predicator never annotates, as
      // it erases the terminators of the blocks it masks.
