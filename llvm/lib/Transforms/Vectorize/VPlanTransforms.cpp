...
/// Carry \p V from the end of \p FromVPBB to the latch, taking \p Absent on
/// paths that bypass \p FromVPBB. \p BodyRPO lists the acyclic loop body from
/// header to latch in reverse post-order. Only joins after \p FromVPBB can
/// need phis; earlier predecessors contribute \p Absent.
static VPValue *carryToLatch(VPValue *V, VPValue *Absent,
                             VPBasicBlock *FromVPBB,
                             ArrayRef<VPBasicBlock *> BodyRPO) {
  auto From = find(BodyRPO, FromVPBB);
  assert(From != BodyRPO.end() && "source must be in the loop body");
  DenseMap<const VPBlockBase *, VPValue *> Reaching;
  Reaching[FromVPBB] = V;
  for (VPBasicBlock *VPBB : make_range(std::next(From), BodyRPO.end())) {
    ...
    Reaching[VPBB] =
        all_equal(Incoming)
            ? Incoming[0]
            : VPBuilder(VPBB, VPBB->begin()).createScalarPhi(Incoming);
  }
...
bool VPlanTransforms::handleUncountableEarlyExits(
  ...
  // With early-exit edges detached, RPO is a topological order of the body.
  auto BodyRPO = vp_rpo_plain_cfg_loop_body(HeaderVPBB);
  DenseMap<const VPBlockBase *, unsigned> RPOIdx;
  for (auto [Idx, VPBB] : enumerate(BodyRPO))
    RPOIdx[VPBB] = Idx;
  ...
  // Carry conditions from their exiting blocks, even if defined earlier,
  // so predication combines them with those blocks' execution masks. Paths
  // bypassing an exit contribute false.
  // Live-outs instead start at their defining blocks and take poison on
  // bypassing paths: extraction selects only a lane that reached the
  // definition.
