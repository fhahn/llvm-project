...
static VPValue *simplifyOrRecipe(VPlan &Plan, VPSingleDefRecipe *Def) {
  ...
  VPValue *Y;
  if (match(Def,
            m_c_BinaryOr(m_LogicalAnd(m_VPValue(A), m_VPValue(Y)),
                         m_LogicalAnd(m_Deferred(A), m_Not(m_Deferred(Y))))))
    return A;

  // X | AllOnes -> AllOnes and X | !X -> AllOnes
  if (match(Def, m_c_BinaryOr(m_VPValue(), m_AllOnes())) ||
      match(Def, m_c_BinaryOr(m_VPValue(A), m_Not(m_Deferred(A)))))
    return Plan.getAllOnesValue(Def->getScalarType());

  // X | 0 -> X
  if (match(Def, m_c_BinaryOr(m_VPValue(A), m_ZeroInt())))
    return A;
  return nullptr;
}
...
static VPValue *simplifyExtractLaneRecipe(VPlan &Plan, VPSingleDefRecipe *Def) {
  VPValue *A, *LaneToExtract;
  if (!Plan.isUnrolled() ||
      !match(Def, m_ExtractLane(m_VPValue(LaneToExtract), m_VPValue(A))))
    return nullptr;
...
/// or 0 if there is none. Recipes without an opcode of their own are mapped to
/// the opcode of the patterns matching them: VPHeaderPHIRecipes to
/// Instruction::PHI, VPBlendRecipes to Instruction::Select (m_SelectLike),
/// intrinsic calls to Instruction::Call (m_Intrinsic) and VPDerivedIVRecipes to
/// their recipe-ID based opcode (m_DerivedIV).
...
static VPValue *simplifyRecipe(VPlan &Plan, VPSingleDefRecipe *Def,
  ...
  case Instruction::And:
    // X & 0 -> 0
    if (match(Def, m_c_BinaryAnd(m_VPValue(), m_ZeroInt())))
      return Plan.getZero(Def->getScalarType());

    // X & AllOnes -> X
    if (match(Def, m_c_BinaryAnd(m_VPValue(A), m_AllOnes())))
      return A;
    return nullptr;
  ...
  // The remaining simplifications can only be applied after unrolling.
...
/// Drop the mask of a predicated store in \p Def masked by the header mask
/// (which is guaranteed to be true at least for the first lane) if both the
/// stored value and the address are uniform across VF and UF. The header
/// mask is still the abstract region value here.
...
/// Combine select-like recipes (VPInstruction::LogicalAnd, or a select
/// matching m_LogicalAnd or m_SelectLike) in \p Def, using \p Builder to
...
combineSelectOrLogicalAndRecipe(VPBuilder &Builder, VPSingleDefRecipe *Def,
...
static VPSingleDefRecipe *combineRecipe(VPlan &Plan, VPSingleDefRecipe *Def) {
  ...
  case Instruction::Select:
  case VPInstruction::LogicalAnd:
    return combineSelectOrLogicalAndRecipe(Builder, Def, CanCreateNewRecipe);
  case Instruction::Or:
  ...
  case VPInstruction::WideIVStep:
    if (match(Def, m_VPInstruction<VPInstruction::WideIVStep>(m_VPValue(X),
      ...
      return Builder.createWidenCast(Instruction::Trunc, X,
                                     Def->getScalarType());
    return nullptr;
  case VPInstruction::ExtractLane:
    ...
    if (Plan.isUnrolled() &&
        match(Def, m_ExtractLane(m_VPValue(X), m_VPValue(Y))))
      return Builder.createNaryOp(Instruction::ExtractElement, {Y, X},
                                  Def->getDebugLoc());
    return nullptr;
