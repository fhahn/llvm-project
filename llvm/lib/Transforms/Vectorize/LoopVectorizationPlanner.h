...
public:
  ...
  /// Finalize \p BestPlan by applying \p VF and \p UF specific transformations, converting abstract recipes to concrete ones and dissolving loop regions.
  ///
  /// Returns a mapping of SCEVs to their expanded IR values.
  /// Note that this is a temporary workaround needed due to the current
  /// epilogue handling.
  DenseMap<const SCEV *, Value *> finalizePlan(VPlan &Plan, ElementCount VF,
                                                   unsigned UF,
