...
VPValue *VPSCEVExpander::expand(const SCEV *S) {
  ...
  case scAddRecExpr: {
    ...
    assert(SE.DT.dominates(cast<SCEVAddRecExpr>(S)->getLoop()->getHeader(),
                           cast<VPIRBasicBlock>(Entry)->getIRBasicBlock()) &&
           "can only expand AddRecs for loops outside VPlan's scope");
    // AddRecs cannot be expanded to VPInstructions and must be expanded to IR
    // later. Reuse an existing VPExpandSCEVRecipe for \p S if there is one,
    // otherwise create a new one in the entry block.
    for (VPRecipeBase &R : *Entry) {
      auto *ExpSCEV = dyn_cast<VPExpandSCEVRecipe>(&R);
      if (ExpSCEV && ExpSCEV->getSCEV() == S)
        return ExpSCEV;
    }
