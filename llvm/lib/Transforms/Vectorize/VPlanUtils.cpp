...
static const SCEV *computeSCEVExprForVPValue(const VPValue *V,
  ...
  if (match(V, m_Select(m_VPValue(CondVal), m_VPValue(LHSVal),
    ...
    const SCEV *LHSExpr = getSCEVExprForVPValueImpl(LHSVal, PSE, L, Cache);
    if (LHSExpr == getSCEVExprForVPValueImpl(RHSVal, PSE, L, Cache))
      return LHSExpr;
    return SE.getCouldNotCompute();
  }
      ...
      TypeSwitch<const VPRecipeBase *, const SCEV *>(DefR)
          ...
          .Case([&SE, &PSE, L, &Cache](const VPBlendRecipe *R) -> const SCEV * {
            ...
            if (all_of(seq<unsigned>(1, R->getNumIncomingValues()),
                       [&](unsigned I) {
                         return getSCEVExprForVPValueImpl(
                                    R->getIncomingValue(I), PSE, L, Cache) ==
                                Common;
                       }))
              return Common;
            return SE.getCouldNotCompute();
