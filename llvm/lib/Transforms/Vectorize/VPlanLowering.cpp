...
void VPlanTransforms::expandBranchOnTwoConds(VPlan &Plan,
  ...
  for (VPInstruction *Br : WorkList) {
    ...
    // two conditions and plant a single conditional branch. It is the latch
    // branch of the vector loop and gets its weights from the estimated trip
    // count, in updateLoopMetadataAndProfileInfo, so none are set here.
    if (Succ0 == Succ1) {
    ...
    if (EstimatedVFxUF > 0 &&
      ...
      BranchProbability NotTakenOnce = BranchProbability::getBranchProbability(
      ...
      EarlyExitBr->setMetadata(LLVMContext::MD_prof,
                               MDBuilder(Plan.getContext())
                                   .createBranchWeights(Taken / GCD,
                                                        NotTakenW / GCD));
    } else {
...
void VPlanTransforms::materializeVectorTripCount(
  ...
  if (RequiresScalarEpilogue) {
    ...
    // trip count is a multiple of it, so assume `TripCount % EstimatedVFxUF` to
    // be equally distributed. That is the same event the middle block's
    // terminator branches on, but the two cannot disagree: a loop requiring a
    // scalar epilogue always leaves a remainder, so its middle block is not
    // terminated by a conditional branch at all.
    if (EstimatedVFxUF > 1) {
      MDBuilder MDB(Plan.getContext());
      cast<VPInstruction>(R)->setMetadata(
          LLVMContext::MD_prof, MDB.createBranchWeights(1, EstimatedVFxUF - 1));
    }
  }
