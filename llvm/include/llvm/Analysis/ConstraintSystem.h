...
class ConstraintSystem {
  ...
  /// Set once Fourier-Motzkin elimination has run on the system, which leaves
  /// it in an unusable state. Only used to catch reuse; the checks sit on the
  /// per-fact and per-query paths, so they are not enabled for plain assertion
  /// builds.
  bool Solved = false;
#endif

  /// Records that the system has been consumed by a solve.
  void markSolved() {
#ifdef EXPENSIVE_CHECKS
    Solved = true;
#endif
  ...
  /// Asserts that the system has not been consumed by a solve yet.
  void assertNotSolved() const {
#ifdef EXPENSIVE_CHECKS
    assert(!Solved && "system has been consumed by a solve");
#endif
  }
