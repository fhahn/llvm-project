...
/// Utility classes to setup casting functionality, as in TypeSwitchTest.cpp.
struct Shape {
...
TEST(STLExtrasTest, MakeIsaRangeReferences) {
  ...
  // Check the addresses of the elements, to ensure they are references to the
  // original objects and not copies.
...
TEST(STLExtrasTest, MakeIsaRangeEarlyIncrement) {
  ...
  // Erasing the current element is safe, as the iterator has already been
  // advanced past it when the body executes.
